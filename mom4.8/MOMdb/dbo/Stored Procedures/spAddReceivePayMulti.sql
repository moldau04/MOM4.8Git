CREATE Procedure [dbo].[spAddReceivePayMulti]
...
BEGIN TRY
	...
	DECLARE db_cursorMul CURSOR FOR 
		SELECT Owner,LocID,Invoice,AmountDue,paymentAmt,CheckNumber,RefTranID FROM @receivePayMulti WHERE Owner!=0 AND LocID!='' AND Invoice!=''
	OPEN db_cursorMul  
	FETCH NEXT FROM db_cursorMul INTO @c_owner,@c_loc,@c_lsInvoice,@c_AmountDue,@c_paymentAmt,@c_CheckNumber,@c_RefTransID
	...
	DECLARE db_cursorCredit CURSOR FOR 
		SELECT Owner,LocID,AmountDue,paymentAmt,CheckNumber,RefTranID FROM @receivePayMulti WHERE Owner!=0 AND LocID<>'' AND Invoice='' AND paymentAmt<>0
	OPEN db_cursorCredit  
	FETCH NEXT FROM db_cursorCredit INTO @c_owner,@c_loc,@c_AmountDue,@c_paymentAmt,@c_CheckNumber,@c_RefTransID
