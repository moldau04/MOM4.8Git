CREATE PROC  [dbo].[spUpdateTestCustomFieldsValueByYear]  
@TestID int,
@EquipmentID int,
...
BEGIN TRANSACTION 
 ...
 FETCH NEXT FROM db_cursor1 INTO @ID, @tblTestCustomFieldsID,@Value, @UpdatedBy,@IsAlert,@TeamMember,@TeamMemberDisplay,@UserRoles,@UserRolesDisplay
    WHILE @@FETCH_STATUS = 0    
	 BEGIN  
	  if Not EXISTS (select 1 from [tblTestCustomFieldsValue] where [TestID]=@TestID and [EquipmentID]= @EquipmentID and [tblTestCustomFieldsID]= @tblTestCustomFieldsID AND TestYear=@TestYear)
		  BEGIN
			  IF (@Value<>'')
				  BEGIN
		  ...
		  BEGIN
		   ...
		   -- Update information for Rol and TeamMember
		    UPDATE [tblTestCustomFieldsValue]
			 ...
			 WHERE [TestID]=@TestID and [EquipmentID]= @EquipmentID and [tblTestCustomFieldsID]= @tblTestCustomFieldsID AND TestYear=@TestYear

           if Not EXISTS (select 1 from [tblTestCustomFieldsValue] where [TestID]=@TestID and [EquipmentID]= @EquipmentID and [tblTestCustomFieldsID]= @tblTestCustomFieldsID and [Value] =@Value AND TestYear=@TestYear)
		   BEGIN

		    SET @oldValue=ISNULL((select [Value] from [tblTestCustomFieldsValue] where [TestID]=@TestID and [EquipmentID]= @EquipmentID and [tblTestCustomFieldsID]= @tblTestCustomFieldsID AND TestYear=@TestYear),'')
		    UPDATE [tblTestCustomFieldsValue]
			   SET 
				  [EquipmentID] = @EquipmentID
