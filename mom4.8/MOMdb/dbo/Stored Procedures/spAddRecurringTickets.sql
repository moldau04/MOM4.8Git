CREATE  PROCEDURE [dbo].[spAddRecurringTickets] @Loc    INT,
                                              @Remarks         VARCHAR(255),
                                              @PerContract     INT,
                                              @ContractRemarks INT,
                                              @Owner           INT,
...
AS 
 ...
 --   @UserID		     INT=1	,
	...
	IF(@StateVal <> '')
		BEGIN
		set @WhereText+=' and l.state = '''+ CONVERT(NVARCHAR(50), @StateVal) + ''''
		END
	  ----### Worker Condition  ###------
    IF( @Worker <> '' )
      BEGIN
...
isnull(ExpirationDate,01/01/1900) ExpirationDate,ctype, isnull(swe,0) as swe	
...
where l.loc is not null and o.status=0 and l.status=0 and j.status=0 and c.status=0 and c.scycle<>-1  
'
          SET @text+=@WhereText
      END
    ELSE
...
------- If PER Equipment Checked
 ...
 SELECT 
	(
	   case WHEN exists (
	   SELECT TOP 1 T.ID  FROM ticketo t 
	   inner join multiple_equipments Mequip on Mequip.ticket_id=t.ID
	   where t.Job = tf.job and t.LID=tf.Loc
       and  convert(datetime, convert(date, isnull(t.Recurring, cast('12/31/9999' as datetime))))  = convert(datetime, convert(date,  tf.scheduledt))
       and Mequip.elev_id=tf.Elev 
	   )
	   then ( 
	   SELECT TOP 1 T.ID  from ticketo t 
	   inner join multiple_equipments Mequip on Mequip.ticket_id=t.ID
	   ...
	   WHEN EXISTS ( select Top 1 t.id  from TicketD t 
	   inner join multiple_equipments Mequip on Mequip.ticket_id=t.ID
	   where t.Job = tf.job and t.Loc=tf.Loc
       and  convert(datetime, convert(date, isnull(t.Recurring, cast('12/31/9999' as datetime))))  = convert(datetime, convert(date,  tf.scheduledt))
        and Mequip.elev_id=tf.Elev  ) 
	   then (
	   select Top 1 t.id  from TicketD t 
	   inner join multiple_equipments Mequip on Mequip.ticket_id=t.ID
	   where t.Job = tf.job and t.Loc=tf.Loc
       and  convert(datetime, convert(date, isnull(t.Recurring, cast('12/31/9999' as datetime))))  = convert(datetime, convert(date,  tf.scheduledt))
        and Mequip.elev_id=tf.Elev  
	   ...
	   when EXISTS( select Top 1 t.id  from ticketo t 
	   where t.Job = tf.job  and t.LID=tf.Loc
       and  convert(datetime, convert(date, isnull(t.Recurring, cast('12/31/9999' as datetime))))  = convert(datetime, convert(date,  tf.scheduledt)) )
	   then (  
	   select Top 1 t.id   from ticketo t 
 ...
 * from #tempWeekendFilter tf   
  ...
  BEGIN 
   SELECT 
    (  
	   [dbo].[GetAlreadyCreatedTicketInCSV](tf.job,tf.scheduledt,tf.Loc,'Ticket') 
    )  
 as	TicketID,  
   (	
       [dbo].[GetAlreadyCreatedTicketInCSV](tf.job,tf.scheduledt,tf.Loc,'Equip')  
    )  
 as	TempEquipmentstr, 
 * from #tempWeekendFilter tf    
...
SELECT 
	(
	   ...
	   inner join multiple_equipments Mequip on Mequip.ticket_id=t.ID
	   where t.Job = tf.job and t.LID=tf.Loc
       and  convert(datetime, convert(date, isnull(t.Recurring, cast('12/31/9999' as datetime))))  = convert(datetime, convert(date,  tf.cdate))
       and Mequip.elev_id=tf.Elev 
	   )
	   then ( 
	   SELECT TOP 1 T.ID  from ticketo t 
