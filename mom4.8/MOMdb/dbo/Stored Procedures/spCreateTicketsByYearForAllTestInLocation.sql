CREATE PROCEDURE [dbo].[spCreateTicketsByYearForAllTestInLocation]
	@LID int,
...
DECLARE cur_Loc CURSOR FOR 	
	select item.LID from LoadTestItem item
	inner join LoadTestItemHistory itemHistory on itemHistory.LID= item.LID
	where itemHistory.TestYear=@TestYear
	and  item.Loc=@curLoc
	--and (select count(1) from LoadTestItemSchedule where LID=item.LID and ScheduledYear=@TestYear and TicketID is null)=1
OPEN cur_Loc  
			...
			BEGIN  
			   set  @NewWorker=(SELECT  w.fDesc FROM tblwork w   WHERE w.id IS NOT NULL  AND w.status=0  and w.id =@c_workerId)
				...
				--	PRINT 'Update Test infor'
					...
					,Worker=@NewWorker
					WHERE [LID]=@LID	AND ScheduledYear=@TestYear AND Worker=@c_worker and ScheduledDate=@schedule
				...
				BEGIN
					...
					BEGIN
						...
						select * from LoadTestItemSchedule order by LID
					END
				END
...
SET @ListTicketID= (select top 1 Convert(varchar,TicketId) from LoadTestItemSchedule where LID=@curLID and ScheduledYear=@TestYear and TicketStatus=1 order by TicketId desc)
