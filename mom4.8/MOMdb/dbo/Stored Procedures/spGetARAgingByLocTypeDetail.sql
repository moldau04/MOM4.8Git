CREATE Procedure [dbo].[spGetARAgingByLocTypeDetail]
	@fDate datetime , 
	@LocType VARCHAR(500),
...
,Original,t.Total as Balance,t.fDesc,
t.Ref,
CASE
 ...
 CASE
       ...
       CASE
           WHEN ((DATEDIFF(DAY, t.fDate, @filterfDate) >= 0)
                 OR (DATEDIFF(DAY, t.fDate, @filterfDate) < 0))
                AND (DATEDIFF(DAY, t.fDate, @filterfDate) <= 7) THEN t.Total
           ELSE 0
       END  as CurrSevenDay,
       CASE
           WHEN (DATEDIFF(DAY, t.fDate, @filterfDate) >= 0)
	...
	,CASE
           WHEN (DATEDIFF(DAY, t.fDate, @filterfDate) >= 0)
                AND (DATEDIFF(DAY, t.fDate, @filterfDate) <= 31) THEN t.Total
           ELSE 0
