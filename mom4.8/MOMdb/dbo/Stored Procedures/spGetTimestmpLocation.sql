 ---   [dbo].[spGetTimestmpLocation] ''

create PROCEDURE [dbo].[spGetTimestmpLocation] 
@tech varchar(100),
@date datetime
AS 
--TicketLocationData

--IF  EXISTS (select 1 from Emp where CallSign=@tech and isnull(DeviceID,'') = '' )
--BEGIN

--EXECUTE  [dbo].[spGetTimestmpLocation2] @tech, @date
--END 
--ELSE
BEGIN  
 ...
 --Map table Data
 DECLARE @MyTableVarCT table(MAPID int, id int,Name varchar(50) ,  latitude varchar(50), longitude varchar(50), date datetime, timestm int, indexID int IDENTITY(1,1) ); 
 DECLARE @MyTableVarOS table(MAPID int, id int,Name varchar(50) ,  latitude varchar(50), longitude varchar(50), date datetime, timestm int, indexID int IDENTITY(1,1) ); 
 DECLARE @MyTableVarER table(MAPID int, id int,Name varchar(50) ,  latitude varchar(50), longitude varchar(50), date datetime, timestm int, indexID int IDENTITY(1,1) ); 
 ...
 from TicketO  t   
  ...
  ------------------------CT--------------------------------------------------------
  insert into @MyTableVarCT (MAPID,id, Name,latitude, longitude,date,timestm)   
  
  Select distinct  m.ID as MAPID, t.id,t.Name, latitude, longitude, date , 1 as timestm from MapDataNew m
  ...
  ----and  date in (select distinct TimeComp from @MyTableVar)    
   ...
   -----------------------OS------------------------------------------------------- 
  insert into @MyTableVarOS(MAPID,id, Name,latitude, longitude,date,timestm)
  Select distinct  m.ID as MAPID, t.id,t.Name, latitude, longitude, date , 2 as timestm from MapDataNew m
inner join @MyTableVar t on m.date=  
  (
 ...
 -------------------------ER-------------------------------------------------------  
 insert into @MyTableVarER(MAPID,id, Name,latitude, longitude,date,timestm)
  Select distinct  m.ID as MAPID, t.id,t.Name, latitude, longitude, date , 3 as timestm  from MapDataNew m 
inner join @MyTableVar t on m.date=  
  (
...
(
select MAX( indexID ) from @MyTableVarER where mapid in (
SELECT MAX(MAPID)
FROM @MyTableVarER
GROUP BY id,timestm)GROUP BY id) 
...
WHERE indexID NOT IN
(
select MAX( indexID ) from @MyTableVarCT where mapid in (
SELECT MAX(MAPID)
FROM @MyTableVarCT
GROUP BY id,timestm)GROUP BY id) 
...
select * from (
  ...
  Select distinct ID as MAPID, 0 as id, '' as name , latitude, longitude, [date] , 0 as timestm, 0 as indexID 
  from MapDataNew 
  where fuser=@tech
  and DATEADD(DAY, DATEDIFF(DAY, 0, date), 0)=@date  
  
  union  
  
  select MAPID,id,name,latitude,longitude,date,timestm, indexID from @MyTableVarER
  
  union 
  
  select MAPID,id,name,latitude,longitude,date,timestm,indexID from @MyTableVarOS 
  
  union 
  
  select MAPID,id,name,latitude,longitude,date,timestm,indexID from @MyTableVarCT
...
END
   ...
   ---[dbo].[spGetTimestmpLocation] 'Artie','2018-01-20'
GO
