CREATE PROCEDURE [dbo].[spUpdateCRUDPermissionLogs]
	...
	BEGIN
		...
		BEGIN 	
			...
			IF(@comPermissionString != @comCurrPermissionString)
				EXEC log2_insert @UpdatedBy,'User',@RefId,@Field,@logCurrPermissionString,@logPermissionString
		END
		...
		BEGIN 	
			SET @comPermissionString = SUBSTRING(@PermissionString,1,1) + SUBSTRING(@PermissionString,3,1)
			SET @comCurrPermissionString = SUBSTRING(@CurrPermissionString,1,1) + SUBSTRING(@CurrPermissionString,3,1)
			Set @logPermissionString = 'Add - ' + SUBSTRING(@PermissionString,1,1)
										--+ ', Edit - ' + SUBSTRING(@PermissionString,2,1)
										+ ', Delete - ' + SUBSTRING(@PermissionString,3,1)
										--+ ', View - ' + SUBSTRING(@PermissionString,4,1)
