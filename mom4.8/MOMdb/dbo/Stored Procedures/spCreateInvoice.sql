...
BEGIN
	...
	BEGIN
		...
		SET @Batch=ISNULL((SELECT   MAX(ISNULL(Batch,0))+1 FROM Trans	),1)										-- Get maximum batch number from trans table.
		...
		SET @CURRENT_INVOICE_NUMBER=isnull((SELECT MAX(ISNULL(Ref,0))+1 FROM Invoice ),1)
		...
		VALUES (@Ref,@ILine,@Acct,@Quan,@fDesc,@Price,@Amount,@IsStax,@job,@Code,@LineTransId,@Measure,@Disc,@StaxAmt,@JobOrg,@Warehouse ,@WHLocID)
		...
		UPDATE d  set ManualInvoice = i.Custom1 from TicketD d inner join Invoice i on i.Ref =d.Invoice   where d.Invoice = @Ref   and ISNULL(ManualInvoice,'')=''   and ISNULL(i.Custom1,'') <> ''
		...
		Begin 	
			...
			SELECT @CurrentCustomer = r.Name FROM  Rol r 
		    INNER JOIN Owner o ON o.Rol = r.ID WHERE o.ID = (Select Owner from Loc where Loc =@loc)
