CREATE PROCEDURE [dbo].[spGetAllTestCoverInLocationWithClassification]
	@Loc INT,
	@TestType Int,
	@YearProposal INT,
	@Chargeable bit,
...
AS
	...
	AND (SELECT count(*) FROM #TEMPSAFETYDETAILS  ts 
				WHERE  ts.TestTypeID= @TestTypeCover and ts.Elev=#TEMPSAFETYDETAILS.Elev and Year(ts.Next)=@YearProposal)<>0
