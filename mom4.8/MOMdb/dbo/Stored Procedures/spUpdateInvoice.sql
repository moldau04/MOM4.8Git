...
OPEN db_cursor  
FETCH NEXT FROM db_cursor INTO @InvID, @Line,@LineItem, @Acct, @Quan, @fDesc, @Price, @Amount, @IsStax, @Job, @Code, @Measure, @Disc, @StaxAmt,@INVType ,@Warehouse ,@WHLocID
...
BEGIN
   ...
   SET @AcctID = ISNULL((SELECT SAcct FROM Inv WHERE ID=@Acct),0)
	...
	INSERT INTO InvoiceI (Ref,Line,Acct,Quan,fDesc,Price,Amount,STax,Job,jobitem,TransID,Measure,Disc,StaxAmt,Warehouse ,WHLocID)
	VALUES (@InvID,@LineItem,@Acct,@Quan,@fDesc,@Price,@Amount,@IsStax,@job,@Code,@LineTransId,@Measure,@Disc,@StaxAmt ,@Warehouse ,@WHLocID)
