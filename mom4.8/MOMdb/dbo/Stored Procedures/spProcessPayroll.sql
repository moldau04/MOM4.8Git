CREATE PROCEDURE [dbo].[spProcessPayroll]          
 (    
...
DECLARE @Ded_JobSpecific int
 ...
 --SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 --INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 --    VALUES (@TransId,@MAXBatch,@CDate,90,0,@PayRegRef,@TransDesc,0,@BankAcctID,@Bank,NULL,0,NULL,0,NULL,NULL)
...
--)    
 ...
 ---------------------------------        
  ...
  ----WHILE ( @WhileLoopCount <= (SELECT COUNT(ID) FROM #tempDetailPay))          
  ----BEGIN          
  -- SET @TransDesc = @Name+' - '+@PeriodDescription+' Week '+@Week          
  -- SELECT  @tAmount = SUM(ISNULL(td.RegAmt, 0))+SUM(ISNULL(td.OTAmt, 0))+SUM(ISNULL(td.NTAmt, 0))+SUM(ISNULL(td.DTAmt, 0))+SUM(ISNULL(td.TTAmt, 0)) FROM #tempDetailPay td          
  -- SELECT TOP 1 @tGL = GL FROM #tempDetailPay          
  -- EXEC @TransId = [dbo].[AddJournal] null,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@tAmount ,@tGL,NULL,NULL,0,NULL,0,NULL,NULL           
 ...
 ---- SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@A_TotalAmt ,@A_GL,NULL,NULL,0,NULL,0,NULL,NULL)
 ----------RESET------->          
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@bonus ,@BonusGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Bonus-------------        
 -- --------------------- Holiday-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@holiday ,@HolidayGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Holiday-------------        
 -- --------------------- Vacation-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@vacation ,@VacationGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Vacation-------------        
 -- --------------------- Zone-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@Zone ,@ZoneGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Zone-------------        
 -- --------------------- Reimbursement-------------        
 ...
 ---- SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@reimb ,@ReimbursementGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Reimbursement-------------        
 -- --------------------- Mileage-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@Milage ,@MileageGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Mileage-------------        
 -- --------------------- Sick-------------        
 ...
 ---- SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@sicktime ,@SickGL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- Sick-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@FIT ,@Payroll_GL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- FEDRAL TAX-------------        
 --  --------------------- STATE TAX-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@SIT ,@Payroll_GL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- STATE TAX-------------        
 --  --------------------- LOCAL TAX-------------        
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@LOCAL ,@Payroll_GL,NULL,NULL,0,NULL,0,NULL,NULL)
 -- END        
 -- --------------------- LOCAL TAX-------------        
 --  --------------------- MEDI-------------    
 --  IF ISNULL(@MEDI,0) >0         
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@MEDI ,@PayrollTax_GL,NULL,NULL,0,NULL,0,NULL,NULL)
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@Ded_DeductionAmount ,@Payroll_GL,NULL,NULL,0,NULL,0,NULL,NULL)
 ...
 ----SET @TransId= ISNULL((SELECT MAX(ISNULL(ID,0))+1 FROM Trans),1)
 ----INSERT INTO Trans ([ID],[Batch],[fDate],[Type],[Line],[Ref],[fDesc],[Amount],[Acct],[AcctSub],[Status],[Sel],[VInt],[VDoub],[EN],[strRef])
 ----    VALUES (@TransId,@MAXBatch,@CDate,91,0,@PayRegRef,@TransDesc,@FICA ,@PayrollTax_GL,NULL,NULL,0,NULL,0,NULL,NULL)
