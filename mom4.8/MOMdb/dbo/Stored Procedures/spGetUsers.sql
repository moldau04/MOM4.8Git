...
AS
    ...
    SET @Text=' select  e.ID, LTRIM(RTRIM(e.fFirst)) as fFirst, LTRIM(RTRIM(e.Last)) as lLast,u.ID as userid, fUser,u.Status,w.super,
	            ...
	            left outer join ' + @DbName + 'tblwork w on u.fuser=w.fdesc
                left outer join ' + @DbName + 'tblUserRole ur on ur.UserId = u.ID
	            left outer join ' + @DbName + 'tblRole ro on ro.Id = ur.RoleId '
    ...
    BEGIN
        IF( @Issuper = 1 )
        BEGIN
            SET @Text += ' where w.super=''' + ISNULL(@super,'') + ''' and ' + @SearchBy + ' like ''' + @SearchValue + '%'''                                                      
        END
        ELSE
        BEGIN
            IF( @SearchBy = 'usertype' )
            BEGIN
                SET @Text += ' where (select case when isnull(fWork,'''')='''' then 0 else 1  end as usertypeid) like ''' + @SearchValue + '%'''
            END
            ELSE
            BEGIN
    ...
    SET @uniontext=' union 
                    ...
                    from ' + @DbName + 'Owner o 
                    left outer join ' + @DbName + 'Rol r on o.Rol=r.ID where internet=1 '
    ...
    BEGIN
        IF( @SearchBy <> 'w.super' AND @Issuper = 0 )
        BEGIN
            IF( @SearchBy <> 'usertype' )
            BEGIN
                SET @Text+=@uniontext
                ...
                IF( @SearchBy <> 'usertype' AND @SearchBy <> 'w.super' )
                BEGIN
                    SET @Text += ' and ' + @SearchBy + ' like ''' + @SearchValue + '%'''
                END
            END
            ELSE IF( @SearchBy = 'usertype' AND @SearchValue = '2' )
            BEGIN
                SET @Text+=@uniontext
            END
        END
