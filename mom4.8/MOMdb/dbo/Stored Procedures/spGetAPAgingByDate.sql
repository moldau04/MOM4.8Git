CREATE PROCEDURE [dbo].[spGetAPAgingByDate]
	@fDate	datetime,
	@EN		INT,
    @UserID	INT,
	@Inclzero INT
	...
	IF @acct!=0
		...
		INSERT INTO #tempAP 
			SELECT 
				...
				,CASE WHEN ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) < 0 
						THEN 0 
				...
				,CASE WHEN (
						...
						AND 
						(ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) <= 7) 
					THEN        
				...
				CASE WHEN (ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) >= 8) 
				...
				CASE WHEN (ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) >= 31) 
						AND (ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) <= 60)   
    				THEN   
    					(ISNULL(APBal.Original,0)-ISNULL(APBal.Paid,0)) 
    				ELSE 0   
				END 
				...
				CASE WHEN (ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) >= 61) 
				...
				CASE WHEN (ISNULL(DATEDIFF(DAY,APBal.Due,@fDate),0) >= 91)  
					THEN       
     					(ISNULL(APBal.Original,0)-ISNULL(APBal.Paid,0))
					ELSE 0      
				END 
			...
			FROM 
				(
					...
					WHERE  --(APBal.Original - APBal.Paid <>0) OR (APBal.Original = 0)
					 CASE @Inclzero 
                WHEN 1 THEN 
                        CASE WHEN (APBal.Original - APBal.Paid <>0) OR (APBal.Original = 0) THEN 1 ELSE 0 END
                ELSE 
                        CASE WHEN (APBal.Original - APBal.Paid <>0)  THEN 1 ELSE 0 END
            END = 1
