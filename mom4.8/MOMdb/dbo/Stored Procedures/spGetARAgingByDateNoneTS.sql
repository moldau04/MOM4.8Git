CREATE PROCEDURE [dbo].[spGetARAgingByDateNoneTS]
	@fDate datetime,
...
BEGIN
	...
	SELECT t1.ID AS TransID,t1.type,
		...
		isnull(t2.Balance,0) AS Total, -- Balance amount
		isnull(t2.Paid,0) AS Paid,
		t1.fDesc,
		isnull(t1.Ref,0) AS Ref,
		isnull((select sum(i2.Amount)*(-1) from InvoiceI i2 inner join Trans t3 on t3.ID=i2.TransID where t3.Ref=t1.Ref and i2.fDesc like '%Retainage%'  and t1.type=1 ) ,0) as RetainageAmount,
		CASE
		...
		CASE
			WHEN ((isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) >= 0)
				OR (isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) < 0))
				AND (isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) <= 7) THEN t2.Balance
			ELSE 0
		END AS CurrSevenDay,
		CASE
			WHEN (isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) >= 0)
		...
		END AS SixtyDay,
		CASE
			WHEN (isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) >= 61) THEN t2.Balance
			ELSE 0
		END AS SixtyOneDay,
		CASE
			WHEN (isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) >= 0)
				AND (isnull(DATEDIFF(DAY, isnull(i.DDate, dbo.GetDueDate(t1.fDate, ISNULL(i.Terms, 0))), @fDate), 0) <= 31) THEN t2.Balance
			ELSE 0
	...
	INNER JOIN
	  ...
	  SELECT ID,Amount, Paid, (ISNULL(Amount, 0)-ISNULL(Paid, 0)) AS Balance, Loc
	   ...
	   WHERE Acct =(SELECT ID FROM Chart  WHERE DefaultNo = 'D1200')
		...
		AND t1.type = 1
		WHERE 1=1
		AND t1.AcctSub in (Select Trans.AcctSub from Trans where Trans.acct=@acct AND Trans.fDate<=@fDate
