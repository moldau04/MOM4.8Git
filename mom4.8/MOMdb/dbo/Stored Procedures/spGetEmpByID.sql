CREATE PROCEDURE [dbo].[spGetEmpByID] @UserID   INT,            
                                      @UserType INT,            
                                      @DbName   VARCHAR(50)            
AS            
...
where  e.ID='            
                   ...
                   --u.Status=0 and            
                     ...
                     + @DbName + 'tblEmailaccounts where userid= (select id from '            
                     + @DbName            
                     + 'tblUser where fuser =(select callsign from '            
                     + @DbName + 'Emp where id ='            
                     + CONVERT(VARCHAR(50), @UserID) + ')) '            
                     --+ CONVERT(VARCHAR(50), @UserID)            
                     + ' select department from ' + @DbName            
            + 'tbljoinempdepartment where emp = '
			+ CONVERT(VARCHAR(50), @UserID)            
			--(select id from '            
              --       + @DbName            
