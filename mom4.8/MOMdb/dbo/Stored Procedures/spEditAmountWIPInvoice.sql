CREATE PROCEDURE [dbo].[spEditAmountWIPInvoice]
@WipID  INT    ,                      
...
AS                
 ...
 BEgin
  ...
  -- Create Invoice item
	...
	DECLARE @c_SalesTax numeric(30,2)
	DECLARE @c_Job int 
		...
		set @c_Price=@c_CompletedThisPeriod
			...
			if @sTaxType<>2
				BEGIN
					if @c_GSTable=1
						BEGIN						
							set @c_GSTAmt=@c_CompletedThisPeriod * ( @GSTRate/100.0)						
						END
				END
			...
			BEGIN
				IF @sTaxType =1
				BEGIN
					SET @c_StaxAmt=(@c_CompletedThisPeriod + @c_GSTAmt)* ( @c_SalesTax/100.0)
                END
                ELSE
                BEGIN
					SET @c_StaxAmt=@c_CompletedThisPeriod * ( @c_SalesTax/100.0)
                END 
...
-- Retainage	
   ...
   SET @RetainageReceivableName  = 'Retainage' 
	...
	OPEN cur_Retainage  
	FETCH NEXT FROM cur_Retainage INTO @c_BillingCode,@c_fDesc,@c_Taxable,@c_RetainageAmount,@c_Job,@c_JobItem,@c_Code
	WHILE @@FETCH_STATUS = 0  
		BEGIN	
			...
			-- Check again
			 ...
			 IF @sTaxType<>2
				BEGIN
					 IF @c_GSTable=1
						BEGIN						
							set @c_GSTAmt=@c_RetainageAmount * ( @GSTRate/100.0)							
						END
				END
			...
			BEGIN
				IF @sTaxType =1
				BEGIN
					SET @c_StaxAmt=(@c_RetainageAmount + @c_GSTAmt)* ( @c_SalesTax/100.0)
                END
                ELSE
                BEGIN
					SET @c_StaxAmt=@c_RetainageAmount * ( @c_SalesTax/100.0)
                END 
  ...
  SET @TaxType=(select Top 1 Stax.Type  from Stax where Stax.Name=@taxRegion)
   ...
   exec spEditInvoice 1, @Invoice,@fdate,@Fdesc,@Amount,@stax,@total,@taxRegion,@taxrate,@Taxfactor,@taxable,@type,@job,@loc,@terms,@PO,@Status,@remarks
   ,@gtax,@mech,@TaxRegion2,@Taxrate2,@BillTo,@Idate,@Fuser,@staxI,'',@InvID,@ddate,@TaxType
