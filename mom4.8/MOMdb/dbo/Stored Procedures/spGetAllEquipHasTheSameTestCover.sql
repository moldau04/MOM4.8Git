CREATE PROCEDURE [dbo].[spGetAllEquipHasTheSameTestCover]
@Loc INT,
@TestType Int,
@YearProposal INT,
@Chargeable bit,
...
and (select count(*) from LoadTestItem  ts where ts.Loc=@Loc and ts.ID= @TestTypeCover and ts.Elev=t.Elev and Year(ts.Next)=@YearProposal)<>0
