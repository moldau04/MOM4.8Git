CREATE PROCEDURE [dbo].[spCreateInvoiceWriteOffCanada]
...
BEGIN
   ...
   BEGIN TRY 
	...
	SET @GTaxAmount = ISNULL((SELECT Convert(NUMERIC(30,2),SUM(GSTAmt))*(-1)  AS GstAmt1 FROM @Invoice ),0)

	----OLD DATA
	--if (select count(1) from InvoiceI where REf=@Ref and GstAmount is null)<>0
	--begin
	--set @GTaxAmount=(select isnull(GTax,0)*(-1) from Invoice where REf in(select top 1 REf from @Invoice ))
	--end
...
SELECT @Ref,@fDate,@fDesc,@Amount,@StaxAmount,
	...
	@ddate,@GSTRate,@AssignedTo,0,@TaxType
		...
		BEGIN
			...
			BEGIN										-- sales tax transaction 
				...
				BEgin
						...
						SET @TransAmount = @item_GSTAmount 
	...
	-------------  $$$$ -- INSERT JOB COST DETAILS ----- $$$$$$$
		...
		VALUES (@Ref,@ILine,@Acct,@Quan*(-1),@fDesc,@Price,@Amount,@IsStax,@job,@Code,@LineTransId,@Measure,@Disc,@StaxAmt*(-1),@JobOrg,@Warehouse ,@WHLocID,@item_GSTAmount*(-1))
