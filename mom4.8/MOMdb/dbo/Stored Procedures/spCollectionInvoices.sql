CREATE PROCEDURE [dbo].[spCollectionInvoices]
...
SELECT * FROM (
					...
					LEFT JOIN	Rol			ON Rol.ID = Owner.Rol
					LEFT JOIN	Branch			ON Branch.ID = Rol.EN
					WHERE #ARAgingTemp.DaysPastDue >= 0
...
--								(Amount120+Amount150+Amount180+Amount360) AS Amount121,
--							CASE t.Type WHEN 1 THEN 'addinvoice.aspx?uid='+ CONVERT(varchar(50),t.Ref)
--									  ELSE 'addreceivedpayment.aspx?id='+ CONVERT(varchar(50),t.Ref)
--									END AS Url,
--							'' AS Contact,
--							'' AS Phone, 
--							'' AS Fax, 
...
--					WHERE DaysPastDue < 0
--					GROUP BY Owner, CustomerName, rol.Contact, rol.Phone, rol.Fax, rol.Cellular, rol.EMail
--)
--	AS t
