CREATE Procedure [dbo].[spGetARAgingByDate]
	@fDate datetime , 
	@HidePartial bit = 0,
...
t.Ref,
isnull((select sum(i2.Amount)*(-1) from InvoiceI i2 inner join Trans t3 on t3.ID=i2.TransID where t3.Ref=t.Ref and i2.fDesc like '%Retainage%'  and t.type=1 ) ,0) as RetainageAmount,
CASE
 ...
 CASE
           WHEN (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) <=1) THEN t.Total
           ELSE 0
       END  AS CurrentDay,
       CASE
           WHEN ((DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) >= 0)
                 OR (DATEDIFF(DAY,ISNULL(t.DDate, t.fDate), @filterfDate) < 0))
                AND (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) <= 7) THEN t.Total
           ELSE 0
       END  as CurrSevenDay,
       CASE
           WHEN (DATEDIFF(DAY,ISNULL(t.DDate, t.fDate), @filterfDate) >= 0)
	...
	,CASE
           WHEN (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) > 30)
                AND (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) <=60) THEN t.Total
           ELSE 0
       END AS SixtyDay
	, CASE
           WHEN (DATEDIFF(DAY,ISNULL(t.DDate, t.fDate), @filterfDate) >= 61) THEN t.Total
           ELSE 0
       END AS SixtyOneDay
	,CASE
           WHEN (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) >= 0)
                AND (DATEDIFF(DAY,ISNULL(t.DDate, t.fDate), @filterfDate) <= 31) THEN t.Total
           ELSE 0
       END AS ZeroThirtyDay
		, CASE
           WHEN (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) >60)
	   ...
	   ,CASE
           WHEN (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) >90)
                AND (DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate)<=120) THEN t.Total
           ELSE 0
		   END as NintyOneDay
        , CASE
							WHEN  DATEDIFF(DAY, ISNULL(t.DDate, t.fDate), @filterfDate) >120 THEN t.Total
							ELSE 0
							END  as OneTwentyDay
	 ...
	 FROM #tempInvoice t
		...
		LEFT JOIN Invoice i ON i.Ref = t.Ref
	WHERE t.total <> 0 AND (@CreditFlag = 0 OR l.CreditFlag = 1)
	ORDER BY LocName
