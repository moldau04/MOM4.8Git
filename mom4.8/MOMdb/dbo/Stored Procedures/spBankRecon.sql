...
BEGIN
	...
	declare @id int
	declare @Reconid int 
	declare @Begningbalance numeric(30,2) 
...
BEGIN TRANSACTION
	...
	SELECT @Begningbalance = Recon from Bank  WHERE ID = @bank 
	...
	begin
		...
		values (@ref, @StatementDate, DATEPART(mm,getdate())+DATEPART(dd,getdate())+DATEPART(yyyy,getdate()), 'Bank reconciliation '+convert(varchar(12),@StatementDate,101), @batch)

		set @fdate = @StatementDate
		
		set @fdesc = 'Bank reconciliation '+convert(varchar(12),@StatementDate,101)
	...
	INSERT INTO BankRecon ([Bank] ,[Begningbalance],[Endbalance],[ReconDate],[ServiceChrg] ,[ServiceAcct],[ServiceDate],[InterestChrg],  
	[InterestAcct],	[InterestDate],[StatementDate] )
	VALUES (@bank,@Begningbalance,@endbalance,@ReconDate,@ServiceChrg,@ServiceAcct,@ServiceDate,@InterestChrg,@InterestAcct,@InterestDate,@StatementDate)
	SELECT @Reconid = SCOPE_IDENTITY()
	INSERT INTO [BankReconI] ([ReconID] ,[TRID] ,[fDate] ,[Type] ,[Ref] ,[Amount] ,	[Batch] ,[TypeNum] ,[Selected] )
	SELECT @Reconid,[ID] ,[fDate] ,	[Type] ,[Ref],[Amount],[Batch],[TypeNum],[Selected] FROM @BankRecon

	UPDATE Trans SET Sel = 1 WHERE ID IN (SELECT ID FROM @BankRecon WHERE TypeNum IN (20,21,5,6))

	UPDATE Trans SET Sel = 1 WHERE	ID IN (SELECT ID FROM @BankRecon WHERE TypeNum IN (30,31)) 
								AND AcctSub = @bank
