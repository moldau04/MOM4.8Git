CREATE PROCEDURE [dbo].[spGetExistTestCoverInLocByTestType]
@Loc INT,
@TestType Int,
@YearProposal INT,
@Chargeable bit,
...
set @TestTypeCover=(select  TestTypeCoverID from TestTypeCover where TestTypeID=@TestType)
...
and t.ID=@TestType
AND h.Chargeable=@Chargeable
and (select count(*) from LoadTestItem  ts
	INNER JOIN LoadTestItemHistory itemHistory ON itemHistory.LID=ts.LID AND itemHistory.TestYear=@YearProposal
	WHERE ts.Loc=@Loc and ts.ID= @TestTypeCover and ts.Elev=t.Elev)<>0
