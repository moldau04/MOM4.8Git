...
AND Dateadd(DAY, Datediff(DAY, 0, edate), 0) = @date  
	...
	if exists (select 1 from TicketLocationData where ticket_id=@ticketID and timeStampType in ('1')  ) and  exists (select 1 from TicketLocationData where ticket_id=@NextID and timeStampType in ('3'))
	begin
	select @lat1=lat,@lon1=lng from TicketLocationData where ticket_id=@ticketID and timeStampType in ('1') 
	select @lat2=lat,@lon2=lng from TicketLocationData where ticket_id=@NextID and timeStampType in ('3')
