...
public partial class PrintBillRegisterGL : System.Web.UI.Page
{
    ...
    GetAPGLRegParam _getAPGLReg = new GetAPGLRegParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    ddlFilterDate.SelectedValue = Request.QueryString["dtype"];
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "BillRegisterGLReport.pdf";
    ...
    protected void StiWebViewerBill_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerBill_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        url = RemoveQueryStringByKey(url, "dtype");
        url = RemoveQueryStringByKey(url, "sd");
        url = RemoveQueryStringByKey(url, "ed");
        url = RemoveQueryStringByKey(url, "type");

        url += "&dtype=" + ddlFilterDate.SelectedValue + "&sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text + "&type=" + type;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/BillRegisterGLReport.mrt");
            if (!string.IsNullOrEmpty(Request["type"]) && Request["type"] == "summary")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/BillRegisterGLSummaryReport.mrt");
            }
            ...
            if (!string.IsNullOrEmpty(Request["dtype"]))
            {
                objPJ.Custom = Request.QueryString["dtype"];
            }

            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objPJ.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
            }
            else
            {
                objPJ.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }

            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objPJ.EndDate = Convert.ToDateTime(Request.QueryString["ed"]).AddDays(1).AddSeconds(-1);
            }
            else
            {
                objPJ.EndDate = DateTime.Now.Date.AddDays(1).AddSeconds(-1);
            }

            if (Request["stype"] != null && Request["stext"] != null)
            {
                var searchType = Request["stype"];
                var searchText = Request["stext"];

                if (searchType == "PO")
                {
                    objPJ.PO = string.IsNullOrEmpty(searchText) ? 0 : Convert.ToInt32(searchText);
                }
                else if(searchType == "Projectnumber")
                {
                    objPJ.ProjectNumber = string.IsNullOrEmpty(searchText) ? 0 : Convert.ToInt16(searchText);
                }
                else if(searchType == "Vendor")
                {
                    objPJ.Vendor = string.IsNullOrEmpty(searchText) ? 0 : Convert.ToInt16(searchText);
                }
                else if(searchType == "Amount")
                {
                    objPJ.Amount = string.IsNullOrEmpty(searchText) ? 0 : Convert.ToDouble(searchText);
                }
                else if(searchType == "AmountDue")
                {
                    objPJ.Balance = string.IsNullOrEmpty(searchText) ? 0 : Convert.ToDouble(searchText);
                }
                else if(searchType == "VendorName")
                {
                    objPJ.vendorName = searchText;
                }
                else if(searchType == "Custom1")
                {
                    objPJ.Custom1 = searchText;
                }
                else if(searchType == "Custom2")
                {
                    objPJ.Custom2 = searchText;
                }

                if(searchType == "Status")
                {
                    objPJ.Status = Convert.ToInt16(searchText);
                }
                else
                {
                    objPJ.Status = 99;
                ...
                if (searchType == "Code")
                {
                    objPJ.Terms = Convert.ToInt16(searchText);
                }
                else
                {
                    objPJ.Terms = 99;
            ...
            if (!string.IsNullOrEmpty(Request["itype"]))
            {
                if (Request["itype"].Equals(2))
                {
                    ...
                    {
                        objPJ.SearchDate = objPJ.EndDate;
                    }
                }
            }
            ...
            {
                objPJ.Ref = Request["ref"];
            }

            var inclClose = false;
            if (!string.IsNullOrEmpty(Request["inclClose"]))
            {
                inclClose = Convert.ToBoolean(Request["inclClose"]);
            }
            ...
            if (Session["Bills_Filters"] != null)
            {
                filters = (List<RetainFilter>)Session["Bills_Filters"];
            }

            DataSet ds = objBL_Bills.GetAPGLReg(objPJ, filters, inclClose);
            ...
            report.Dictionary.Variables["StartDate"].Value = objPJ.StartDate.ToString();
            report.Dictionary.Variables["EndDate"].Value = objPJ.EndDate.ToString();
            report.Render();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Bill Register GL Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Bill Register GL Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "BillRegisterGLReport.pdf")
                        {
                ...
                mail.FileName = "BillRegisterGLReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "BillRegisterGLReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=BillRegisterGLReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Bill Register GL Report.";
