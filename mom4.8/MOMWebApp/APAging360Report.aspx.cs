...
public partial class APAging360Report : System.Web.UI.Page
{
    #region Variables
    ...
    GetBillsDetails360ByDueParam _GetBillsDetails360ByDue = new GetBillsDetails360ByDueParam();
    GetAPAging360ByDateParam _GetAPAging360ByDate = new GetAPAging360ByDateParam();
    #endregion
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        var url = "APAging360Report.aspx?stype=" + ddlInvoice.SelectedValue + "&sdate=" + txtSearchDate.Text + "&type=" + type;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/APAging360Report.mrt");

            if (!string.IsNullOrEmpty(Request["type"]) && Request["type"] == "summary")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/APAgingSummary360Report.mrt");
            }
            ...
            //API
            _GetBillsDetails360ByDue.ConnConfig = connString;

            _GetAPAging360ByDate.ConnConfig = connString;
            _GetAPAging360ByDate.UserID = Convert.ToInt32(Session["UserID"].ToString());

            if (!string.IsNullOrEmpty(Request["stype"]))
            {
                objPJ.SearchValue = Convert.ToInt16(Request.QueryString["stype"]);
                _GetBillsDetails360ByDue.SearchValue = Convert.ToInt16(Request.QueryString["stype"]);
            }
            else
            {
                objPJ.SearchValue = 3;
                _GetBillsDetails360ByDue.SearchValue = 3;
            }

            if (!string.IsNullOrEmpty(Request["sdate"]))
            {
                objPJ.SearchDate = Convert.ToDateTime(Request.QueryString["sdate"]);
                _GetBillsDetails360ByDue.SearchDate = Convert.ToDateTime(Request.QueryString["sdate"]);
                _GetAPAging360ByDate.SearchDate = Convert.ToDateTime(Request.QueryString["sdate"]);
            }
            else
            {
                objPJ.SearchDate = DateTime.Now;
                _GetBillsDetails360ByDue.SearchDate = DateTime.Now;
                _GetAPAging360ByDate.SearchDate = DateTime.Now;
            }
            ...
            if (Session["CmpChkDefault"].ToString() == "1")
            {
                ...
                _GetAPAging360ByDate.EN = 1;
            }
            else
            {
                objPJ.EN = 0;

                _GetAPAging360ByDate.EN = 0;
            }
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                if (_GetBillsDetails360ByDue.SearchValue != 3)
                {
                    string APINAME = "BillAPI/BillsReport_GetBillsDetails360ByDue";

                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetBillsDetails360ByDue, true);
                    JavaScriptSerializer serializer = new JavaScriptSerializer();

                    serializer.MaxJsonLength = Int32.MaxValue;
                    _lstGetBillsDetails360ByDue = serializer.Deserialize<List<GetBillsDetails360ByDueViewModel>>(_APIResponse.ResponseData);
                    ds = CommonMethods.ToDataSet<GetBillsDetails360ByDueViewModel>(_lstGetBillsDetails360ByDue);
                    ...
                    report.Dictionary.Variables["EndDate"].Value = _GetBillsDetails360ByDue.SearchDate.ToLongDateString();
                }
                else
                {
                    _GetAPAging360ByDate.fDate = _GetAPAging360ByDate.SearchDate;

                    string APINAME = "BillAPI/BillsReport_GetAPAging360ByDate";

                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetAPAging360ByDate, true);
                    JavaScriptSerializer serializer = new JavaScriptSerializer();

                    serializer.MaxJsonLength = Int32.MaxValue;
                    _lstGetAPAging360ByDate = serializer.Deserialize<List<GetAPAging360ByDateViewModel>>(_APIResponse.ResponseData);
                    ds = CommonMethods.ToDataSet<GetAPAging360ByDateViewModel>(_lstGetAPAging360ByDate);
                    ...
                    report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
                    report.Dictionary.Variables["EndDate"].Value = _GetAPAging360ByDate.SearchDate.ToLongDateString();
                }
            }
            else
            {
                ...
                {
                    ds = objBL_Bills.GetBillsDetails360ByDue(objPJ);
                ...
                else
                {
                    ...
                    ds = objBL_Bills.GetAPAging360ByDate(objPJ);
