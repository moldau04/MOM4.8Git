...
public partial class LeadListingReport : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Customer Name" || sortBy == "Sales Person")
                    {
                        sortBy = "[" + sortBy + "]";
                    }
                }
    ...
    {
        DataSet dsGetEstimateReportFiltersValue = new DataSet();
        objProp_User.DBName = Session["dbname"].ToString();
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetEstimateReportFiltersValue = objBL_ReportsData.GetLeadReportFiltersValue(objProp_User);

        if (dsGetEstimateReportFiltersValue.Tables[0].Rows.Count > 0)
        {
            drpProspect.DataSource = dsGetEstimateReportFiltersValue.Tables[0];
            drpProspect.DataTextField = "Prospect#";
            drpProspect.DataValueField = "Prospect#";
            drpProspect.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[1].Rows.Count > 0)
        {
            drpType.DataSource = dsGetEstimateReportFiltersValue.Tables[1];
            drpType.DataTextField = "Type";
            drpType.DataValueField = "Type";
            drpType.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[2].Rows.Count > 0)
        {
            drpCustomerName.DataSource = dsGetEstimateReportFiltersValue.Tables[2];
            drpCustomerName.DataTextField = "Customer Name";
            drpCustomerName.DataValueField = "Customer Name";
            drpCustomerName.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[3].Rows.Count > 0)
        {
            drpName.DataSource = dsGetEstimateReportFiltersValue.Tables[3];
            drpName.DataTextField = "Name";
            drpName.DataValueField = "Name";
            drpName.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[4].Rows.Count > 0)
        {
            drpCity.DataSource = dsGetEstimateReportFiltersValue.Tables[4];
            drpCity.DataTextField = "City";
            drpCity.DataValueField = "City";
            drpCity.DataBind();
        }
        if (dsGetEstimateReportFiltersValue.Tables[5].Rows.Count > 0)
        {
            drpState.DataSource = dsGetEstimateReportFiltersValue.Tables[5];
            drpState.DataTextField = "State";
            drpState.DataValueField = "State";
            drpState.DataBind();
        }      

        if (dsGetEstimateReportFiltersValue.Tables[6].Rows.Count > 0)
        {
            drpStatus.DataSource = dsGetEstimateReportFiltersValue.Tables[6];
            drpStatus.DataTextField = "Status";
            drpStatus.DataValueField = "Status";
            drpStatus.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[7].Rows.Count > 0)
        {
            drpDays.DataSource = dsGetEstimateReportFiltersValue.Tables[7];
            drpDays.DataTextField = "#Days";
            drpDays.DataValueField = "#Days";
            drpDays.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[8].Rows.Count > 0)
        {
            drpOpps.DataSource = dsGetEstimateReportFiltersValue.Tables[8];
            drpOpps.DataTextField = "#Opps";
            drpOpps.DataValueField = "#Opps";
            drpOpps.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[9].Rows.Count > 0)
        {
            drpSalesPerson.DataSource = dsGetEstimateReportFiltersValue.Tables[9];
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            dsGetCustDetails = objBL_ReportsData.GetLeadDetails(objProp_User);
            ...
            if (dsGetCustDetails.Tables.Count > 0)
            {
                ...
                dsGetCustDetails.Tables[0].TableName = "Lead";
    ...
    {       
        ...
        {
            query += " FROM vw_LeadReportDetails v order by " + sortBy;
        }
        else
        {
            ...
            if (selectedFiltersColumns != null)
            {
                for (int i = 0; i <= selectedFiltersColumns.Count() - 1; i++)
                {
                    ...
                    if (selectedFiltersColumns[i] == "SalesPerson")
                    {
                        selectedFiltersColumns[i] = "Sales Person";
                    }
            ...
            query += " FROM vw_LeadReportDetails v where " + filters + " order by " + sortBy;
        }
    ...
    public static string GetLeadPreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            LeadListingReport _a = new LeadListingReport();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (DataSortBy == "Customer Name" || DataSortBy == "Sales Person")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_LeadReportDetails v order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_LeadReportDetails v where " + filters + " order by " + DataSortBy;
            }
            #endregion
