...
public partial class UTaxLocReport : System.Web.UI.Page
{
    #region "Variables"
    ...
    BL_Invoice objBL_Invoices = new BL_Invoice();
    ...
    GetUTaxLocReportParam _getUTaxLocReport = new GetUTaxLocReportParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtToDate.Text = now.ToString("MM/dd/yyyy");
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "UTaxLocationReport.pdf";
    ...
    {
        Response.Redirect("managebills.aspx?f=c");
    }

    protected void btnSearch_Click(object sender, EventArgs e)
    {
        var url = "UTaxLocReport.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/UTaxLocJobReport.mrt");
            ...
            objInvoices.ConnConfig = connString;

            _getUTaxLocReport.ConnConfig = connString;

            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objInvoices.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
                _getUTaxLocReport.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
            }
            else
            {
                objInvoices.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                _getUTaxLocReport.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }

            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objInvoices.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
                _getUTaxLocReport.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
            }
            else
            {
                objInvoices.EndDate = DateTime.Now;
                _getUTaxLocReport.EndDate = DateTime.Now;
            }

            DataSet ds = new DataSet();
            List<GetUTaxLocReportViewModel> _lstGetUTaxLocReport= new List<GetUTaxLocReportViewModel>();
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "BillAPI/BillsReport_GetUTaxLocReport";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getUTaxLocReport, true);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetUTaxLocReport = serializer.Deserialize<List<GetUTaxLocReportViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<GetUTaxLocReportViewModel>(_lstGetUTaxLocReport);
            }
            else
            {
                ds = objBL_ReportsData.GetUTaxLocReport(objInvoices);
            }
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                ...
                report.Dictionary.Variables["StartDate"].Value = _getUTaxLocReport.StartDate.ToLongDateString();
                report.Dictionary.Variables["EndDate"].Value = _getUTaxLocReport.EndDate.ToLongDateString();
            }
            else
            {
                report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
                report.Dictionary.Variables["StartDate"].Value = objInvoices.StartDate.ToLongDateString();
                report.Dictionary.Variables["EndDate"].Value = objInvoices.EndDate.ToLongDateString();
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "UTax Location Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the UTax Location Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "UTaxLocationReport.pdf")
                        {
                ...
                mail.FileName = "UTaxLocationReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "UTaxLocationReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=UTaxLocationReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached UTax Location Report.";
