<%@ Page Title="" Language="C#" MasterPageFile="~/Mom.master" AutoEventWireup="true" MaintainScrollPositionOnPostback="true" Inherits="AddInvoice" Codebehind="AddInvoice.aspx.cs" ValidateRequest="false"%>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <telerik:RadScriptBlock ID="RadScriptBlockFormattedName" runat="server">
        ...
        <script type="text/javascript">   
            ...
            $(document).ready(function () {
                ...
                $("#<%= chkrecurring.ClientID %>").change(function () {
                    if ($(this).is(':checked')) {
                        var st = $('#<%=hdnIsContract.ClientID%>').val();
                     
                        if (st == '0') {
                            $("#<%=chkrecurring.ClientID%>").prop('checked', false);
                            noty({
                                text: 'Cannot create Recurring Invoice for Projects',
                                type: 'warning',
                                layout: 'topCenter',
                ...
                $("[id*=txtBillingCode]").autocomplete({
                    ...
                    select: function (event, ui) {
                        ...
                        var txtBCodeType = document.getElementById(txtBillingCode.replace('txtBillingCode', 'txtBCodeType'));
                         var hdnQtyOnHand = document.getElementById(txtBillingCode.replace('txtBillingCode', 'hdnQtyOnHand'));
                        ...
                        if (ui.item.id == null) {
                            ...
                            try {
                                ...
                                if ($(hdnAStatus).val() == "1" || $(hdnStatus).val() == "1") {
                                     DisplayNotification("This Account\\Billing code is inactive. Please change account\\billing code name before proceeding.");
                ...
                $("[id*=txtGvWarehouse]").autocomplete({
                    ...
                    source: function (request, response) {
                        ...
                        $.ajax({
                            ...
                            error: function (result) {
                                DisplayNotification("Due to unexpected errors we were unable to load account name");
                            }
                        });
                    },
                    select: function (event, ui) {
                        try {
                            ...
                            //// Check ON Hand
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        if (data.d == "false") {
                                            DisplayNotification("Item selected is not on hand, please choose another one.");
                                            $(txtGvWarehouse).val('');
                                            $(hdnWarehouse).val('');
                                        }
                                    },
                                    error: function (result) {
                                        // DisplayNotification("Due to unexpected errors we were unable to load item.");
                                    },
            ...
            $("#<%=txtProject.ClientID%>").autocomplete({
                ...
                source: function (request, response) {
                    ...
                    $.ajax({
                            ...
                            error: function (result) {
                                DisplayNotification("Due to unexpected errors we were unable to load projects");
                            }
                        });
                    },
                select: function (event, ui) {
                    ...
                    if (ui.item.IsContract == '1') {
                      
                        $("#<%=hdnIsContract.ClientID%>").val("1");
                    }
                    else {
                        
                             $("#<%=hdnIsContract.ClientID%>").val("0"); 
                        $("#<%=chkrecurring.ClientID%>").prop('checked', false);
                    }
    ...
    <script type="text/javascript">
        ...
        function validateBillingQuantity() {
            ...
            for (var i = 0; i < count; i++) {
                ...
                var hand = item.findElement("hdnQtyOnHand");
                var quan = item.findElement("lblQuantity");
                var code = item.findElement("txtBillingCode");
                var txtBCodeType = item.findElement("txtBCodeType");
                if (txtBCodeType.value == "0") {
                    if (parseFloat(quan.value) > parseFloat(hand.value)) {
                        return code.value;
                    }
                }
        ...
        //ES-7744-Gable || Billing/Invoice ||Error Message When Generating Invoices
        function DueDateInvoiceValidation() { 
            ...
            if (InvoiceDate > DueDate) {   return false;  }  return true;
         }

        function ZeroAmtValidate(control) {

            /////  $$$$  If Inventory Tracking ON then allow  to validate $$$$ ////////
            ...
            if ($('#<%=hdnISInventoryTrackingON.ClientID%>').val() == '1') {
                ...
                strBilling = validateBillingQuantity();

            }

            var validDueDate = DueDateInvoiceValidation();
            if (!validDueDate) {
                 noty({
                                text: `the due date can’t be prior than the invoice date`,
                                type: 'warning',
                                layout: 'topCenter',
                                closeOnSelfClick: false,
                                timeout: 4000,
                ...
                //DisplayNotification(`the due date can’t be prior than the invoice date`);
                return false;
            }
            if (strBilling!='') {
                 noty({
                                text: 'You do not have enough on hand for item ' + strBilling,
                                type: 'warning',
                                layout: 'topCenter',
                                closeOnSelfClick: false,
                                timeout: 4000,
                ...
                if (validated) {
                    ...
                    if (project === "") {
                        ...
                        for (var i = 0; i < count; i++) {
                            item = masterTable.get_dataItems()[i];
                            var code = item.findElement("ddlProjectCode");
                            var amt = item.findElement("lblQuantity");
                         
                            var amtVal = $(amt).val();
                            if ($(code).val() === "0" && amtVal !== 0) {

                                    noty({
                                    text: 'Please select the Code.',
                                    type: 'warning',
                                    layout: 'topCenter',
                                    closeOnSelfClick: false,
                                    timeout: 4000,
            ...
            $("#<%=txtCustomer.ClientID%>").autocomplete({
                ...
                source: function (request, response) {
                    ...
                    $.ajax({
                        ...
                        error: function (result) {
                            DisplayNotification("Due to unexpected errors we were unable to load customers");
                        }
                    });
                },
                select: function (event, ui) {
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
                                ...
                                <div class="row">
                                    ...
                                    <div class="buttonContainer">
                                        ...
                                        <ul id="dropdown1" class="dropdown-content">
                                            <li>
                                                <asp:LinkButton ID="lnkPDF" runat="server" Enabled="true" OnClick="lnkPDF_Click"> <i class="fa fa-file-pdf-o email_op"  aria-hidden="true"></i>&nbsp;&nbsp; Invoice <i class="fa fa-download down_icon" aria-hidden="true" ></i></asp:LinkButton>
                                            </li>
                                            <li>
                                       ...
                                       <%-- <div class="btnlinks">
                                        ...
                                        <div class="btnlinks">
                                            <a class="dropdown-button" href="#!" data-activates="dropdownPreview">Preview
                                            </a>
                                        </div>
                                          <ul id="dropdownPreview" class="dropdown-content w-200" >
                                            <li>
                                                <asp:LinkButton ID="lnkPreviewEmail" runat="server" Text="Invoice" OnClientClick="return ZeroAmtValidate(this);" OnClick="lnkPreviewEmail_Click" Visible="true"></asp:LinkButton>
                                            </li>
                                            <li>
                                                <asp:LinkButton ID="lnkPreviewEmailInvTicket" runat="server" CausesValidation="true" OnClick="lnkPreviewEmailInvTicket_Click" OnClientClick="return ZeroAmtValidate(this);" Enabled="true">Invoice With Ticket</asp:LinkButton>
                                            </li>
                                        </ul>
                                        <div class="btnlinks">
                        ...
                        <div class="row">
                            <div class="tblnks">
                                <ul class="anchor-links">
                                    ...
                                    <li id="liEmailLogs" runat="server" style="display: none"><a href="#accrdEmaillogs">Email History Log</a></li>
            ...
            <div class="col s12 m12 l12">
                <div class="row">
                                        ...
                                        <div class="form-section-row ">
                                            <div class="form-input-row">
                                                    ...
                                                    <div class="input-field col s12">
                                                        <div class="row">
                                                         ...
                                                         <div class="srchclr btnlinksicon rowbtn">
                                                            <asp:HyperLink for="txtCustomer" ID="lnkCustomerID" Visible="true" Target="_blank" runat="server"><i class="mdi-social-people ml" ></i></asp:HyperLink>
                                                        </div>
                                                    </div>
                                                    <div class="input-field col s12">
                                                        <div class="row">
                                                ...
                                                <div class="form-section3">
                                                    ...
                                                    <div class="input-field col s12 mt-10" >
                                                        <div class="row">
                                                ...
                                                <div class="form-section3 invoicemgncustom">
                                                      ...
                                                      <div class="form-section4 w-100p" >
                                                            ...
                                                            <asp:TextBox ID="txtInvoiceDate" CssClass="datepicker_mom" runat="server" MaxLength="50"></asp:TextBox>
                                                                    ...
                                                                    <div class="input-field col s4">
                                                                                <div class="row">
                                                                                     <div class="checkrow">
                                                                                     <asp:CheckBox ID="chkrecurring" runat="server" CssClass="css-checkbox" Text="Recurring"    />
                                                                                         </div>
                                                                                </div>
                                                                            </div>
                                                      </div>
                                                    ...
                                                    <div class="input-field col s12">
                                                        <div class="row">
                                                            ...
                                                            <asp:HiddenField ID="hdnIsContract" runat="server" value=""/>
                                                        </div>
                                                    </div>

                                                    <div class="srchclr btnlinksicon rowbtn">
                                                            <asp:HyperLink for="txtProject" ID="lnkProjectID" Target="_blank" runat="server"><i runat="server" id="iProIcon" class="mdi-notification-play-install ml" style="margin-left:0px !important;"></i></asp:HyperLink>
                                                        </div>

                                                     <div id="divProject" class="grid_container set-project" >
                                                        <div class="form-section-row setsub-project" >
                                                            ...
                                                            <div id="dvprojectSlider slider-css"   style="position:absolute; max-height:200px; overflow:auto;z-index:99; background-color:white;border:solid 1px  #9e9e9e;box-shadow: 0 5px #ccc;border-radius: 5px !important">
                                                            ...
                                                            <telerik:RadListView ID="RadProjectListView" runat="server" RenderMode="Auto" 
                                                                ...
                                                                <ItemTemplate>
                                                                        ...
                                                                        <li class="ui-menu-item ui-css" style="border:solid dashed 1px">
                                                    ...
                                                    <div class="form-section4" style="width:100% !important;" runat="server" id="ProjectBillingInfo" visible="false">
                                                                              <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtBillingType" disabled="disabled" runat="server" ReadOnly="true" AutoCompleteType="None"
                                                                            ...
                                                                            <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtBillRate" disabled="disabled" runat="server"  ReadOnly="true"  AutoCompleteType="None"
                                                                                        MaxLength="15" ></asp:TextBox>
                                                                                </div>
                                                                            </div>
                                                                            <div class="input-field col s2">
                                                                            ...
                                                                            <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtOt" runat="server" disabled="disabled" ReadOnly="true"  AutoCompleteType="None"
                                                                                        MaxLength="15" ></asp:TextBox>
                                                                                </div>
                                                                            </div>
                                                                            <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtNt" runat="server" disabled="disabled"  ReadOnly="true"  AutoCompleteType="None"
                                                                                        MaxLength="15" ></asp:TextBox>
                                                                                </div>
                                                                            </div>
                                                                            <div class="input-field col s2">
                                                                            ...
                                                                            <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtDt" runat="server" disabled="disabled" ReadOnly="true"  AutoCompleteType="None"
                                                                                        MaxLength="15" ></asp:TextBox>
                                                                                </div>
                                                                            </div>
                                                                            <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtTravel" runat="server"  ReadOnly="true" disabled="disabled"  AutoCompleteType="None"
                                                                                        MaxLength="15" ></asp:TextBox>
                                                                                </div>
                                                                            </div>
                                                                            <div class="input-field col s2">
                                                                            ...
                                                                            <div class="input-field col s5">
                                                                                <div class="row">
                                                                                    ...
                                                                                    <asp:TextBox ID="txtMileage" runat="server" disabled="disabled"  ReadOnly="true"  AutoCompleteType="None"
                                                                                        MaxLength="15" ></asp:TextBox>
                                                                                </div>
                                                                            </div>
                                                             </div>
                                                ...
                                                <div class="collapsible-body" id="divBillingDetails">
                                                    <div class="form-content-wrap">
                                                        <div class="form-content-pd">
                                                                            ...
                                                                            <div class="form-sectioninvoice">
                                                                                ...
                                                                                <div class="input-field col s12">
                                                                                    <div class="row">
                                                                                        ...
                                                                                        <%--Status Dropdown--%>
                                                                                        <asp:DropDownList ID="ddlStatus" runat="server" CssClass="browser-default" Enabled="false">
                                                            ...
                                                            <div class="form-section-row">
                                                                <div class="form-section-row" style="margin-bottom: 10px">
                                                                    <div class="btnlinks">
                    ...
                    OnClick="btnCopyPrevious_Click" Text="Copy Previous" Style="display: none;"></asp:LinkButton>
                                                                ...
                                                                <div class="grid_container" style="overflow: scroll;">
                                                                    ...
                                                                    <div class="RadGrid RadGrid_Material FormGrid">
                                                                        ...
                                                                        <telerik:RadGrid RenderMode="Auto" ID="RadGrid_gvInvoices" HeaderStyle-CssClass="cent"
                                                                            runat="server" AllowSorting="true" OnPreRender="RadGrid_gvInvoices_PreRender" 
                                                                            OnItemCreated="RadGrid_gvInvoices_ItemCreated"                                                                          
                                                                            onblur="resetIndexF6()">
                                                                            <CommandItemStyle />
                                                                            ...
                                                                            <MasterTableView AutoGenerateColumns="false" ShowFooter="True" DataKeyNames="Line">
                                                                                <Columns>
                                                                                    ...
                                                                                    <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="Code" HeaderStyle-Width="170" FooterStyle-HorizontalAlign="left" UniqueName="ProjectCode">
                                                                                    ...
                                                                                    <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="Quan" UniqueName="Quan" HeaderStyle-Width="70">
                                                                                        <ItemStyle HorizontalAlign="Center"></ItemStyle>
                                                                                        <ItemTemplate>
                                                                                    ...
                                                                                    <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="Billing Code" HeaderStyle-Width="210" UniqueName="BillingCode">
                                                                                        <ItemTemplate>
                                                                                           ...
                                                                                           <asp:HiddenField ID="hdnAStatus" runat="server" Value='<%# Eval("AStatus") %>'></asp:HiddenField>
                                                                                            ...
                                                                                            <asp:HiddenField ID="hdnWarehouseLocationID" Value="0" runat="server"></asp:HiddenField>
                                                                                              ...
                                                                                              <asp:HiddenField ID="hdnQtyOnHand" Value="0" runat="server"></asp:HiddenField>
                                                                                    ...
                                                                                    <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="Price Per" UniqueName="Price" ItemStyle-HorizontalAlign="Right" FooterStyle-HorizontalAlign="Right" HeaderStyle-Width="100" FooterStyle-Width="100">
                                                                                        ...
                                                                                        <FooterTemplate>
                                                                                            <asp:Label ID="lblPricePerTotal" class="lbl-price" runat="server" ></asp:Label>
                                                                                        </FooterTemplate>
                                                                                    </telerik:GridTemplateColumn>
                                                                                    ...
                                                                                    <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="Pretax Amount" UniqueName="priceQuant" ItemStyle-CssClass="cent" FooterStyle-CssClass="cent" ItemStyle-HorizontalAlign="Right" FooterStyle-HorizontalAlign="Right" HeaderStyle-Width="100" FooterStyle-Width="100">
                                                                                        <ItemTemplate>
                                                                                            <asp:Label Style="font-weight: bold; color: #2392D7; text-align: center;" class="pricequant-css" ID="lblPretaxAmount" runat="server"
                                                                                   ...
                                                                                   <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="GST" UniqueName="EnableGSTTax"  ItemStyle-CssClass="left-align" HeaderStyle-CssClass="left-align"  ItemStyle-HorizontalAlign="Left"  ItemStyle-Width="100" HeaderStyle-Width="100" FooterStyle-Width="100">
                                                                                    ...
                                                                                    <telerik:GridTemplateColumn AllowFiltering="false" ShowFilterIcon="false" HeaderText="Amount" UniqueName="Amount" HeaderStyle-CssClass="cent" ItemStyle-CssClass="cent" FooterStyle-CssClass="cent" HeaderStyle-Width="90" FooterStyle-Width="100">
                                                                                        <ItemTemplate>
                                            ...
                                            <li id="tbEmailLogs" runat="server" style="display: none">
                                                <div id="accrdEmaillogs" class="collapsible-header accrd  accordian-text-custom"><i class="mdi-content-content-paste"></i>Email History Log</div>
                                                <div class="collapsible-body">
                                                    <div class="form-content-wrap">
                                                        <div class="form-content-pd">
                                                            <div class="grid_container">
    ...
    <asp:HiddenField runat="server" ID="hdnIsOldData"/>
    ...
    <script type="text/javascript">
        ...
        function CalculateGridAmount() {
            try {
                ...
                var hdnIsOldData = document.getElementById('<%=hdnIsOldData.ClientID%>');
                    ...
                    for (i = 0; i < masterTable.get_dataItems().length; i++) {
                        ...
                        if (cell.childNodes[1].value != '' && cell1.childNodes[1].value != '') {
                          // pretax = parseFloat((parseFloat(cell.childNodes[1].value.toString().replace(/[\$\(\),]/g, '')) * parseFloat(cell1.childNodes[1].value.toString().replace(/[\$\(\),]/g, '')))).toFixed(2);
                             pretax = (parseFloat(cell.childNodes[1].value.toString().replace(/[\$\(\),]/g, '')) * parseFloat(cell1.childNodes[1].value.toString().replace(/[\$\(\),]/g, '')));
                        
                            cell3.childNodes[1].innerHTML = roundNumber(pretax).toLocaleString("en-US", { minimumFractionDigits: 2 });
                        }
                        //GST
                        ...
                        if (isGst == 1) {
                            ...
                            if (cell6 != null) {
                                cell6.childNodes[1].value = (roundNumber(gtaxAmt)).toLocaleString("en-US", { minimumFractionDigits: 2 });
                            }
                        ...
                        if (hdnsTaxType.value == '3') {
                            ...
                            if (stax.value != '' && cell.childNodes[1].value != '' && cell1.childNodes[1].value != '') {
                                ...
                                if (cell5.childNodes[1].checked == true) {
                                    if (hdnsTaxType.value == '1') {
                                        if (hdnIsOldData.value == '1') {
                                            ...
                                            staxAmt = staxAmt == 0 ? staxAmt : staxAmt * (-1);
                                        } else {
                                ...
                                cell2.childNodes[1].value = (roundNumber(staxAmt)).toLocaleString("en-US", { minimumFractionDigits: 2 });
                        ...
                        if (cell.childNodes[1].value != '' && cell1.childNodes[1].value != '') {
                              total = parseFloat(pretax) + parseFloat(cell2.childNodes[1].value.replace(/[\$\(\),]/g, ''));
                           // total = parseFloat(pretax) + parseFloat(staxAmt);
                            if (isGst == 1) {
                                if (cell6 == null) {
                                    total = total + parseFloat(gtaxAmt);
                                } else {
                                    total = total + parseFloat(cell6.childNodes[1].value.replace(/[\$\(\),]/g, ''));
                                }
                               // total = total + parseFloat(gtaxAmt);
                                
                            }
                            cell4.childNodes[1].innerHTML = (roundNumber(total)).toLocaleString("en-US", { minimumFractionDigits: 2 });
                        }
                        if (cell1.childNodes[1].value != '') {
                            PricePerTotal =roundNumber( (parseFloat(PricePerTotal) + parseFloat(cell1.childNodes[1].value.toString().replace(/[\$\(\),]/g, '')))) ;
                        }
                        if (cell3.childNodes[1].innerHTML != '') {
                            PretaxAmountTotal = roundNumber((parseFloat(PretaxAmountTotal) + parseFloat(cell3.childNodes[1].innerHTML.toString().replace(/[\$\(\),]/g, ''))));
                        }
                        if (cell2 != null) {
                            if (cell2.childNodes[1].value != '') {

                            SalesTaxamountTotal =roundNumber((parseFloat(SalesTaxamountTotal) + parseFloat(cell2.childNodes[1].value.toString().replace(/[\$\(\),]/g, ''))));
                        }
                        }
                        
                        if (cell4.childNodes[1].innerHTML != '') {
                            AmountTotal = roundNumber((parseFloat(AmountTotal) + parseFloat(cell4.childNodes[1].innerHTML.toString().replace(/[\$\(\),]/g, ''))));
                        }
                        if (isGst == 1) {
                            if (cell6 != null) {
                                if (cell6.childNodes[1].value != '') {
                                    GstTaxamountTotal = roundNumber((parseFloat(GstTaxamountTotal) + parseFloat(cell6.childNodes[1].value.toString().replace(/[\$\(\),]/g, ''))));
                                }
                            }
     ...
     <script>
         ...
         function pageLoad(sender, args) {
             ...
             $("[id*=ddlProjectCode]").focus(function () {
                 ...
                 $(hdnSelectPOIndex).val(hdnIndex.value - 1);
             });
