function registerEvents(e) { var i = $("[id$=hdnCurrentUserName]").val(), t = parseInt($("[id$=hdnCurrentUserID]").val()); e.server.connect(i, t) } 
function registerClientMethods(e) { e.client.onConnected = function (t, n, s, a, o) { for ($("#hdId").val(t), $("#hdUserName").val(n), i = 0; i < s.length; i++)
AddUser(e, s[i].UserID, s[i].UserName, o); for (i = 0; i < a.length; i++) AddMessage(a[i].UserName, a[i].Message) } } function CheckHiddenWindow() 
...
chatHub.client.sendPrivateMessage = function (e, i, t, n) { var s = "private_" + e; if (0 == $("#" + s).length) 
createPrivateChatWindow(chatHub, e, s, i, t), $("#chatAudio")[0].play(); else { var a = CheckHiddenWindow(); "none" == $("#" + s).parent().css("display") && 
($("#" + s).parent().parent().effect("shake", { times: 2 }, 1e3), a = !0), 1 == a && $("#chatAudio")[0].play() } 
$("#" + s).chatbox("option", "boxManager").addMsg(i, n), 
$("#typing_" + e).hide() }, chatHub.client.GetLastMessages = function (e, t)
{ var n = "private_" + e, s = ""; for (i = 0; i < t.length; i++) s += '<div style="display: block; max-width: 200px;" class="ui-chatbox-msg">', i == t.length - 1 ? $("#" + n).children().last().html() != "<b>" + t[i].FromUserName + ": </b><span>" + t[i].Message + "</span>" && (s += "<b>" + t[i].FromUserName + ": </b><span>" + t[i].Message + "</span>") : s += "<b>" + t[i].FromUserName + ": </b><span>" + t[i].Message + "</span>", s += "</div>"; $("#" + n).prepend(s) }, chatHub.client.ReceiveTypingRequest = function (e) { var i = "private_" + e; $("#" + i).length > 0 && (jQuery("#typing_" + e).show(), jQuery("#typing_" + e).delay(6e3).fadeOut("slow")) }; var showList = new Array, config = { width: 200, gap: 20, maxBoxes: 5 }, getNextOffset = function () { return (config.width + config.gap) * showList.length }, TypingFlag = !0;