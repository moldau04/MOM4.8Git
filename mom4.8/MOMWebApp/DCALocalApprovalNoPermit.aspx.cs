...
public partial class DCALocalApprovalNoPermit : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Approval DCA" || sortBy == "Project #" || sortBy == "Drawings Submitted To")
                    {
                        sortBy = "[" + sortBy + "]";
                    }
    ...
    private void GetCustReportFiltersValue()
    {
        ...
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetCustReportFiltersValue = objBL_ReportsData.GetApprovalReportFiltersValue(objProp_User);
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            dsGetCustDetails = objBL_ReportsData.GetApprovalDetails(objProp_User);
            ...
            if (dsGetCustDetails.Tables.Count > 0)
            {
                ...
                dsGetCustDetails.Tables[0].TableName = "DCA & Local Approval";
    ...
    private void BindGridReport(string[] checkedColumns, string[] selectedFiltersColumns, string[] selectedFiltersValues, string sortBy)
    {
        ...
        {
            query += " FROM vw_DCALocalApprovalNoPermitReport order by " + sortBy;
        }
        else
        {
            ...
            if (selectedFiltersColumns != null)
            {
                for (int i = 0; i <= selectedFiltersColumns.Count() - 1; i++)
                {
                    ...
                    if (selectedFiltersColumns[i] == "ApprovalDCA")
                    {
                        selectedFiltersColumns[i] = "Approval DCA";
                    }
            ...
            query += " FROM vw_DCALocalApprovalNoPermitReport where " + filters + " order by " + sortBy;
        }
    ...
    public static string GetApprovalPreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            DCALocalApprovalNoPermit _a = new DCALocalApprovalNoPermit();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (DataSortBy == "Approval DCA" || DataSortBy == "Project #" || DataSortBy == "Drawings Submitted To")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_DCALocalApprovalNoPermitReport order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_DCALocalApprovalNoPermitReport where " + filters + " order by " + DataSortBy;
            }
            #endregion
