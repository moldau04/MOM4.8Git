...
public partial class RouteListingReport : System.Web.UI.Page
{
    ...
    {
        DataSet dsGetRouteReportFiltersValue = new DataSet();
        objProp_User.DBName = Session["dbname"].ToString();
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetRouteReportFiltersValue = objBL_ReportsData.GetRouteReportFiltersValue(objProp_User);


        if (dsGetRouteReportFiltersValue.Tables[0].Rows.Count > 0)
        {
            drpName.DataSource = dsGetRouteReportFiltersValue.Tables[0];
            drpName.DataTextField = "Name";
            drpName.DataValueField = "Name";
            drpName.DataBind();
        }

        if (dsGetRouteReportFiltersValue.Tables[1].Rows.Count > 0)
        {
            drpMechName.DataSource = dsGetRouteReportFiltersValue.Tables[1];
            drpMechName.DataTextField = "MechName";
            drpMechName.DataValueField = "MechName";
            drpMechName.DataBind();
        }

        if (dsGetRouteReportFiltersValue.Tables[2].Rows.Count > 0)
        {
            drpHour.DataSource = dsGetRouteReportFiltersValue.Tables[2];
            drpHour.DataTextField = "Hour";
            drpHour.DataValueField = "Hour";
            drpHour.DataBind();
        }

        if (dsGetRouteReportFiltersValue.Tables[3].Rows.Count > 0)
        {
            drpAmount.DataSource = dsGetRouteReportFiltersValue.Tables[3];
        ...
        if (dsGetRouteReportFiltersValue.Tables[4].Rows.Count > 0)
        {
            drpRemarks.DataSource = dsGetRouteReportFiltersValue.Tables[4];
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            dsGetCustDetails = objBL_ReportsData.GetRouteDetails(objProp_User);
            ...
            if (dsGetCustDetails.Tables.Count > 0)
            {
                ...
                dsGetCustDetails.Tables[0].TableName = "Routes";
    ...
    private void BindGridReport(string[] checkedColumns, string[] selectedFiltersColumns, string[] selectedFiltersValues, string sortBy)
    {
        ...
        {
            query += " FROM vw_RouteReportDetails order by " + sortBy;
        }
        else
        {
            ...
            query += " FROM vw_RouteReportDetails where " + filters + " order by " + sortBy;
        }
    ...
    public static string GetRoutePreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            RouteListingReport _a = new RouteListingReport();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_RouteReportDetails order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_RouteReportDetails where " + filters + " order by " + DataSortBy;
            }
            #endregion
