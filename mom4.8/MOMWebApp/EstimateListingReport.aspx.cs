...
public partial class EstimateListingReport : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Estimate Price" || sortBy == "Quoted Price")
                    {
                        sortBy = "[" + sortBy + "]";
                    }
                }
    ...
    private void GetCustReportFiltersValue()
    {
        ...
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetEstimateReportFiltersValue = objBL_ReportsData.GetEstimateReportFiltersValue(objProp_User);

        if (dsGetEstimateReportFiltersValue.Tables[0].Rows.Count > 0)
        {
            drpID.DataSource = dsGetEstimateReportFiltersValue.Tables[0];
        ...
        {
            drpDescription.DataSource = dsGetEstimateReportFiltersValue.Tables[2];
        ...
        {
            drpRemarks.DataSource = dsGetEstimateReportFiltersValue.Tables[3];
            drpRemarks.DataTextField = "Remarks";
            drpRemarks.DataValueField = "Remarks";
            drpRemarks.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[4].Rows.Count > 0)
        {
            drpJob.DataSource = dsGetEstimateReportFiltersValue.Tables[4];
            drpJob.DataTextField = "Job";
            drpJob.DataValueField = "Job";
            drpJob.DataBind();
        }

        if (dsGetEstimateReportFiltersValue.Tables[5].Rows.Count > 0)
        {
            drpContact.DataSource = dsGetEstimateReportFiltersValue.Tables[5];
        ...
        {
            drpType.DataSource = dsGetEstimateReportFiltersValue.Tables[6];
            drpType.DataTextField = "Type";
            drpType.DataValueField = "Type";
            drpType.DataBind();
        }
        ...
        {
            drpStatus.DataSource = dsGetEstimateReportFiltersValue.Tables[7];
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            dsGetCustDetails = objBL_ReportsData.GetEstimateDetails(objProp_User);
            ...
            if (dsGetCustDetails.Tables.Count > 0)
            {
                ...
                dsGetCustDetails.Tables[0].TableName = "Estimate";
    ...
    protected void btnClose_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("Estimate.aspx", false);
        }
        catch
        {
            //
    ...
    {       
        ...
        {
            query += " FROM vw_EstimateReportDetails v order by " + sortBy;
        }
        else
        {
            ...
            query += " FROM vw_EstimateReportDetails v where " + filters + " order by " + sortBy;
        }
    ...
    public static string GetEstimatePreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {          
            EstimateListingReport _a = new EstimateListingReport();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (DataSortBy == "Estimate Price" || DataSortBy == "Quoted Price")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_EstimateReportDetails v order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_EstimateReportDetails v where " + filters + " order by " + DataSortBy;
            }
            #endregion
