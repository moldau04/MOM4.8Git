...
public partial class ServiceSalesCheckUpReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            GetSMTPUser();
            SetAddress();
            string FileName = "ServiceSalesCheckUpReport.pdf";
    ...
    protected void lnkExcel_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("Content-Disposition", $"attachment;filename=ServiceSalesCheckUpReport.xls");
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Service Sales Check Up Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Service Sales Check Up Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "ServiceSalesCheckUpReport.pdf")
                        {
                ...
                mail.FileName = "ServiceSalesCheckUpReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ServiceSalesCheckUpReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ServiceSalesCheckUpReport.pdf");
    ...
    {
        string reportPathStimul = Server.MapPath("StimulsoftReports/ServiceSalesCheckUpReport.mrt");
        ...
        {
            objPropContracts.EN = 1;
        }
        else
        {
            objPropContracts.EN = 0;
        }

        if (!string.IsNullOrEmpty(Request.QueryString["stype"]))
        {
            objPropContracts.SearchBy = Request.QueryString["stype"];
        }

        if (!string.IsNullOrEmpty(Request.QueryString["stext"]))
        {
            objPropContracts.SearchValue = Request.QueryString["stext"];
        }

        bool includeClose = false;
        if (!string.IsNullOrEmpty(Request.QueryString["close"]))
        {
            includeClose = Convert.ToBoolean(Request.QueryString["close"]);
        }

        bool isPassedInspection = false;
        if (!string.IsNullOrEmpty(Request.QueryString["passedInspection"]))
        {
            isPassedInspection = Convert.ToBoolean(Request.QueryString["passedInspection"]);
        }
        ...
        var ds = bL_Report.GetServiceSalesCheckUpReport(objPropContracts, filters, includeClose, isPassedInspection);
        if (ds != null)
        {
    ...
    private void SetAddress()
    {
        ...
        if (dsC.Tables[0].Rows.Count > 0)
        {
            ...
            string mailBody = "Please review the attached Service Sales Check Up Report.";
