...
public partial class ProjectVendorDemandByVendor : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    StiWebViewerProjectVendor.Visible = true;
                }

                if (Request.QueryString["vendors"] != null && !string.IsNullOrEmpty(Request.QueryString["vendors"]))
                {
                    if (Request.QueryString["vendors"] == "all")
                    {
                        foreach (RadComboBoxItem item in rcVendor.Items)
                        {
                            item.Checked = true;
                        }
                    }
                    ...
                    {
                        var vendors = HttpUtility.UrlDecode(Request.QueryString["vendors"].Trim());
                        var vendorArray = vendors.Split(',');

                        for (int i = 0; i < vendorArray.Length; i++)
                        {
                            RadComboBoxItem item = rcVendor.FindItemByValue(vendorArray[i]);
                            if (item != null)
                                item.Checked = true;
                        }
                    }
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["inclClosedPO"]))
                {
                    chkIncludeClose.Checked = Convert.ToBoolean(Request.QueryString["inclClosedPO"]);
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "ProjectVendorDemandByVendor.pdf";
    ...
    protected void StiWebViewerProjectVendor_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerProjectVendor_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        List<string> selectedVendor = new List<string>();
        foreach (RadComboBoxItem item in rcVendor.Items)
        {
        ...
        if (selectedVendor.Count != rcVendor.Items.Count)
        {
            if (selectedVendor.Count > 0)
            {
                vendors = string.Join(",", selectedVendor);
            }
        }
        else
        {
        ...
        {
            depts = string.Join(",", selectedDept);
        }

        var url = Request.Url.ToString();
        url = RemoveQueryStringByKey(url, "vendors");
        url = RemoveQueryStringByKey(url, "depts");
        url = RemoveQueryStringByKey(url, "inclClosedPO");
        url += "&vendors=" + HttpUtility.UrlEncode(vendors) + "&depts=" + HttpUtility.UrlEncode(depts) + "&inclClosedPO=" + chkIncludeClose.Checked;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/Project/ProjectVendorDemandByVendor.mrt");
            ...
            int includeClosedProject = 0;
            if (!string.IsNullOrEmpty(Request.QueryString["close"]))
            {
                if (Convert.ToBoolean(Request.QueryString["close"]))
                {
                    includeClosedProject = 1;
                }
            }

            var vendor = string.Empty;
            if (!string.IsNullOrEmpty(Request.QueryString["vendors"]) && Request.QueryString["vendors"] != "all")
            {
                vendor = HttpUtility.UrlDecode(Request.QueryString["vendors"].Trim());
            }
            ...
            int includeClosedPO = 0;
            if (!string.IsNullOrEmpty(Request.QueryString["inclClosedPO"]))
            {
                if (Convert.ToBoolean(Request.QueryString["inclClosedPO"]))
                {
                    includeClosedPO = 1;
                }
            }
            ...
            if (string.IsNullOrEmpty(Request.QueryString["page"]) || Request.QueryString["page"] != "addProject")
            {
                filters = Session["GridFilters"] != null ? (List<RetainFilter>)Session["GridFilters"] : new List<RetainFilter>();
            }

            var ds = objBL_Report.GetProjectVendorDemand(objPropCustomer, filters, vendor, depts, includeClosedProject, includeClosedPO);
            if (ds != null)
            {
    ...
    private void FillVendor()
    {
        try
        {
            ...
            var ds = objBL_Report.GetListVendorDemand(objPropCustomer);

            rcVendor.DataSource = ds.Tables[0];
            rcVendor.DataTextField = "Name";
            rcVendor.DataValueField = "ID";
            rcVendor.DataBind();
        }
        catch (Exception ex)
        {
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Project Vendor Demand by Vendor Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Project Vendor Demand by Vendor Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "ProjectVendorDemandByVendor.pdf")
                        {
                ...
                mail.FileName = "ProjectVendorDemandByVendor.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ProjectVendorDemandByVendor.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ProjectVendorDemandByVendor.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Project Vendor Demand by Vendor Report.";
