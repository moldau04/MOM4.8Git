...
public partial class ARAgingReportByBusinessType : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                pageTitle.InnerText = $"AR Aging by {Session["BusinessTypeLabel"].ToString()} Report";
                pageEmailTitle.InnerText = $"AR Aging by {Session["BusinessTypeLabel"].ToString()} Report";
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = $"ARAgingBy{Session["BusinessTypeLabel"].ToString().Replace(" ", "")}Report.pdf";
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        var url = "ARAgingReportByBusinessType.aspx?sdate=" + txtSearchDate.Text + "&type=" + type + "&pHide=" + chkHidePartial.Checked + "&inclNotes=" + chkIncludeNotes.Checked + "&creditFlag=" + chkCreditFlag.Checked + "&showRetainage=" + chkRetainageAmount.Checked;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingReportByBusinessType.mrt");
            if (!string.IsNullOrEmpty(Request["showRetainage"]) && Request["showRetainage"] == "True")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingReportByBusinessType_Retainage.mrt");
                if (!string.IsNullOrEmpty(Request["type"]) && Request["type"] == "summary")
                {
                    reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingSummaryReportByBusinessType_Retainage.mrt");
                }
            }
            else
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingReportByBusinessType.mrt");
                if (!string.IsNullOrEmpty(Request["type"]) && Request["type"] == "summary")
                {
                    reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingSummaryReportByBusinessType.mrt");
                }
            }
            ...
            DataSet ds = objBL_Contracts.GetARAgingByBusinessTypeOfDate(objContract, creditFlag);
            ...
            report.Dictionary.Variables["BusinessTypeLabel"].Value = businessTypeLabel.ToUpper();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = $"AR Aging by {Session["BusinessTypeLabel"].ToString()} Report";              
                ...
                {
                    mail.Text = $"This is report email sent from Mobile Office Manager. Please find the AR Aging by {Session["BusinessTypeLabel"].ToString()} Report attached.";                   
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != $"ARAgingBy{Session["BusinessTypeLabel"].ToString().Replace(" ", "")}Report.pdf")
                ...
                mail.FileName = $"ARAgingBy{Session["BusinessTypeLabel"].ToString().Replace(" ", "")}Report.pdf";              
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
           if (DownloadFileName == $"ARAgingBy{Session["BusinessTypeLabel"].ToString().Replace(" ", "")}Report.pdf")
                {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", $"attachment;filename=ARAgingBy{Session["BusinessTypeLabel"].ToString().Replace(" ", "")}Report.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = $"Please review the attached AR Aging by {Session["BusinessTypeLabel"].ToString()} Report.";
