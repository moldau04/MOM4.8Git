...
public partial class EditCheck : System.Web.UI.Page
{
    #region Variables
    ...
    GetTransByBatchTypeParam _getTransByBatchType = new GetTransByBatchTypeParam();
    UpdateAPCDDateParam _updateAPCDDate = new UpdateAPCDDateParam();
    UpdateNextCheckParam _updateNextCheck = new UpdateNextCheckParam();
    UpdateTransDateByBatchParam _updateTransDateByBatch = new UpdateTransDateByBatchParam();
    GetAPCheckLogsParam _getAPCheckLogs = new GetAPCheckLogsParam();
    ...
    IsExistCheckNumOnEditParam _IsExistCheckNumOnEdit = new IsExistCheckNumOnEditParam();
    #endregion
    ...
    public bool CheckNumValidOnEdit(string checkno, string bank, string cdId, string PaymentType)
    {
        ...
        _IsExistCheckNumOnEdit.IsExistCheckNo = false;
        if (!string.IsNullOrEmpty(checkno) && !string.IsNullOrEmpty(bank))
        {
            ...
            _objCD.Vendor = Convert.ToInt32(PaymentType);

            //API
            _IsExistCheckNumOnEdit.ConnConfig = Session["config"].ToString();
            _IsExistCheckNumOnEdit.ID = Convert.ToInt32(cdId);
            _IsExistCheckNumOnEdit.Ref = long.Parse(checkno);
            _IsExistCheckNumOnEdit.Bank = Convert.ToInt32(bank);
            _IsExistCheckNumOnEdit.Vendor = Convert.ToInt32(PaymentType);

            _objAPIIntegration = (APIIntegrationModel)Session["IsAPIIntegration"];
            //if (IsAPIIntegrationEnable == "YES")
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "ManageChecksAPI/EditCheck_IsExistCheckNumOnEdit";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _IsExistCheckNumOnEdit);
    ...
    protected void lnkSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (checknoexist == false)
            {
                ...
                //API
                _updateAPCDDate.ConnConfig = Session["config"].ToString();
                _updateAPCDDate.ID = Convert.ToInt32(Request.QueryString["id"]);
                _updateAPCDDate.fDate = Convert.ToDateTime(txtCheckDate.Text);
                _updateAPCDDate.Ref = long.Parse(txtCheck.Text);
                _updateAPCDDate.Vendor = Convert.ToInt32(hdnVendorID.Value);
                _updateAPCDDate.MOMUSer = Session["Username"].ToString();

                // _objBLBill.UpdateCDDate(_objCD);
                ...
                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                {
                    string APINAME = "ManageChecksAPI/EditCheck_UpdateAPCDDate";

                    APIResponse APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _updateAPCDDate);
                ...
                //API
                _updateNextCheck.ConnConfig = Session["config"].ToString();
                _updateNextCheck.ID = Convert.ToInt32(hdnBankID.Value);
                _updateNextCheck.NextC = long.Parse(txtCheck.Text) + 1;
                ...
                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                {
                    string APINAME = "ManageChecksAPI/EditCheck_UpdateNextCheck";

                    APIResponse APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _updateNextCheck);
                ...
                //API
                _updateTransDateByBatch.ConnConfig = Session["config"].ToString();
                _updateTransDateByBatch.BatchID = Convert.ToInt32(hdnBatch.Value);
                _updateTransDateByBatch.TransDate = Convert.ToDateTime(txtCheckDate.Text);
                _updateTransDateByBatch.Ref = long.Parse(txtCheck.Text);
                ...
                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                {
                    string APINAME = "ManageChecksAPI/EditCheck_UpdateTransDateByBatch";

                    APIResponse APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _updateTransDateByBatch);
    ...
    protected void lnkPrevious_Click(object sender, EventArgs e)
    {
        try
        {
            DataTable dt = (DataTable)Session["Checks"];
            if (dt != null)
            {
    ...
    public void SetDataForEdit()
    {
        try
        {
            ...
            _getCDByID.ID = Convert.ToInt32(Request.QueryString["id"]);
            ...
            if (_dsCD.Tables[0].Rows.Count > 0)
            {
                ...
                if (Convert.ToString(_dr["VendorName"]) != "")
                {
                    lblCheckNo.Text = lblCheckNo.Text + " | " + Convert.ToString(_dr["VendorName"]);
                }
                hdnVendorID.Value = _dr["Vendor"].ToString();
                hdnVendorAcct.Value = _dr["Acct#"].ToString();
                txtPayee.Text = _dr["fDesc"].ToString();
                txtMemo.Text = _dr["Memo"].ToString();
                txtCheckDate.Text = Convert.ToDateTime(_dr["fDate"]).ToString("MM/dd/yyyy");
                txtCheck.Text = _dr["Ref"].ToString();
                hdnBatch.Value = _dr["Batch"].ToString();
                hdnCDID.Value = _dr["ID"].ToString();
                hdnBankID.Value = _dr["Bank"].ToString();
                txtBank.Text = _dr["BankName"].ToString();
                lblTotalVal.Text = string.Format("{0:c}", Convert.ToDouble(_dr["Amount"]));
                ...
                //API
                _getTransByBatchType.ConnConfig = Session["config"].ToString();
                _getTransByBatchType.Type = 20;
                _getTransByBatchType.BatchID = Convert.ToInt32(_dr["Batch"]);
                ...
                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                {
                    string APINAME = "ManageChecksAPI/EditCheck_GetTransByBatchType";

                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getTransByBatchType);

                    _lstTransactionViewModel = (new JavaScriptSerializer()).Deserialize<List<TransactionViewModel>>(_APIResponse.ResponseData);
                    _dsTran = CommonMethods.ToDataSet<TransactionViewModel>(_lstTransactionViewModel);
                }
                else
                {
                ...
                {
                    if (Convert.ToInt16(_dsTran.Tables[0].Rows[0]["Sel"]).Equals(1))            //Cleared check
                    {
                    ...
                    else if (Convert.ToInt16(_dsTran.Tables[0].Rows[0]["Sel"]).Equals(2))      //Voided check
                    {
    ...
    private void BindCheckGrid()
    {
        try
        {
            ...
            _objPaid.PITR = Convert.ToInt32(Request.QueryString["id"]);

            _getPaidDetailByID.ConnConfig = Session["config"].ToString();
            _getPaidDetailByID.PITR = Convert.ToInt32(Request.QueryString["id"]);
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                ...
                _ds = CommonMethods.ToDataSet<PaidViewModel>(_lstPaidViewModel);
                _ds.Tables[0].Columns["Paid1"].ColumnName = "Paid";
            }
            else
            {
                _ds = _objBLBill.GetPaidDetailByID(_objPaid);
            }

            DataTable distinctTable = _ds.Tables[0].DefaultView.ToTable("Ref", true);
            DataTable newDt = new DataTable();

            if (distinctTable.Rows.Count > 0)
            {
                newDt = distinctTable.AsEnumerable()
                 .GroupBy(x => new Tuple<string, int>(x.Field<string>("Ref"), x.Field<int>("Batch")))
                 .Select(y => y.First())
                 .CopyToDataTable();
            }
            else
            {
                newDt = distinctTable.Clone();
            }

            ViewState["Bills"] = newDt;
            gvCheck.DataSource = newDt;
            gvCheck.DataBind();
            CalculateBalance();
        }
        catch (Exception ex)
    ...
    private void CalculateBalance()
    {
        try
        {
            DataTable dt = (DataTable)ViewState["Bills"];
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                //Label lblOrigTotal = (Label)gvCheck.FooterRow.FindControl("lblOrigTotal");
                //Label lblBalanceTotal = (Label)gvCheck.FooterRow.FindControl("lblBalanceTotal");
                //Label lblDiscTotal = (Label)gvCheck.FooterRow.FindControl("lblDiscTotal");
                //Label lblPaidTotal = (Label)gvCheck.FooterRow.FindControl("lblPaidTotal");

                GridFooterItem footerItem = (GridFooterItem)gvCheck.MasterTableView.GetItems(GridItemType.Footer)[0];
    ...
    protected void imgPrintTemp8_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            int vid = Convert.ToInt32(hdnVendorID.Value);
            ...
            foreach (GridDataItem gr in gvCheck.Items)
            {
                ...
                //HiddenField hdnBillPostDate = (HiddenField)gr.FindControl("hdnID");
    ...
    protected void imgPrintTemp9_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            _getBankCD.ID = Convert.ToInt32(hdnBankID.Value);
    ...
    private StiReport FillMiddleDataSetReport(string reportName, bool isedit)
    {
        ...
        _dtCheck.Columns.Add(new DataColumn("TotalAmountPay", typeof(double)));
        _dtCheck.Columns.Add(new DataColumn("Memo", typeof(string)));
        _dtCheck.Columns.Add(new DataColumn("VendorAcct", typeof(string)));
        ...
        _drC["Memo"] = txtMemo.Text;
        _drC["RemitAddress"] = _dsV.Tables[0].Rows[0]["RemitAddress"];
        _drC["VendorAcct"] = _dsV.Tables[0].Rows[0]["Acct#"];
        _dtCheck.Rows.Add(_drC);
    ...
    private StiReport FillDataSetToReport(string reportName, bool isedit)
    {
        ...
        foreach (GridDataItem gr in gvCheck.Items)
        {
            ...
            Label lblfDate = (Label)gr.FindControl("lblfDate");
            Label lblDesc = (Label)gr.FindControl("lblDesc");
            Label lblBillPostDate = (Label)gr.FindControl("lblPostBillDate");

            _dri = _dti.NewRow();
            _dri["Ref"] = lblRef.Text;
           

            if (Session["dbname"].ToString().ToUpper() == "INNOVATIVE" || Session["dbname"].ToString().ToUpper() == "INNOVATIVE_WEBSTAGE")
            {
                _dri["InvoiceDate"] = lblBillPostDate.Text;
            }
            else
            {
                _dri["InvoiceDate"] = lblfDate.Text;
    ...
    protected void RadGrid_gvLogs_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        try
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                //API
                _getAPCheckLogs.ConnConfig = Session["config"].ToString();
                //_getAPCheckLogs.Ref = long.Parse(txtCheck.Text);
                _getAPCheckLogs.Ref = long.Parse(Request.QueryString["id"]);
                ...
                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                {
                    string APINAME = "ManageChecksAPI/EditCheck_GetAPCheckLogs";

                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getAPCheckLogs);
