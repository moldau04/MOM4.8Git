...
public partial class InventoryReport : System.Web.UI.Page
{
    ...
    BusinessEntity.Inventory objInventory = new BusinessEntity.Inventory();
    ...
    GetInventoryTransParam _GetInventoryTrans = new GetInventoryTransParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    StiWebViewerInventory.Visible = true;
                    txtEndDate.Text = Request.QueryString["ed"];
                }
                else
                {
    ...
    protected void StiWebViewerInventory_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadInventoryReport();
    }

    protected void StiWebViewerInventory_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    {
        Response.Redirect("Inventory.aspx?fil=1");
    }
    ...
    private StiReport LoadInventoryReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/InventoryReport.mrt");
            ...
            List<GetCompanyDetailsViewModel> _lstGetCompanyDetails = new List<GetCompanyDetailsViewModel>();

            if (IsAPIIntegrationEnable == "YES")
            {
                string APINAME = "InventoryAPI/InventoryReport_GetCompanyDetails";
                ...
                _lstGetCompanyDetails = serializer.Deserialize<List<GetCompanyDetailsViewModel>>(_APIResponse.ResponseData);

                companyInfo = CommonMethods.ToDataSet<GetCompanyDetailsViewModel>(_lstGetCompanyDetails);
            ...
            objInventory.ConnConfig = connString;
            _GetInventoryTrans.ConnConfig = connString;

            if (!string.IsNullOrEmpty(Request.QueryString["ed"]))
            {
                objInventory.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
                _GetInventoryTrans.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
            }
            else
            {
                objInventory.EndDate = DateTime.Now;
                _GetInventoryTrans.EndDate = DateTime.Now;
            }

            // Search
            if (!string.IsNullOrEmpty(Request["stype"]) && !string.IsNullOrEmpty(Request["stext"]))
            {
                objInventory.SearchField = HttpUtility.UrlDecode(Request.QueryString["stype"]);
                objInventory.SearchValue = HttpUtility.UrlDecode(Request.QueryString["stext"]);

                _GetInventoryTrans.SearchField = HttpUtility.UrlDecode(Request.QueryString["stype"]);
                _GetInventoryTrans.SearchValue = HttpUtility.UrlDecode(Request.QueryString["stext"]);
            }
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["inclInactive"]))
            {
                ...
                _GetInventoryTrans.inclInactive = Convert.ToBoolean(Request.QueryString["inclInactive"]);
            }

            List<RetainFilter> filters = new List<RetainFilter>();
            if (Session["Inv_Filters"] != null)
            {
                ///Get  rad grid view  search filter value from Inventory View
                filters = (List<RetainFilter>)Session["Inv_Filters"];
                _GetInventoryTrans.filters = (List<RetainFilter>)Session["Inv_Filters"];
            }

            DataSet ds = new DataSet();
            List<GetInventoryTransViewModel> _lstGetInventoryTransV = new List<GetInventoryTransViewModel>();

            if (IsAPIIntegrationEnable == "YES")
            {
                string APINAME = "InventoryAPI/InventoryReport_GetInventoryTrans";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetInventoryTrans);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetInventoryTransV = serializer.Deserialize<List<GetInventoryTransViewModel>>(_APIResponse.ResponseData);

                ds = CommonMethods.ToDataSet<GetInventoryTransViewModel>(_lstGetInventoryTransV);

            }
            else
            {
                ds = objBL_Inventory.GetInventoryTrans(objInventory, filters, inclInactive);
            }
            ...
            if (IsAPIIntegrationEnable == "YES")
            {
                report.Dictionary.Variables["EndDate"].Value = _GetInventoryTrans.EndDate.ToShortDateString();
            }
            else
            {
                report.Dictionary.Variables["EndDate"].Value = objInventory.EndDate.ToShortDateString();
            }
    ...
    private void GetSMTPUser()
    {
        ...
        List<SMTPEmailViewModel> _lstSMTPEmail = new List<SMTPEmailViewModel>();

        if (IsAPIIntegrationEnable == "YES")
        {
            string APINAME = "InventoryAPI/InventoryReport_GetSMTPByUserID";

            APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetSMTPByUserID);

            JavaScriptSerializer serializer = new JavaScriptSerializer();

            serializer.MaxJsonLength = Int32.MaxValue;

            _lstSMTPEmail = serializer.Deserialize<List<SMTPEmailViewModel>>(_APIResponse.ResponseData);

            ds = CommonMethods.ToDataSet<SMTPEmailViewModel>(_lstSMTPEmail);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Inventory Report attached.";
                }
                ...
                service.ExportTo(LoadInventoryReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "InventoryReport.pdf")
                        {
                ...
                mail.FileName = "InventoryReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "InventoryReport.pdf")
            {
                byte[] buffer1 = null;
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Inventory Report.";
