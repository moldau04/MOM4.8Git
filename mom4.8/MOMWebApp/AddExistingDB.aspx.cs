    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["Type"] != null & Request.QueryString["ID"] == null) {
                ddlDBType.SelectedValue = Request.QueryString["Type"];
            ...
            {
                lblPageTitle.Text = "Edit Existing Database";
                ViewState["mode"] = 1;
        ...
        try
        {
            ...
            if (txtDB.Text != string.Empty)
            {
                ...
                if (dsDB.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ddlDBType.SelectedValue == "TS")
                    {
                        if (dsChk.Tables[0].Rows.Count > 0 && dsChk.Tables[1].Rows.Count == 0)
                        {
                            if (Convert.ToInt32(ViewState["mode"]) == 1)
                            {
                                ...
                                ScriptManager.RegisterStartupScript(this, Page.GetType(), "keySucc", "noty({text: 'Database updated successfully.',  type : 'success', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                            }
                            else
                            {
                                objBL_User.AddDatabaseName(objPropUser);
                                //lblMsg.Text = "Database added successfully.";
                                ScriptManager.RegisterStartupScript(this, Page.GetType(), "keySucc", "noty({text: 'Database added successfully.',  type : 'success', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                                ClearControls();
                            }
                        }
                        else
                        {
                            ...
                            ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyError", "noty({text: 'Not a valid TS-9 database.',  type : 'error', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                            return;
                        }
                    }
                    else if (ddlDBType.SelectedValue == "MSM")
                    {
                        ...
                        else
                        {
                            ...
                            ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyError", "noty({text: 'Not a valid MSM database.',  type : 'error', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                            return;
                        }
                    }
                }
                else
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyError", "noty({text: 'Database does not exists.',  type : 'error', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                    return;
                }
            }
            else
            {
                ...
                ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyError", "noty({text: 'Please enter database name.',  type : 'error', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                return;
            }
