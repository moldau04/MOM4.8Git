...
public partial class OpenTicketReportbyRoutes : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(Request["RType"]))
                {
                    StiWebViewerOpenTicketbyRoutesReport.Visible = true;
                    if ((Request["RType"]) == "All")
                    {
                        foreach (RadComboBoxItem itm in ddlDefRoute.Items)
                        {
                            itm.Checked = true;
                        }
                    }
                    ...
                    {
                        var routes = HttpUtility.UrlDecode(Request["RType"]).Split(',');
                        foreach (var route in routes)
                        {
                            var item = ddlDefRoute.Items.FindItem(x => x.Value == route);
                            if (item != null)
                            {
                                item.Checked = true;
                            }
                ...
                {
                    StiWebViewerOpenTicketbyRoutesReport.Visible = false;
                    ClientScript.RegisterStartupScript(Page.GetType(), "keySucc", "noty({text: 'Please Select Route!',  type : 'warning', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                }
                ...
                {
                    txtToDate.Text = lastDay.ToString();
                }
                GetSMTPUser();
                SetAddress();
                string FileName = "OpenTicketbyRoutesReport.pdf";
    ...
    protected void lnkExcel_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("Content-Disposition", "attachment;filename=OpenTicketbyRoutesReport.xls");
    ...
    {
        Response.Redirect("OpenTicketReportbyRoutes.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text);
    }
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/OpenTicketReportbyRoutes.mrt");
            ...
            objCust.UserID = Convert.ToInt32(Session["UserID"].ToString());
            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objCust.StartDate = HttpUtility.UrlDecode(Request.QueryString["sd"]);
            }
            else
            {
                objCust.StartDate = DateTime.Now.ToString();
            }
            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objCust.EndDate = HttpUtility.UrlDecode(Request.QueryString["ed"]);
            }
            else
            {
                objCust.EndDate = DateTime.Now.ToString();
            }
            if (!string.IsNullOrEmpty(Request["RType"]))
            {
                if ((Request["RType"]) == "All")
                {
                    ...
                    foreach (RadComboBoxItem itm in ddlDefRoute.Items)
                    {
                        Allroutes.Add(itm.Value.ToString());
                    }
                    objCust.RouteSequence = string.Join(",", Allroutes);
                }
                else
                {
                    var routes = HttpUtility.UrlDecode(Request["RType"]);
                    objCust.RouteSequence = routes.ToString();
                }
            }

            DataSet ds = new DataSet();
            ds = objBL_Bills.GetOpenTicketbyRoutesReport(objCust);
            if (ds != null && ds.Tables.Count > 0)
            {
                report.RegData("ReportData", ds.Tables[0]);
            }
            var ReportName = string.Format("Open Ticket by {0} Report", getOpenTicketReportName());
            report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
            report.Dictionary.Variables["StartDate"].Value = objCust.StartDate;
            report.Dictionary.Variables["EndDate"].Value = objCust.EndDate;
            report.Dictionary.Variables["ReportName"].Value = ReportName.ToString().ToUpper();
            report.Render();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Open Ticket Report by Routes";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Open Ticket Report by Routes attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "OpenTicketReportbyRoutes.pdf")
                        {
                ...
                mail.FileName = "OpenTicketReportbyRoutes.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "OpenTicketbyRoutesReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=OpenTicketbyRoutesReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Open Ticket Report by Routes.";
    ...
    protected void btnSearch_Click1(object sender, EventArgs e)
    {
        ...
        var routes = GetRadComboBoxSelectedItems(ddlDefRoute);
        if (!string.IsNullOrEmpty(routes))
        {
            var url = "OpenTicketReportbyRoutes.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text + "&RType=" + routes;
            Response.Redirect(url);
        }
        else
        {
            var url = "OpenTicketReportbyRoutes.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
            Response.Redirect(url);
        }
    }

    protected void StiWebViewerOpenTicketbyRoutesReport_GetReportData(object sender, StiReportDataEventArgs e)
    {

    }

    protected void ddlDefRoute_DataBinding(object sender, EventArgs e)
    {
        ddlDefRoute.DataValueField = "ID";
        ddlDefRoute.DataTextField = "label";
        ddlDefRoute.DataSource = FillDefaultRoute();
    }

    private DataSet FillDefaultRoute()
    {
        try
        {
            DataSet ds = new DataSet();
    ...
    protected void StiWebViewerOpenTicketbyRoutesReport_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }
}
