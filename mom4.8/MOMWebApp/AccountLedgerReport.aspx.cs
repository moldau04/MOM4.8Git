...
public partial class AccountLedgerReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "AccountLedger.pdf";
    ...
    {
        if (Session["stardate"] != null && Session["stardate"] != null)
        {
           // DataTable dtHead = (DataTable)Session["AccountLedgerHead"];

            string urlString = "accountledger.aspx?id=" + Convert.ToInt32(Session["acctId"]) +
                "&s=" + Convert.ToDateTime(Session["stardate"]).ToString("MM/dd/yyyy") + "&e=" + Convert.ToDateTime(Session["enddate"]).ToString("MM/dd/yyyy");
    ...
    protected void StiWebViewerGLAccountLedger_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = GetGLAccountLedgerReport();
    }

    protected void StiWebViewerGLAccountLedger_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {

    }

    private StiReport GetGLAccountLedgerReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/AccountLedgerReport.mrt");
            ...
            //Get data
            var startDate = Convert.ToDateTime(Session["stardate"]);
            ...
            _objChart.ID = Convert.ToInt32(Session["acctId"]);
            _objChart.StartDate = Convert.ToDateTime(startDate);
            _objChart.EndDate = Convert.ToDateTime(endDate);
            dsChart = _objBLChart.GetAccountLedger(_objChart);

            DataTable dt = dsChart.Tables[0]; ;
            DataTable dtHead = dsChart.Tables[1];

            report.RegData("CompanyDetail", dsC.Tables[0]);
            report.RegData("DsAcctLedger", dt);
            report.RegData("AcctLedgerHead", dtHead);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Account Ledger Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Account Ledger Report attached.";
                }
                ...
                service.ExportTo(GetGLAccountLedgerReport(), stream, settings);
                ...
                mail.FileName = "AccountLedger.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "AccountLedger.pdf")
            {
                byte[] buffer1 = null;
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Account Ledger Report.";
