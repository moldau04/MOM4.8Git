...
public partial class ProjectSummaryReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "ProjectSummaryReport.pdf";
    ...
    protected void lnkProjectSummaryReport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var tempFile = string.Format("ProjectSummaryReport{0}.xlsx", DateTime.UtcNow.ToString("yyyyMMddHHmmssfff"));
            ...
            if (strFileName.Length > 0)
            {
                try
                {
                    DownloadDocument(strFileName, "ProjectSummaryReport.xlsx");
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/Project/ProjectSummaryReport-Gable.mrt");
        ...
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "ProjectSummaryReport.pdf")
                        {
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ProjectSummaryReport.pdf")
            {
                byte[] buffer1 = null;
