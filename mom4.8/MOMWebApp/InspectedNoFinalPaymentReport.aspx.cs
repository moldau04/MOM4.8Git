...
public partial class InspectedNoFinalPaymentReport : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Passed Inspection" || sortBy == "Project #" || sortBy == "Final Pymt Rcvd")
                    {
                        sortBy = "[" + sortBy + "]";
                    }
    ...
    private void GetCustReportFiltersValue()
    {
        ...
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetCustReportFiltersValue = objBL_ReportsData.GetInspectionReportFiltersValue(objProp_User);
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            dsGetCustDetails = objBL_ReportsData.GetInspectedDetails(objProp_User);
            ...
            {
                dsGetCustDetails.Tables[0].TableName = "Inspected Payment";
    ...
    protected void btnSaveReport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            objCustReport.Module = "InspectedPayment";
    ...
    private void BindGridReport(string[] checkedColumns, string[] selectedFiltersColumns, string[] selectedFiltersValues, string sortBy)
    {
        ...
        {
            query += " FROM vw_InspectedPaymentDetails order by " + sortBy;
        }
        else
        {
            ...
            if (selectedFiltersColumns != null)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(selectedFiltersValues[i]))
                    {
                        ...
                        if (selectedFiltersColumns[i] == "FinalInvoice")
                        {
                            selectedFiltersColumns[i] = "Final Invoice";
                        }

                        if (selectedFiltersColumns[i] == "Passed Inspection")
                        {
                            selectedFiltersColumns[i] = "Passed Inspection";
                        }
            ...
            query += " FROM vw_InspectedPaymentDetails where " + filters + " order by " + sortBy;
        }
    ...
    public static string GetInspectedPaymentPreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            InspectedNoFinalPaymentReport _a = new InspectedNoFinalPaymentReport();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (DataSortBy == "Final Pymt Rcvd" || DataSortBy == "Project #" || DataSortBy == "Passed Inspection")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_InspectedPaymentDetails order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                {                 
                    ...
                    if (selectedFiltersColumns[i] == "FinalPymtRcvd")
                    {
                        selectedFiltersColumns[i] = "Final Pymt Rcvd";
                    }

                    if (selectedFiltersColumns[i] == "PassedInspection")
                    {
                        selectedFiltersColumns[i] = "Passed Inspection";
                    }
                        if (selectedFiltersColumns[i].ToLower() != "balance" && selectedFiltersColumns[i].ToLower() != "loc" && selectedFiltersColumns[i].ToLower() != "equip" && selectedFiltersColumns[i].ToLower() != "opencall" && selectedFiltersColumns[i].ToLower() != "equipmentprice" && selectedFiltersColumns[i].ToLower() != "equipmentcounts")
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_InspectedPaymentDetails where " + filters + " order by " + DataSortBy;
            }
            #endregion
