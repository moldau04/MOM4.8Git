...
public partial class SalesTaxReportByVendor : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                StiWebViewerSalesTaxByVendor.Visible = false;
            }
            else
            {
                StiWebViewerSalesTaxByVendor.Visible = true;
            }
        }
        catch (Exception ex)
        {
    ...
    {
        processSaleTaxReportByVendor();
    }

    protected void StiWebViewerSalesTaxByVendor_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        string reportPathStimul = Server.MapPath("StimulsoftReports/SalesTaxCollectedReportByVendor.mrt");
        if (string.IsNullOrEmpty(Request.QueryString["type"]))
        {
            reportPathStimul = Server.MapPath("StimulsoftReports/SalesTaxCollectedReportByVendor.mrt");
        }
        else
        {
            if (Request.QueryString["type"] == "0")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/SalesTaxCollectedSummaryReportByVendor.mrt");
            }
        }
    ...
    protected void StiWebViewerSalesTaxByVendor_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        var report = e.Report;
        try
        {
            ...
            if (Request.QueryString["type"] == "0")
            {
                ds = objBLInvoices.GetSalesTaxCollectedByVendor(objInvoices);
            }
            if (Request.QueryString["type"] == "1")
            {
                ds = objBLInvoices.GetSalesTaxCollectedDetailByVendor(objInvoices);
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Sales Tax Collected Report By Vendor";
                ...
                {
                    mail.Text = "This report is generated from Mobile Office Manager. Please find attached the Sales Tax Collected Report By Vendor.";
                }
                ...
                if (poToPrint != null && poToPrint.Count > 0)
                {
                    ...
                    mail.FileName = "SalesTaxCollectedByVendor.pdf";
                }
    ...
    private byte[] ExportReportToPDF()
    {
        ...
        service.ExportTo(StiWebViewerSalesTaxByVendor.Report, stream, settings);
    ...
    public void processSaleTaxReportByVendor()
    {
        try
        {
            ...
            String url = "SalesTaxReportByVendor.aspx?redirect=Invoices.aspx&type=" + type + "&fromDate=" + fromDate.ToString("MM/dd/yyyy") + "&toDate=" + endDate.ToString("MM/dd/yyyy");
