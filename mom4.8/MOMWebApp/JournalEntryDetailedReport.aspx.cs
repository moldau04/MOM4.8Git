...
public partial class JournalEntryDetailedReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtfromDate.Text = firstDate.ToString("MM/dd/yyyy");
                }
                ...
                {
                    txtToDate.Text = firstDate.AddMonths(1).AddDays(-1).ToString("MM/dd/yyyy");
                }

                GetSMTPUser();
                SetAddress();
                string FileName = "JournalEntryDetailed.pdf";
    ...
    protected void StiWebViewerJournalEntry_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadJournalEntryReport();
    }

    protected void StiWebViewerJournalEntry_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    {
        var urlString = "JournalEntry.aspx";
    ...
    {
        var url = "JournalEntryDetailedReport.aspx";
        url += "?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;

        Response.Redirect(url);
    }

    private StiReport LoadJournalEntryReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/JournalEntryDetailedReport.mrt");
            ...
            if (Request.QueryString["sd"] != null && !string.IsNullOrEmpty(Request.QueryString["sd"]))
            {
                _objJournal.StartDate = Convert.ToDateTime(Request.QueryString["sd"].ToString());
            }
            else
            {
                _objJournal.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }

            if (Request.QueryString["ed"] != null && !string.IsNullOrEmpty(Request.QueryString["ed"]))
            {
                _objJournal.EndDate = Convert.ToDateTime(Request.QueryString["ed"].ToString());
            }
            else
            {
                _objJournal.EndDate = _objJournal.StartDate.AddMonths(1).AddDays(-1);
            }

            List<RetainFilter> filters = new List<RetainFilter>();
            if (Session["Journal_Filters"] != null)
            {
                filters = (List<RetainFilter>)Session["Journal_Filters"];
            }

            DataSet ds = _objBLJournal.GetJournalEntryDetailed(_objJournal, filters);

            if (ds != null && ds.Tables.Count > 0)
            {
                report.RegData("JournalEntry", ds.Tables[0]);
                report.RegData("JournalEntryDetailed", ds.Tables[1]);
            }

            report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
            report.Dictionary.Variables["StartDate"].Value = _objJournal.StartDate.ToString();
            report.Dictionary.Variables["EndDate"].Value = _objJournal.EndDate.ToString();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Journal Entry Detailed Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Journal Entry Detailed Report attached.";
                }
                ...
                service.ExportTo(LoadJournalEntryReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "JournalEntryDetailed.pdf")
                        {
                ...
                mail.FileName = "JournalEntryDetailed.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "JournalEntryDetailed.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=JournalEntryDetailed.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Journal Entry Detailed Report.";
