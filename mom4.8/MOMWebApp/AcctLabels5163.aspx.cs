...
public partial class AcctLabels5163 : System.Web.UI.Page
{
    ...
    GetAccountLabelParam _GetAccountLabel = new GetAccountLabelParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    ddlLocationSearch.SelectedValue = Request.QueryString["stype"];
                }

                if (!string.IsNullOrEmpty(Request.QueryString["stext"]))
                {
                    txtLocationSearch.Text = Request.QueryString["stext"];
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "LocationLabel5163.pdf";
    ...
    protected void StiWebViewerLocationLabel_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadLocationLabelReport();
    }

    protected void StiWebViewerLocationLabel_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    {
        Response.Redirect("AcctLabels5163.aspx?stype=" + ddlLocationSearch.SelectedValue + "&stext=" + txtLocationSearch.Text);
    }

    private StiReport LoadLocationLabelReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/AccountLabel5163.mrt");
            ...
            objLoc.ConnConfig = connString;
            objLoc.ConnConfig = Session["config"].ToString();

            _GetAccountLabel.ConnConfig = connString;
            _GetAccountLabel.ConnConfig = Session["config"].ToString();

            if (!string.IsNullOrEmpty(Request.QueryString["stype"]))
            {
                if (Request.QueryString["stype"] == "Tag")
                {
                    objLoc.Tag = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _GetAccountLabel.Tag = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "Address")
                {
                    objLoc.Address = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _GetAccountLabel.Address = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "City")
                {
                    objLoc.City = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _GetAccountLabel.City = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "State")
                {
                    objLoc.State = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _GetAccountLabel.State = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "Zip")
                {
                    objLoc.Type = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _GetAccountLabel.Type = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
            }

            DataSet ds = new DataSet();
            List<GetAccountLabelViewModel> _lstGetAccountLabel = new List<GetAccountLabelViewModel>();
            if (IsAPIIntegrationEnable == "YES")
            {
                string APINAME = "LocationsAPI/LocationReport_GetAccountLabel";

                _GetAccountLabel.IsSelesAsigned = new GeneralFunctions().GetSalesAsigned();

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetAccountLabel, true);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetAccountLabel = serializer.Deserialize<List<GetAccountLabelViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<GetAccountLabelViewModel>(_lstGetAccountLabel);
            }
            else
            {
                ds = objBL_Report.GetAccountLabel(objLoc, new GeneralFunctions().GetSalesAsigned());
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Location Label 5163 Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Location Label 5163 Report attached.";
                }
                ...
                service.ExportTo(LoadLocationLabelReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "LocationLabel5163.pdf")
                        {
                ...
                mail.FileName = "LocationLabel5163.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "LocationLabel5163.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=LocationLabel5163.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Location Label 5163 Report.";
