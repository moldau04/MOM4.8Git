...
public partial class CustomerTransactionLedger : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(Request["sd"]) && !string.IsNullOrEmpty(Request["ed"]))
                {
                    txtfromDate.Text = HttpUtility.UrlDecode(Request.QueryString["sd"]);
                    txtToDate.Text = HttpUtility.UrlDecode(Request.QueryString["ed"]);
                    StiWebViewerGeneralLedger.Visible = true;
                }
                else
                {
                    StiWebViewerGeneralLedger.Visible = false;
                }

                GetSMTPUser();
                SetAddress();
                string FileName = "CustomerTransactionLedger.pdf";
    ...
    {
        Response.Redirect("addCustomer.aspx?uid=" + Request.QueryString["uid"]);
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, Page.GetType(), "warningInvInactive", "noty({text: 'Please select Start date!',  type : 'warning', layout:'topCenter',closeOnSelfClick:false, timeout : 5000,theme : 'noty_theme_default',  closable : true});", true);
            return;
        }

        if (string.IsNullOrEmpty(txtToDate.Text))
        {
            ScriptManager.RegisterStartupScript(this, Page.GetType(), "warningInvInactive", "noty({text: 'Please select End date!',  type : 'warning', layout:'topCenter',closeOnSelfClick:false, timeout : 5000,theme : 'noty_theme_default',  closable : true});", true);
            return;
        }

        var url = "CustomerTransactionLedger.aspx?uid=" + Request.QueryString["uid"] + "&sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;

        Response.Redirect(url);
    }

    protected void StiWebViewerGeneralLedger_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadCustomerTransactionLedger();
    }

    protected void StiWebViewerGeneralLedger_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {

    }

    private StiReport LoadCustomerTransactionLedger()
    {
        string reportPathStimul = Server.MapPath("StimulsoftReports/CustomerTransactionLedger.mrt");
        ...
        {
            startDate = Convert.ToDateTime(HttpUtility.UrlDecode(Request.QueryString["sd"]));
        }
        else
        {
        ...
        {
            endDate = Convert.ToDateTime(HttpUtility.UrlDecode(Request.QueryString["ed"]));
        }
        else
        {
        ...
        {
            cusID = Convert.ToInt32(Request.QueryString["uid"]);
        }

        // Get Customer detail
        objPropUser.CustomerID = cusID;
        var dsCus = objBL_User.getCustomerAddress(objPropUser);

        report.RegData("CustomerDetails", dsCus.Tables[0]);

        // Report data
        objProp_Contracts.ConnConfig = connString;
        objProp_Contracts.StartDate = startDate;
        objProp_Contracts.EndDate = endDate;
        objProp_Contracts.CustID = cusID;
        objProp_Contracts.Loc = 0;
        objProp_Contracts.SearchBy = "All";
        objProp_Contracts.SearchValue = "All";

        DataSet ds = objBL_Invoice.GetARRevenueCust(objProp_Contracts);
        if (ds != null)
        {
            var prevAmount = Convert.ToDouble(ds.Tables[1].Rows[0][0].ToString());
            var dt = AddRunningTotal(ds.Tables[0], prevAmount);

            report.RegData("ReportData", dt);
        }

        report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
        report.Dictionary.Variables["StartDate"].Value = startDate.ToString();
        report.Dictionary.Variables["EndDate"].Value = endDate.ToString();
    ...
    private DataTable AddRunningTotal(DataTable dt, double prevAmount)
    {
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Customer Transaction Ledger Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Customer Transaction Ledger Report attached.";
                }
                ...
                service.ExportTo(LoadCustomerTransactionLedger(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "CustomerTransactionLedger.pdf")
                        {
                ...
                mail.FileName = "CustomerTransactionLedger.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "CustomerTransactionLedger.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=CustomerTransactionLedger.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Customer Transaction Ledger Report.";
