...
public partial class Federal940Form : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    DateTime FDate = Convert.ToDateTime(HttpUtility.UrlDecode(Request.QueryString["sd"]));
                    DateTime EDate = Convert.ToDateTime(HttpUtility.UrlDecode(Request.QueryString["ed"]));
                    if (FDate.Day != 1 || FDate.Month != 1 || EDate.Day !=31 || EDate.Month !=12 || FDate.Year != EDate.Year)
                    {
                        ClientScript.RegisterStartupScript(Page.GetType(), "keySucc", "noty({text: 'The start date or end date does not correspond to the start or end date of any fiscal year!',  type : 'warning', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
                    }
                    else
                    {
                        StiWebViewerFedral940Form.Visible = true;
                    }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "Federal940Form.pdf";
    ...
    {
        var url = "Federal940Form.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
        Response.Redirect(url);
    }

    //Get Report With Data Event
    protected void StiWebViewerFedral940Form_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadFederal940FormReport();
    }

    //Report Load Function
    //Get Data For Report
    private StiReport LoadFederal940FormReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/Federal940Form.mrt");
            ...
            ds = objBL_Report.GetPayrollFederal940FormReport(objPropContracts);


            if (ds != null && ds.Tables.Count > 0)
            {
                report.RegData("ReportData", ds.Tables[0]);
                report.RegData("ReportData1", ds.Tables[1]);
            }
    ...
    //Report Event
    protected void StiWebViewerFedral940Form_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Payroll-Federal 940 Form Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Payroll-Federal 940 Form Report attached.";
                }
                ...
                service.ExportTo(LoadFederal940FormReport(), stream, settings);
                ...
                mail.FileName = "Federal940Form.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "Federal940Form.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=Federal940Form.pdf");
    ...
    private void SetAddress()
    {
        ...
        if (dsC.Tables[0].Rows.Count > 0)
        {
            ...
            string mailBody = "Please review the attached Payroll-Federal 940 Form Report.";
