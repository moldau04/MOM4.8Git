...
public partial class LocationEquipmentListReport : System.Web.UI.Page
{
    ...
    GetLocationEquipmentListParam _GetLocationEquipmentList = new GetLocationEquipmentListParam();
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            GetSMTPUser();
            SetAddress();
            string FileName = "LocationEquipmentListReport.pdf";
    ...
    protected void StiWebViewerLocationList_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadLocationEquipmentListReport();
    }

    protected void StiWebViewerLocationList_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    protected void lnkExportExcel_Click(object sender, EventArgs e)
    {
        ...
        service.ExportTo(LoadLocationEquipmentListReport(), stream, settings);
        buffer1 = stream.ToArray();

        Response.Clear();
        MemoryStream ms = new MemoryStream(buffer1);
        Response.ContentType = "text/csv";
        Response.AddHeader("content-disposition", "attachment;filename=LocationEquipmentListReport.xlsx");
        Response.Buffer = true;
        ms.WriteTo(Response.OutputStream);
        Response.End();
    }

    private StiReport LoadLocationEquipmentListReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/LocationEquipmentListReport.mrt");
            ...
            _GetLocationEquipmentList.ConnConfig = connString;
            ...
            if (!string.IsNullOrEmpty(Request["stype"]) && !string.IsNullOrEmpty(Request["stext"]))
            {
                ...
                _GetLocationEquipmentList.SearchBy = HttpUtility.UrlDecode(Request.QueryString["stype"]);
                _GetLocationEquipmentList.SearchValue = HttpUtility.UrlDecode(Request.QueryString["stext"]);
            }
            ...
            if (IsAPIIntegrationEnable == "YES")
            {
                string APINAME = "LocationsAPI/LocationReport_GetLocationEquipmentList";

                _GetLocationEquipmentList.filters = filters;
                _GetLocationEquipmentList.includeInactive = inclInactive;

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetLocationEquipmentList, true);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetLocationEquipmentList = serializer.Deserialize<List<GetLocationEquipmentListViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<GetLocationEquipmentListViewModel>(_lstGetLocationEquipmentList);
            }
            else
            {
                ds = bL_Report.GetLocationEquipmentList(objPropMapData, filters, inclInactive);
            }

            if (ds != null && ds.Tables.Count > 0)
            {
                report.RegData("dtLocation", ds.Tables[0]);
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Location Equipment List Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Location Equipment List Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "LocationEquipmentListReport.pdf")
                        {
                ...
                mail.FileName = "LocationEquipmentListReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "LocationEquipmentListReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=LocationEquipmentListReport.pdf");
                Response.Buffer = true;
                ms.WriteTo(Response.OutputStream);
                Response.End();
            }
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Location Equipment List Report.";
