...
public partial class UserPaymentReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "UserPaymentReport.pdf";
    ...
    protected void StiWebViewerUserPayment_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadUserPaymentReport();
    }

    protected void StiWebViewerUserPayment_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    private StiReport LoadUserPaymentReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/UserPaymentReport.mrt");
            ...
            objPropUser.inclInactive = false;
            if (!string.IsNullOrEmpty(Request["incInactive"]))
            {
                objPropUser.inclInactive = Convert.ToBoolean(Request.QueryString["incInactive"]);
            }

            if (!string.IsNullOrEmpty(Request["isSuper"]))
            {
                objPropUser.IsSuper = Convert.ToInt32(Request.QueryString["isSuper"]);
            }

            // Search text
            if (!string.IsNullOrEmpty(Request["stype"]) && !string.IsNullOrEmpty(Request["svalue"]))
            {
                objPropUser.SearchBy = HttpUtility.UrlDecode(Request.QueryString["stype"]);
                objPropUser.SearchValue = HttpUtility.UrlDecode(Request.QueryString["svalue"]);
            }

            List<RetainFilter> filters = new List<RetainFilter>();
            if (Session["Uesrs_Filters"] != null)
            {
                filters = (List<RetainFilter>)Session["Uesrs_Filters"];
            }

            DataSet ds = objBL_Report.GetUserPaymentReport(objPropUser, filters);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "User Payment Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the User Payment Report attached.";
                }
                ...
                service.ExportTo(LoadUserPaymentReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "UserPaymentReport.pdf")
                        {
                ...
                mail.FileName = "UserPaymentReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "UserPaymentReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=UserPaymentReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached User Payment Report.";
