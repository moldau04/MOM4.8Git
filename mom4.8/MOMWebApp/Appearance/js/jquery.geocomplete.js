...
;(function($, window, document, undefined) {
    ...
    $.extend(GeoComplete.prototype, {
        ...
        bounds = geometry.bounds;
            ...
            var str_array = result.formatted_address.split(',');
            for (var i = 0; i < str_array.length; i++) {
                str_array[i] = str_array[i].replace(/^\s*/, "").replace(/\s*$/, "");
            }
            alert(str_array);
            var add = this.options.address;
            alert(add);
            setTimeout(function() { $(add).val(str_array[0]) }, 500);
            if (str_array.length > 3) {
                $(this.options.city).val(str_array[str_array.length - 3].trim());
                $(this.options.state).val(str_array[str_array.length - 2].substring(0, 2).trim());
                $(this.options.zip).val(str_array[str_array.length - 2].substring(2).trim());
                $(this.options.country).val(str_array[str_array.length - 2].substring(2).trim());
                $(this.options.lat).val(geometry.location.lat());
                $(this.options.lng).val(geometry.location.lng());
                initialize();
            }
        },
