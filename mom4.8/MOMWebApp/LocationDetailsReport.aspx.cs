...
public partial class LocationDetailsReport : System.Web.UI.Page
{
    ...
    GetLocationDetailsReportParam _GetLocationDetailsReport = new GetLocationDetailsReportParam();
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "LocationDetailsReport.pdf";
    ...
    protected void StiWebViewerLocationDetails_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadLocationDetailsReport();
    }

    protected void StiWebViewerLocationDetails_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    protected void lnkExcel_Click(object sender, EventArgs e)
    {
        ...
        report = LoadLocationDetailsReport();
        ...
        Response.AddHeader("Content-Disposition", $"attachment;filename=LocationDetailsReport.xls");
    ...
    private StiReport LoadLocationDetailsReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/LocationDetailsReport.mrt");
            ...
            _GetLocationDetailsReport.ConnConfig = connString;
            // Search text
            if (!string.IsNullOrEmpty(Request["stype"]) && !string.IsNullOrEmpty(Request["stext"]))
            {
                ...
                _GetLocationDetailsReport.SearchBy = HttpUtility.UrlDecode(Request.QueryString["stype"]);
                _GetLocationDetailsReport.SearchValue = HttpUtility.UrlDecode(Request.QueryString["stext"]);
            }
            ...
            if (IsAPIIntegrationEnable == "YES")
            {
                string APINAME = "LocationsAPI/LocationReport_GetLocationDetailsReport";

                _GetLocationDetailsReport.filters = filters;
                _GetLocationDetailsReport.includeInactive = inclInactive;

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetLocationDetailsReport, true);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetLocationDetailsReport = serializer.Deserialize<List<GetLocationDetailsReportViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<GetLocationDetailsReportViewModel>(_lstGetLocationDetailsReport);
            }
            else
            {
                ds = objBL_Report.GetLocationDetailsReport(objPropMapData, filters, inclInactive);
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = $"Location Details Report";
                ...
                {
                    mail.Text = $"This is report email sent from Mobile Office Manager. Please find the Location Details Report attached.";
                }
                ...
                System.IO.MemoryStream stream = new System.IO.MemoryStream();
                service.ExportTo(LoadLocationDetailsReport(), stream, settings);
                buffer1 = stream.ToArray();
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "LocationDetailsReport.pdf")
                        {
                ...
                mail.FileName = "LocationDetailsReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "LocationDetailsReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", $"attachment;filename=LocationDetailsReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = $"Please review the attached Location Details Report.";
