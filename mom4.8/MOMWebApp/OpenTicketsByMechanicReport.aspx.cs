...
public partial class OpenTicketsByMechanicReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "OpenTicketsByMechanic.pdf";
    ...
    protected void StiWebViewerOpenTicketsByMechanic_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerOpenTicketsByMechanic_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/OpenTicketsByMechanic.mrt");
            ...
            report["PrintedBy"] = Session["Username"].ToString(); 
            ...
            var totalTickets = 0;
            for (int i = 0; i < dsGetMechanics.Tables[0].Rows.Count; i++)
            {
                var newMechanicName = "";
                var oldMechanicName = dsGetMechanics.Tables[0].Rows[i]["MechanicName"].ToString();
                if (i + 1 < dsGetMechanics.Tables[0].Rows.Count)
                    newMechanicName = dsGetMechanics.Tables[0].Rows[i + 1]["MechanicName"].ToString();

                dr["Mechanic Name"] = dsGetMechanics.Tables[0].Rows[i]["MechanicName"];
                dr["Department"] = dsGetMechanics.Tables[0].Rows[i]["Department"];
                var Assigned = Convert.ToInt32(dsGetMechanics.Tables[0].Rows[i]["TicketCount"]);
                if ((string.IsNullOrEmpty(dsGetMechanics.Tables[0].Rows[i]["Category"].ToString()) || dsGetMechanics.Tables[0].Rows[i]["Category"].ToString().Equals("None")) && dsGetMechanics.Tables[0].Rows[i]["MechanicName"].ToString().Trim().Equals(oldMechanicName))
                {
                    var prevNoneValue = dr["None"].ToString();
                    var noneVal = 0;
                    if (!string.IsNullOrEmpty(prevNoneValue))
                    {
                        noneVal = Convert.ToInt32(prevNoneValue) + Convert.ToInt32(dsGetMechanics.Tables[0].Rows[i]["TicketCount"].ToString());
                    }
                    else
                        noneVal += Convert.ToInt32(dsGetMechanics.Tables[0].Rows[i]["TicketCount"].ToString());
                    dr["None"] = noneVal;
                }

                for (int j = 0; j < dsGetCategories.Tables[0].Rows.Count; j++)
                {
                    if (dsGetMechanics.Tables[0].Rows[i]["Category"].ToString().Trim().Equals(dsGetCategories.Tables[0].Rows[j]["Category"].ToString().Trim()) && dsGetMechanics.Tables[0].Rows[i]["MechanicName"].ToString().Trim().Equals(oldMechanicName))
                    {
                        var prevValue = dr[dsGetCategories.Tables[0].Rows[j]["Category"].ToString().Trim()].ToString();
                        var val = 0;
                        if (!string.IsNullOrEmpty(prevValue))
                        {
                            val = Convert.ToInt32(prevValue) + Convert.ToInt32(dsGetMechanics.Tables[0].Rows[i]["TicketCount"].ToString());
                        }
                        else
                            val += Convert.ToInt32(dsGetMechanics.Tables[0].Rows[i]["TicketCount"].ToString());
                        dr[dsGetCategories.Tables[0].Rows[j]["Category"].ToString().Trim()] = val;
                    }
                }

                if (!oldMechanicName.Equals(newMechanicName))
                {
                    for (int j = 0; j < dsGetCategories.Tables[0].Rows.Count; j++)
                    {
            ...
            objMapData.FromDate = StartDate.ToString();
            objMapData.Todate = EndDate.ToString();
            var allWorkers = objBL_MapData.GetAllWorkers(objMapData);
            DataView view = new DataView(allWorkers.Tables[0]);
            DataTable distinctUsers = view.ToTable(true, "MechanicName", "Department");
            var isWorkerAdded = false;
            foreach (DataRow row1 in distinctUsers.Rows)
            {
                isWorkerAdded = false;
                foreach (DataRow row in sTable.Rows)
                {
                    if (row1["MechanicName"].ToString().Trim().Equals(row["Mechanic Name"].ToString().Trim()))
                    {
                        isWorkerAdded = true;
                        break;
                    }
                ...
                if (!isWorkerAdded)
                {
                    var eRow = sTable.NewRow();
                    eRow["Mechanic Name"] = row1["MechanicName"].ToString();
                    eRow["Department"] = row1["Department"].ToString();
                    sTable.Rows.Add(eRow);
                }
            }
            ...
            int sum = 0;
            var sumRow = sTable.NewRow();
            sumRow["Mechanic Name"] = "Tickets Summary";
            foreach (DataColumn dc in sTable.Columns)
            {
                foreach (DataRow row in sTable.Rows)
                {
                    if (!dc.ColumnName.Equals("Mechanic Name") && !dc.ColumnName.Equals("Department") && !(string.IsNullOrEmpty(row[dc.ColumnName].ToString())))
                        sum += Convert.ToInt32(row[dc.ColumnName].ToString());
                }
                if (!dc.ColumnName.Equals("Mechanic Name"))
                {
                    sumRow[dc.ColumnName] = sum;
                    sum = 0;
                }
            }

            sTable.Rows.Add(sumRow);
            ...
            double catWidth = page.Width - StiAlignValue.AlignToMaxGrid(1.0, 0.1, true) - StiAlignValue.AlignToMaxGrid(0.3, 0.1, true) / 22;
            catWidth = 0.3;
            ...
            foreach (DataColumn dataColumn in sTable.Columns)
            {
                ...
                else if (dataColumn.ColumnName.Equals("Total"))
                {
                    columnWidth = StiAlignValue.AlignToMaxGrid(0.3, 0.1, true);
                    hText = new StiText(new RectangleD(pos, 0, columnWidth, 1.8));
                    hText.Angle = 90;
                }
                else
                ...
                if (dataColumn.ColumnName.Equals("Total"))
                {
                    columnWidth = StiAlignValue.AlignToMaxGrid(0.3, 0.1, true);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Open Tickets By Mechanic Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Open Tickets By Mechanic Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "OpenTicketsByMechanic.pdf")
                        {
                ...
                mail.FileName = "OpenTicketsByMechanic.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "OpenTicketsByMechanic.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=OpenTicketsByMechanic.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Open Tickets By Mechanic Report.";
