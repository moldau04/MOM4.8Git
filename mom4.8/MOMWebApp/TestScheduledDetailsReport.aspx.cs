...
public partial class TestScheduledDetailsReport : System.Web.UI.Page
{
    #region Variables
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                if (!string.IsNullOrEmpty(Request["scheduledDate"]))
                {
                    ...
                    txtScheduledDate.Text = HttpUtility.UrlDecode(Request.QueryString["scheduledDate"]);
                }
                else
                {
                    txtScheduledDate.Text = DateTime.Now.ToString("MM/dd/yyyy");
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "TestScheduledDetailsReport.pdf";
    ...
    protected void lnkClose_Click(object sender, EventArgs e)
    {
        ...
        {
            if (Request.QueryString["page"] == "SafetyTest")
            {
    ...
    {
        var url = "TestScheduledDetailsReport.aspx?scheduledDate=" + txtScheduledDate.Text;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/TestScheduledDetailsReport.mrt");
            ...
            objproptest.ScheduleDate = txtScheduledDate.Text;
            objproptest.Startdate = Session["RP_StartDate"].ToString();
            objproptest.Enddate = Session["RP_EndDate"].ToString();
            objproptest.Proposal = Session["RP_Proposal"].ToString();

            DataSet ds = objtestbl.GetTestReport(objproptest, "",
                                Session["RP_SearchTerm"].ToString(),
                                Session["RP_SqlFilter"].ToString(), Session["RP_SortOrderBy"].ToString(), Session["RP_SortType"].ToString());

            if (ds != null && ds.Tables.Count > 0)
            {
                report.RegData("ReportData", ds.Tables[1]);
                report.RegData("Violations", ds.Tables[2]);
            }

            report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
            report.Dictionary.Variables["StartDate"].Value = Session["RP_StartDate"].ToString();
            report.Dictionary.Variables["EndDate"].Value = Session["RP_EndDate"].ToString();
            report.Dictionary.Variables["ScheduledDate"].Value = txtScheduledDate.Text;
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Test Scheduled Details Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the AR Aging Summary by Location Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "TestScheduledDetailsReport.pdf")
                        {
                ...
                mail.FileName = "TestScheduledDetailsReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "TestScheduledDetailsReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=TestScheduledDetailsReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        if (dsC.Tables[0].Rows.Count > 0)
        {
            ...
            string mailBody = "Please review the attached Test Scheduled Details Report.";
