...
public partial class ManageCompanies : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Permission();
        HighlightSideMenu("progMgr", "lnkManageCompanies", "progMgrSub");

        if (!IsPostBack)
        {
            if (Convert.ToInt32(Request.QueryString["uid"]) > 0 && Convert.ToInt32(Request.QueryString["off"]) == 1)
            {
                gvOffice.Visible = true;
                gvUsers.Visible = false;
                //FillGridSearch();
            }
            else if (Convert.ToInt32(Request.QueryString["uid"]) > 0 && Convert.ToInt32(Request.QueryString["off"]) == 0)
            {
            ...
            {
                if (Session["ddlSearch_Company"] != null)
                {
                    String selectedValue = Convert.ToString(Session["ddlSearch_Company"]);
                    ddlSearch.SelectedValue = selectedValue;
                    if (Session["ddlSearch_Value_Company"] != null)
                    {
                        String searchValue = Convert.ToString(Session["ddlSearch_Value_Company"]);
                        if (selectedValue == "B.Status")
                        {
                ...
                if (Session["chk_Company"] != null)
                {
                    lnkChk.Checked = (bool)Session["chk_Company"];
                }
            }
            else
            {
                Session["ddlSearch_Company"] = null;
                Session["ddlSearch_Value_Company"] = null;
                Session["chk_Company"] = null;
            }
            #endregion
    ...
    {
        if (ddlSearch.SelectedValue == "B.Status")
        {
    ...
    private void Permission()
    {
        ...
        if (Session["type"].ToString() != "am")
        {
            ...
            hdnAddeOwner.Value = OwnerPermission.Length < 1 ? "Y" : OwnerPermission.Substring(0, 1);
            hdnEditeOwner.Value = OwnerPermission.Length < 2 ? "Y" : OwnerPermission.Substring(1, 1);
            hdnDeleteeOwner.Value = OwnerPermission.Length < 3 ? "Y" : OwnerPermission.Substring(2, 1);
            hdnViewOwner.Value = OwnerPermission.Length < 4 ? "Y" : OwnerPermission.Substring(3, 1);
    ...
    private void FillCompanyType()
    {
        ...
        ds = objBL_Company.getCompanyType(objCompany);
    ...
    private void GetDataAll()
    {
        ...
        ds = objBL_Company.getCompanies(objCompany);
    ...
    private void FillGrid()
    {
        ...
        dt = getdata();
        if (ddlSearch.SelectedIndex == 3 || ddlSearch.SelectedIndex == 4)
        {
            gvUsers.Visible = false;
            gvOffice.Visible = true;
            gvOffice.Rebind();
            //BindGridDatatableOffice(dt);
        }
        else
        {
            gvUsers.Visible = true;
    ...
    private DataTable getdataSearch()
    {
        DataSet ds = new DataSet();
        objCompany.ConnConfig = Session["config"].ToString();
        objCompany.DBName = Session["dbname"].ToString();
        ...
        if (Convert.ToInt32(Request.QueryString["uid"]) > 0 && Convert.ToInt32(Request.QueryString["off"]) == 1)
        {
            dt = Session["searchdataOffice"] as DataTable;
            Session["searchdataOffice"] = dt;
        }
        else
        {
            dt = Session["searchdataCompany"] as DataTable;
            Session["searchdataCompany"] = dt;
        }
        return dt;
    }
    ...
    private void FillGridPagedOffice()
    {
        DataTable dt = new DataTable();

        dt = PageSortDataOffice();
        BindGridDatatableOffice(dt);
        //gvUsers.DataSource = dt;
        //gvUsers.DataBind();
        //CalculateBalance();
...

        {
            ...
            else
            {
                if (dt.Rows.Count > 0)
                {
                    if (!lnkChk.Checked)
                    {
                        DataRow[] result = dt.Select("Status = 0");
                        if (result.Length > 0)
                        {
                            DataTable filterdt = dt.Select("Status = 0").CopyToDataTable();
                            //gvUsers.DataSource = filterdt;
                            Session["filteredDT"] = filterdt;
                            dt = filterdt;                            
                        }
                        else
                        {
    ...
    private void BindGridDatatableOffice(DataTable dt)
    {
        Session["Office"] = dt;
        Session["searchdataOffice"] = dt;
            ...
            else
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (result.Length > 0)
                    {
                        DataTable filterdt = dt.Select("Status = 0").CopyToDataTable();
                        dt = filterdt;
                        //gvOffice.DataSource = filterdt;
                        Session["filteredDT"] = filterdt;
                        lblRecordCount.Text = filterdt.Rows.Count.ToString() + " Record(s) found";
                    }
                    else
                    {
                        DataTable filterdt = new DataTable();
    ...
    private DataTable getdata()
    {
        ...
        {
            objCompany.SearchBy = ddlSearch.SelectedValue;
            objCompany.SearchValue = txtSearch.Text.Replace("'", "''");

            if (ddlSearch.SelectedValue == "B.Status")
            {
                objCompany.SearchValue = rbStatus.SelectedValue;
            }
            if (ddlSearch.SelectedValue == "BR.ID" || ddlSearch.SelectedValue == "BR.Name")
            {
                objCompany.SearchValue = txtSearch.Text;
                ds = objBL_Company.getOfficeSearch(objCompany);
            }
            else
            {
                ds = objBL_Company.getCompanySearch(objCompany);
            }
        }
        else
        {
    ...
    private DataTable PageSortDataOffice()
    {
        DataTable dt = new DataTable();
        dt = (DataTable)Session["searchdataOffice"];
        return dt;
    }
    ...
    private void RowSelect()
    {
        foreach (GridDataItem gr in gvUsers.Items)
        {
            ...
            if (hdnEditeOwner.Value == "Y")
            {
                gr.Attributes["ondblclick"] = "window.open('AddManageCompanies.aspx?uid=" + lblUserID.Text + "','_blank');";
            }
            else
            {
        ...
        //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "key1", "SelectedRowStyle('" + gvUsers.ClientID + "');", true);
    }

    private void RowSelectOffice()
    {
        foreach (GridDataItem gr in gvOffice.SelectedItems)
        {
            ...
            //gr.Attributes["onclick"] = "SelectRow('" + hdnSelected.ClientID + "','" + gr.ClientID + "','" + chkSelect.ClientID + "','" + gvOffice.ClientID + "',event);";

        }
        //ClientScript.RegisterStartupScript(Page.GetType(), "key1", "SelectedRowStyle('" + gvOffice.ClientID + "');", true);
    }
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (ddlSearch.SelectedIndex == 3 || ddlSearch.SelectedIndex == 4)
        {
            foreach (GridDataItem di in gvOffice.SelectedItems)
            {
                ...
                {
                    Response.Redirect("AddManageCompanies.aspx?uid=" + lblUserID.Text + "&off=" + 1);
                }
            }
        }
        else
        {
            foreach (GridDataItem di in gvUsers.SelectedItems)
            {

                HiddenField hdnSelected = (HiddenField)di.Cells[1].FindControl("hdnSelected");
                Label lblUserID = (Label)di.FindControl("lblId");
                Response.Redirect("AddManageCompanies.aspx?uid=" + lblUserID.Text + "&off=" + 0);
                //if (hdnSelected.Value == "1")
                //{
                //    Response.Redirect("AddManageCompanies.aspx?uid=" + lblUserID.Text + "&off=" + 0);
                //}
    ...
    protected void btnSubmitOffice_Click(object sender, EventArgs e)
    {
        foreach (GridDataItem di in gvOffice.SelectedItems)
        {
    ...
    protected void btnCopy_Click(object sender, EventArgs e)
    {
        foreach (GridDataItem di in gvUsers.SelectedItems)
        {
            HiddenField hdnSelected = (HiddenField)di.FindControl("hdnSelected");
            Label lblUserID = (Label)di.FindControl("lblId");
            Response.Redirect("AddManageCompanies.aspx?uid=" + lblUserID.Text + "&t=c");
            //if (hdnSelected.Value == "1")
            //{
            //    Response.Redirect("AddManageCompanies.aspx?uid=" + lblUserID.Text + "&t=c");
            //}
    ...
    //{
    //    Response.Redirect("AddManageCompanies.aspx");
    //}
    ...
    protected void lnkShowAll_Click(object sender, EventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "key1", "SwitchIncChk();", true);
    }
    ...
    protected void lnkSearch_Click(object sender, EventArgs e)
    {
        ...
        Session["ddlSearch_Company"] = selectedValue;

        if (selectedValue == "B.Status")
        {
            Session["ddlSearch_Value_Company"] = rbStatus.SelectedValue;
        }
        else
        {
            Session["ddlSearch_Value_Company"] = txtSearch.Text;
        }
        Session["chk_Company"] = lnkChk.Checked;
        #endregion
    ...
    //protected void gvOffice_DataBound(object sender, EventArgs e)
    //{
    //    GridViewRow gvrPager = gvOffice.BottomPagerRow;

    //    if (gvrPager == null) return;

    //    // get your controls from the gridview
    //    DropDownList ddlPages = (DropDownList)gvrPager.Cells[0].FindControl("ddlPagesOffice");
    //    Label lblPageCount = (Label)gvrPager.Cells[0].FindControl("lblPageCount");

    //    if (ddlPages != null)
    //    {
    //        // populate pager
    //        for (int i = 0; i < gvOffice.PageCount; i++)
    //        {

    //            int intPageNumber = i + 1;
    //            ListItem lstItem = new ListItem(intPageNumber.ToString());

    //            if (i == gvOffice.PageIndex)
    //                lstItem.Selected = true;
    ...
    //    // populate page count
    //    if (lblPageCount != null)
    //        lblPageCount.Text = gvOffice.PageCount.ToString();
    //}
    ...
    //    FillGridPaged();
    //}
    //protected void ddlPagesOffice_SelectedIndexChanged(Object sender, EventArgs e)
    //{
    //    GridViewRow gvrPager = gvUsers.BottomPagerRow;
    //    DropDownList ddlPages = (DropDownList)gvrPager.Cells[0].FindControl("ddlPagesOffice");

    //    gvOffice.PageIndex = ddlPages.SelectedIndex;
    ...
    //    FillGridPaged();
    //}
    //protected void PaginateOffice(object sender, CommandEventArgs e)
    //{
    //    // get the current page selected
    //    int intCurIndex = gvOffice.PageIndex;
    ...
    //            break;
    //        case "prev":
    //            gvOffice.PageIndex = intCurIndex - 1;
    //            break;
    //        case "next":
    //            gvOffice.PageIndex = intCurIndex + 1;
    //            break;
    ...
    protected void lnkClear_Click(object sender, EventArgs e)
    {
        ...
        //txtSearch.Text = string.Empty;
        ////ddlSearch_SelectedIndexChanged(sender, e);
        //foreach (GridColumn column in gvUsers.MasterTableView.OwnerGrid.Columns)
        //{
        //    column.CurrentFilterFunction = GridKnownFunction.NoFilter;
        //    column.CurrentFilterValue = string.Empty;
        //}
        //gvUsers.MasterTableView.FilterExpression = string.Empty;
        ////gvUsers.Rebind();
        ////pnlHeader.Update();        
        //ScriptManager.RegisterStartupScript(Page, Page.GetType(), "key1", "SwitchIncChk();", true);
        Response.Redirect("ManageCompanies.aspx?f=c");
    }
    ...
    protected void lnkchk_Click(object sender, EventArgs e)
    {
        ...
        //else
        //{
        //    DataTable dt = (DataTable)Session["filteredDT"];
        //    gvUsers.DataSource = dt;
        //    gvUsers.DataBind();
    ...
    protected void lnkOffice_Click(object sender, EventArgs e)
    {
        LinkButton lnkOfficeGrid = sender as LinkButton;
        if (lnkOfficeGrid != null)
        {
            GridViewRow grv = lnkOfficeGrid.NamingContainer as GridViewRow;
            if (grv != null)
            {
                Label lblCompID = (Label)grv.FindControl("lblId"); // Suppose employee id is unique
                objCompany.ID = Convert.ToInt32(lblCompID.Text);
                objCompany.DBName = Session["dbname"].ToString();
                objCompany.ConnConfig = Session["config"].ToString();
                DataSet dts = new DataSet();
                dts = objBL_Company.getOfficeByCompanyID(objCompany);
                if (dts.Tables[0].Rows.Count > 0)
                {
                    liEditCompany.Visible = false;
                    liEditOffice.Visible = true;
                    gvUsers.Visible = false;
                    gvOffice.Visible = true;
                    gvOffice.DataSource = dts.Tables[0];
                    BindGridDatatableOffice(dts.Tables[0]);
                    lblRecordCount.Text = dts.Tables[0].Rows.Count.ToString() + " Record(s) found";
                }
                else
                {
                    liEditCompany.Visible = true;
                    liEditOffice.Visible = false;
                    gvUsers.Visible = true;
                    gvOffice.Visible = false;
                }
            }
    ...
    protected void gvUsers_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Convert.ToString(Request.QueryString["f"]) != "c")
            {
                if (Session["Comp_FilterExpression"] != null && Convert.ToString(Session["Comp_FilterExpression"]) != "" && Session["Comp_Filters"] != null)
                {
                    gvUsers.MasterTableView.FilterExpression = Convert.ToString(Session["Comp_FilterExpression"]);
                    var filtersGet = Session["Comp_Filters"] as List<RetainFilter>;
                    if (filtersGet != null)
                    {
                        foreach (var _filter in filtersGet)
                        {
                            GridColumn column = gvUsers.MasterTableView.GetColumnSafe(_filter.FilterColumn);
            ...
            {
                Session["Comp_FilterExpression"] = null;
                Session["Comp_Filters"] = null;
            }
        }
        #endregion
    ...
    protected void gvOffice_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
    {
        DataTable dt = getdata();
        BindGridDatatableOffice(dt);
    }
    ...
    protected void gvUsers_PreRender(object sender, EventArgs e)
    {
        ...
        String filterExpression = Convert.ToString(gvUsers.MasterTableView.FilterExpression);
        if (filterExpression != "")
        {
            Session["Comp_FilterExpression"] = filterExpression;
    ...
    protected void gvUsers_ItemCreated(object sender, GridItemEventArgs e)
    {
