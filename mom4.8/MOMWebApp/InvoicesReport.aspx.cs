...
public partial class InvoicesReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "InvoicesReport.pdf";
    ...
    {
        var url = "InvoicesReport.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/InvoicesListReport.mrt");
            ...
            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objContract.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
            }
            else
            {
                objContract.EndDate = objContract.StartDate.AddMonths(1).AddDays(-1);
            }
            ...
            if (!string.IsNullOrEmpty(Request["amtpaid"]))
            {
                objContract.SearchAmtPaidUnpaid ="(" + Request.QueryString["amtpaid"] + ")";
            }

            if (!string.IsNullOrEmpty(Request["printemail"]))
            {
                objContract.SearchPrintMail = Request.QueryString["printemail"];
            }

            List<RetainFilter> filters = new List<RetainFilter>();
            if (Session["Invoice_Filters"]!= null)
            {
                filters = (List<RetainFilter>)Session["Invoice_Filters"];
            }

            DataSet ds = objBL_Contracts.GetInvoices(objContract, filters, null, null);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Invoices Report attached.";
                }
                ...
                mail.FileName = "InvoicesReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "InvoicesReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=InvoicesReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Invoices Report.";
