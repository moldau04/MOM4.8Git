...
public partial class OpportunityForecastReport : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    txtfromDate.Text = now.ToString("MM/dd/yyyy");
                }
                ...
                {
                    pageTitle.InnerText = "Open Opportunities by Month [Maintenance & Modernization]";
                    pageEmailTitle.InnerText = "Open Opportunities by Month [Maintenance & Modernization]";
                }
                else
                {
                    pageTitle.InnerText = "Open Opportunities by Month [M&R, Repair, Other]";
                    pageEmailTitle.InnerText = "Open Opportunities by Month [M&R, Repair, Other]";
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "OpenOpportunitiesByMonthReport.pdf";
    ...
    protected void StiWebViewerOpportunity_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerOpportunity_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    {
        var url = "OpportunityForecastReport.aspx?sd=" + txtfromDate.Text  + "&ed=" + txtToDate.Text;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/OpportunityForecastReport.mrt");
            ...
            objProp_Customer.ConnConfig = connString;

            var reportType = 0;
            if (!string.IsNullOrEmpty(Request["type"]))
            {
                reportType = Convert.ToInt32(Request["type"]);
            }

            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objProp_Customer.StartDate = Request["sd"];
            }
            else
            {
                DateTime now = DateTime.Now;
                objProp_Customer.StartDate = new DateTime(now.Year, now.Month, 1).ToShortDateString();
            }

            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objProp_Customer.EndDate = Request["ed"];
            }
            else
            {
                objProp_Customer.EndDate = DateTime.Now.ToShortDateString();
            }
            ...
            DataSet  ds = objBL_Customer.GetOpportunityForecast(objProp_Customer, reportType);
            if (ds != null && ds.Tables.Count > 0)
            {
                report.RegData("Opportunities", ds.Tables[0]);
                report.RegData("ChartData", ds.Tables[1]);
            }
            ...
            report.Dictionary.Variables["BusinessTypeLabel"].Value = GetBusinessTypeLabel();

            if (reportType != 0)
            {
                report.Dictionary.Variables["Department"].Value = "[Maintenance & Modernization]";
            }
            else
            {
                report.Dictionary.Variables["Department"].Value = "[M&R, Repair, Other]";
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Open Opportunities by Month Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Open Opportunities by Month Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "OpenOpportunitiesByMonthReport.pdf")
                        {
                ...
                mail.FileName = "OpenOpportunitiesByMonthReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "OpenOpportunitiesByMonthReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=OpenOpportunitiesByMonthReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Open Opportunities by Month Report.";
