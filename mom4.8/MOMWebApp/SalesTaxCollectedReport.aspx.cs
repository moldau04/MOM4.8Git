...
public partial class SalesTaxCollectedReport : System.Web.UI.Page
{
    ...
    {
        string reportPathStimul = Server.MapPath("StimulsoftReports/SalesTaxCollectedReport.mrt");
        if (string.IsNullOrEmpty(Request.QueryString["type"]))
        {
            reportPathStimul = Server.MapPath("StimulsoftReports/SalesTaxCollectedReport.mrt");
        }
        else
        {
            if (Request.QueryString["type"] == "0")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/SalesTaxCollectedSummaryReport.mrt");
            }
        }
    ...
    protected void StiWebViewerSalesTax_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        ...
        try
        {
            ...
            if (Request.QueryString["type"] == "0")
            {
                ds = objBLInvoices.GetSalesTaxCollected(objInvoices);
            }
            if (Request.QueryString["type"] == "1")
            {
                ds = objBLInvoices.GetSalesTaxCollectedDetail(objInvoices);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Sales Tax Collected Report";
                ...
                {
                    mail.Text = "This report is generated from Mobile Office Manager. Please find attached the Sales Tax Collected Report.";
                }
                ...
                if (poToPrint != null && poToPrint.Count > 0)
                {
                    ...
                    mail.FileName = "SalesTaxCollected.pdf";
                }
    ...
    private List<byte[]> PrintTemplate(StiWebViewer rvTemplate)
    {
        ...
        try
        {
            ...
            DataSet ds = objBLInvoices.GetSalesTaxCollected(objInvoices);
            if (ds != null)
            {
                report.RegData("ReportData", ds.Tables[0]);
            }
    ...
    public void processSaleTaxReport()
    {
        try
        {
            ...
            String url = "SalesTaxCollectedReport.aspx?redirect=Invoices.aspx&type=" + type + "&fromDate=" + fromDate.ToString("MM/dd/yyyy") + "&toDate=" + endDate.ToString("MM/dd/yyyy");
