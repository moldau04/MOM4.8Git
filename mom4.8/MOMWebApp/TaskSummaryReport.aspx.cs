...
public partial class TaskSummaryReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (!IsPostBack)
                {
                    HighlightSideMenu("SalesMgr", "lnkTasks", "SalesMgrSub");

                    GetSMTPUser();
                    SetAddress();
                    string FileName = "TaskSummaryReport.pdf";
    ...
    protected void StiWebViewerTaskSummary_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerTaskSummary_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/TaskSummaryReport.mrt");
            ...
            {
                objProp_Customer.EndDate = Request.QueryString["ed"];
            }

            if (!string.IsNullOrEmpty(Request.QueryString["stype"]) && !string.IsNullOrEmpty(Request.QueryString["stext"]))
            {
                objProp_Customer.SearchBy = Request.QueryString["stype"];
                objProp_Customer.SearchValue = Request.QueryString["stext"];
            }
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["chkAllUsers"]) && Convert.ToBoolean(Request.QueryString["chkAllUsers"]))
            {
            ...
            int mode = 0;
            if (!string.IsNullOrEmpty(Request.QueryString["chkAllTask"]))
            {
                if (Convert.ToBoolean(Request.QueryString["chkAllTask"]))
                {
                    mode = 5;
                }
                else
            ...
            DataSet ds = objBL_Customer.getTasks(objProp_Customer, new GeneralFunctions().GetSalesAsigned());
            if (ds != null)
            {
                report.RegData("ReportData", ds.Tables[0]);

                var chartData = from row in ds.Tables[0].AsEnumerable()
                                group row by row.Field<string>("fUser") into sales
                                orderby sales.Key
                                select new
                                {
                                    Salesperson = sales.Key,
                                    Quantity = sales.Count()
                                };

                var dtChart = ToDataTable(chartData.ToList());

                report.RegData("ChartData", dtChart);
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Task Summary Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Task Summary Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "TaskSummaryReport.pdf")
                        {
                ...
                mail.FileName = "TaskSummaryReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "TaskSummaryReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=TaskSummaryReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Task Summary Report.";
