...
public partial class DeliveredNoDeliveryPaymentReport : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Unit Delivered" || sortBy == "Project #" || sortBy == "Delivery Pymt Rcvd")
                    {
                        sortBy = "[" + sortBy + "]";
                    }
    ...
    private void GetCustReportFiltersValue()
    {
        ...
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetCustReportFiltersValue = objBL_ReportsData.GetDeliveryReportFiltersValue(objProp_User);
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            objProp_User.DBName = Session["dbname"].ToString();
            objProp_User.ConnConfig = Session["config"].ToString();
            dsGetCustDetails = objBL_ReportsData.GetDeliveryDetails(objProp_User);
            ...
            if (dsGetCustDetails.Tables.Count > 0)
            {
                ...
                _newLstBox.Text += "<label id='lblText' style='font-weight:bolder'>DND Payment</label>";
    ...
    protected void btnSaveReport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            objCustReport.Module = "Delivered";
    ...
    private void BindGridReport(string[] checkedColumns, string[] selectedFiltersColumns, string[] selectedFiltersValues, string sortBy)
    {
        ...
        {
            query += " FROM vw_DeliveryReportDetails order by " + sortBy;
        }
        else
        {
            ...
            if (selectedFiltersColumns != null)
            {
                for (int i = 0; i <= selectedFiltersColumns.Count() - 1; i++)
                {
                    ...
                    if (selectedFiltersColumns[i] == "DeliveryPymtRcvd")
                    {
                        selectedFiltersColumns[i] = "Delivery Pymt Rcvd";
                    }

                    if (selectedFiltersColumns[i] == "UnitDelivered")
                    {
                        selectedFiltersColumns[i] = "Unit Delivered";
                    }
            ...
            if (!string.IsNullOrEmpty(filters))
            {
                ...
                query += " FROM vw_DeliveryReportDetails where " + filters + " order by " + sortBy;
            }
            else
            {
                query += " FROM vw_DeliveryReportDetails order by " + sortBy;
    ...
    public static string GetDeliveryPreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            DeliveredNoDeliveryPaymentReport _a = new DeliveredNoDeliveryPaymentReport();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (DataSortBy == "Unit Delivered" || DataSortBy == "Project #" || DataSortBy == "Delivery Pymt Rcvd")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_DeliveryReportDetails order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_DeliveryReportDetails where " + filters + " order by " + DataSortBy;
            }
            #endregion
