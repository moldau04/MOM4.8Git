...
public partial class AddInvoiceNew : System.Web.UI.Page
{
...
"We appreciate your business!" + Environment.NewLine + Environment.NewLine + address;
    ...
    private void FillLocInfo()
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                hdnsTaxType.Value = ds.Tables[0].Rows[0]["sTaxType"].ToString();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (GridDataItem gr in RadGrid_gvInvoices.Items)
            {
                ...
                {
                    //ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyProj", "noty({text: 'This Account\\Billing code is inactive. Please change account\\billing code name before proceeding.', dismissQueue: true,  type : 'warning', layout:'topCenter',closeOnSelfClick:true, timeout : false,theme : 'noty_theme_default',  closable : false});", true);
    ...
    private void Submit(Boolean IsPreview = false)
    {
                ...
                if (Validate(dtInvoiceCopy))
                {
                    ...
                    if (Convert.ToInt32(ViewState["mode"]) == 1)
                    {
                        ...
                        if (ddlStatus.SelectedValue == "2")
                        {
                            ...
                            Response.Redirect("AddInvoiceNew.aspx?uid=" + Request.QueryString["uid"] + page);
                        }
                        #endregion
                    ...
                    else
                    {
                        ...
                        if (Request.QueryString["tickid"] != null)
                        {
                            ScriptManager.RegisterStartupScript(this, Page.GetType(), "updateparent", "if(window.opener && !window.opener.closed) { if(window.opener.document.getElementById('lnkRefressTicketList')) window.opener.document.getElementById('lnkRefressTicketList').click();setTimeout('window.close();', 4000);}", true);
    ...
    protected void lnkClose_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("invoicesnew.aspx?fil=1");
        }
    }
    ...
    private DataSet BuildLineItems(double totalTime, double Expenses, int mileage)
    {
        ...
        try
        {
            ...
            if (Request.QueryString["tickid"] != null)
            {
                ...
                //string RT_TT = "0";
                ///////////////
                string BillRatePrice = "0";
                string RateOTPrice = "0";
                string RateDTPrice = "0";
                string RateNTPrice = "0";
                string RateMileagePrice = "0";
                string RateTravelPrice = "0";
                ...
                if (dsLineItem2.Tables[0].Rows.Count > 0 && dsLineItem2.Tables[1].Rows.Count > 0 && dsLineItem2.Tables[2].Rows.Count > 0 && dsLineItem2.Tables[3].Rows.Count > 0 && dsLineItem2.Tables[4].Rows.Count > 0 && dsLineItem2.Tables[5].Rows.Count > 0 && dsLineItem2.Tables[6].Rows.Count > 0)
                {
                    ...
                    BillRatePrice = dsLineItem2.Tables[6].Rows[0]["BillRatePrice"].ToString().Trim();
                    RateOTPrice = dsLineItem2.Tables[6].Rows[0]["RateOTPrice"].ToString().Trim();
                    RateDTPrice = dsLineItem2.Tables[6].Rows[0]["RateDTPrice"].ToString().Trim();
                    RateNTPrice = dsLineItem2.Tables[6].Rows[0]["RateNTPrice"].ToString().Trim();
                    RateMileagePrice = dsLineItem2.Tables[6].Rows[0]["RateMileagePrice"].ToString().Trim();
                    RateTravelPrice = dsLineItem2.Tables[6].Rows[0]["RateTravelPrice"].ToString().Trim();
                ...
                if (Request.QueryString["tickid"] != null)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        if (ds.Tables[0].Rows[0]["invoice"] != DBNull.Value)
                        {
                            ...
                            {
                                Response.Redirect("addinvoiceNew.aspx?o=1&uid=" + ds.Tables[0].Rows[0]["invoice"].ToString());
                            }
                        }
                        ...
                        txtRemarks.Text = "Invoice for Ticket# " + ViewState["othertickets"].ToString() + Environment.NewLine;
    ...
    protected void RadGrid_gvInvoices_PreRender(object sender, EventArgs e)
    {
        foreach (GridDataItem gr in RadGrid_gvInvoices.Items)
        {
            ...
            //txtBillingCode.Text = "";// ddlBillingCode.SelectedItem.Text;
            //txtBillingCodeID.Value = "";// ddlBillingCode.SelectedItem.Value;
            lblQuantity.Attributes["onkeyup"] = "document.getElementById('" + hdnFocus.ClientID + "').value='" + lblQuantity.ClientID + "'; CalculateGridAmount(); ";
            // CheckAddRow('" + gvInvoices.ClientID + "','" + Convert.ToInt32(lblIndex.Text) + "','" + lnkAddnew.ClientID + "');
            ...
            //gr.Attributes["onclick"] = "SelectRowChk('" + gr.ClientID + "','" + chkSelect.ClientID + "','" + RadGrid_gvInvoices.ClientID + "',event);";
    ...
    protected void RadGrid_gvInvoices_ItemCreated(object sender, GridItemEventArgs e)
    {
        ...
        if (!Convert.ToBoolean(ViewState["isCanada"]))
        {
            ...
            {
                GridHeaderItem gd = (GridHeaderItem)e.Item;
        ...
        try
        {
            ...
            if (Convert.ToBoolean(ViewState["isCanada"]))
            {
                if (hdnsTaxType.Value == "3")
                {
                    ...
                    RadGrid_gvInvoices.Columns.FindByUniqueName("stax").Visible = false;
                    RadGrid_gvInvoices.Columns.FindByUniqueName("STaxAmt").Visible = false;
                }
                else
                {
                    if (hdnsTaxType.Value == "2")
    ...
    private void SetupCanadaCompanyUI()
    {
        ...
        if (Convert.ToBoolean(ViewState["isCanada"]))
        {
            //for grid      RadGrid_gvInvoices.Columns.FindByUniqueName("stax").HeaderText = "HST/PST";
