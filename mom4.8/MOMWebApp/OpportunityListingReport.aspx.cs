...
public partial class OpportunityListingReport : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Sales Person" || sortBy == "Close Date" || sortBy == "Create Date")
                    {
                        sortBy = "[" + sortBy + "]";
                    }

                    sortBy += " " + (isAscending == true ? "Asc" : "Desc");

                    condition = dsGetRptDetails.Tables[0].Rows[0]["Condition"].ToString();
                    if (!string.IsNullOrEmpty(condition))
                    {
                        string[] spearator = { " AND " };
                        var items = condition.Split(spearator, 2, StringSplitOptions.RemoveEmptyEntries);
                        foreach(var item in items)
                        {
                            if(item.Contains("v.[Create Date] >="))
                            {
                                txtStartDate.Text = Convert.ToDateTime(item.Replace("v.[Create Date] >=", "").Replace("'", "").Trim()).ToString("MM/dd/yyyy");
                            }

                            if (item.Contains("v.[Create Date] <="))
                            {
                                txtEndDate.Text = Convert.ToDateTime(item.Replace("v.[Create Date] <=", "").Replace("'", "").Trim()).ToString("MM/dd/yyyy");
                            }
                        }
                    }
                }
    ...
    {
        DataSet dsGetOpportunityReportFiltersValue = new DataSet();
        objProp_User.DBName = Session["dbname"].ToString();
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetOpportunityReportFiltersValue = objBL_ReportsData.GetOpportunityReportFiltersValue(objProp_User);

        if (dsGetOpportunityReportFiltersValue.Tables[0].Rows.Count > 0)
        {
            drpOpportunity.DataSource = dsGetOpportunityReportFiltersValue.Tables[0];
            drpOpportunity.DataTextField = "Opportunity#";
            drpOpportunity.DataValueField = "Opportunity#";
            drpOpportunity.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[2].Rows.Count > 0)
        {
            drpName.DataSource = dsGetOpportunityReportFiltersValue.Tables[2];
            drpName.DataTextField = "Name";
            drpName.DataValueField = "Name";
            drpName.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[4].Rows.Count > 0)
        {
            drpProbability.DataSource = dsGetOpportunityReportFiltersValue.Tables[4];
            drpProbability.DataTextField = "Probability";
            drpProbability.DataValueField = "Probability";
            drpProbability.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[1].Rows.Count > 0)
        {
            drpContact.DataSource = dsGetOpportunityReportFiltersValue.Tables[1];
            drpContact.DataTextField = "Contact";
            drpContact.DataValueField = "Contact";
            drpContact.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[5].Rows.Count > 0)
        {
            drpCloseDate.DataSource = dsGetOpportunityReportFiltersValue.Tables[5];
            drpCloseDate.DataTextField = "Close Date";
            drpCloseDate.DataValueField = "Close Date";
            drpCloseDate.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[3].Rows.Count > 0)
        {
            drpStatus.DataSource = dsGetOpportunityReportFiltersValue.Tables[3];
            drpStatus.DataTextField = "Status";
            drpStatus.DataValueField = "Status";
            drpStatus.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[6].Rows.Count > 0)
        {
            drpClosed.DataSource = dsGetOpportunityReportFiltersValue.Tables[6];
            drpClosed.DataTextField = "Closed";
            drpClosed.DataValueField = "Closed";
            drpClosed.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[8].Rows.Count > 0)
        {
            drpSalesPerson.DataSource = dsGetOpportunityReportFiltersValue.Tables[8];
            drpSalesPerson.DataTextField = "Sales Person";
            drpSalesPerson.DataValueField = "Sales Person";
            drpSalesPerson.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[10].Rows.Count > 0)
        {
            drpProject.DataSource = dsGetOpportunityReportFiltersValue.Tables[10];
            drpProject.DataTextField = "Project#";
            drpProject.DataValueField = "Project#";
            drpProject.DataBind();
        }

        if (dsGetOpportunityReportFiltersValue.Tables[9].Rows.Count > 0)
        {
            drpEstimate.DataSource = dsGetOpportunityReportFiltersValue.Tables[9];
            drpEstimate.DataTextField = "Estimate#";
            drpEstimate.DataValueField = "Estimate#";
            drpEstimate.DataBind();
        }
    }
    ...
    private void GetReportColumnsByRepId()
    {
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                BindGridReport(checkedColumns, selectedFiltersColumns, selectedFiltersValues, sortBy, condition);
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            dsGetCustDetails = objBL_ReportsData.GetOpportunityDetails(objProp_User);
            ...
            {
                dsGetCustDetails.Tables[0].TableName = "Opportunity";
    ...
    protected void btnSaveReport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            objCustReport.Module = "Opportunity";
            ...
            {
                conditionText += "v.[Create Date] >= '" + Convert.ToDateTime(txtStartDate.Text).ToString() + "' ";
            }
            ...
            if (!string.IsNullOrEmpty(txtEndDate.Text))
            {
                ...
                conditionText += "v.[Create Date] <= '" + Convert.ToDateTime(txtEndDate.Text).AddDays(1).AddSeconds(-1).ToString() + "' ";
            }

            objCustReport.Condition = conditionText;
    ...
    private void BindGridReport(string[] checkedColumns, string[] selectedFiltersColumns, string[] selectedFiltersValues, string sortBy, string condition)
    {
        ...
        if (selectedFiltersColumns != null)
        {
            ...
            {
                if (selectedFiltersColumns[i] == "EstimatePrice")
                {
                    selectedFiltersColumns[i] = "Estimate Price";
                }

                if (selectedFiltersColumns[i] == "QuotedPrice")
                {
                    selectedFiltersColumns[i] = "Quoted Price";
                }
        ...
        {
            query += " FROM vw_OpportunityReportDetails v order by " + sortBy;
        }
        else
        {
            filters = filters.Substring(0, filters.Length - 4);
            query += " FROM vw_OpportunityReportDetails v where " + filters + " order by " + sortBy;
        }
    ...
    public static string GetOpportunityPreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            OpportunityListingReport _a = new OpportunityListingReport();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            BusinessEntity.User objProp_User = new BusinessEntity.User();
            CustomerReport objCustReport = new CustomerReport();
            DataSet dsCompDetail = new DataSet();
            ...
            if (DataSortBy == "Sales Person" || DataSortBy == "Close Date" || DataSortBy == "Create Date")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_OpportunityReportDetails v order by " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_OpportunityReportDetails v where " + filters + " order by " + DataSortBy;
            }
            #endregion
