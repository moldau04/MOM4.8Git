...
public partial class ReceivePaymentListReport : System.Web.UI.Page
{
    ...
    private StiReport LoadLReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/ReceivePaymentListReport.mrt");
            ...
            {
                string APINAME = "ReceivePaymentAPI/ReceivePaymentListReport_GetCompanyDetails";
            ...
            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                ...
                objReceivePay.StartDate = startDate;
            }
            ...
            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                ...
                objReceivePay.EndDate = endDate.AddDays(1).AddSeconds(-1);
            }
            ...
            {
                ///Get  rad grid view  search filter value from Receive Payment View
                filters = (List<RetainFilter>)Session["ReceivePayment_Filters"];
            }

            objReceivePay.ConnConfig = connString;

            var dsReceive = objBL_Deposit.GetReceivePaymentReport(objReceivePay, filters, intEN);

            if (dsReceive != null)
            {
                report.RegData("ReceivePayment", dsReceive.Tables[0]);
            }
    ...
    private void GetSMTPUser()
    {
        ...
        if (IsAPIIntegrationEnable == "YES")
        {
            string APINAME = "ReceivePaymentAPI/ReceivePaymentListReport_GetSMTPByUserID";
