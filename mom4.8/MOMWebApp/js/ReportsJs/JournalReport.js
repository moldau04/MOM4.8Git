...
function EmptyFilters() {
    ...
    $('#ctl00_ContentPlaceHolder1_drpDate input[type="checkbox"]').attr('checked', false);

    $("#tblInternalRef").css("display", "none");
    $('#ctl00_ContentPlaceHolder1_drpInternalRef input[type="checkbox"]').attr('checked', false);
...
function setFiltersValue(filterName, filterValue) {
    ...
    if (filterName == "Due Date") {
        ...
        if (filterValue != '') {
            var splitedNames = filterValue.split("|");
            for (i = 0; i <= splitedNames.length - 1; i++) {
                $('#ctl00_ContentPlaceHolder1_drpDate input[type=checkbox]').each(function () {
                    var lblValue = $(this).next('label').html();
                    splitedNames[i] = splitedNames[i].replace("'", "").replace("'", "");
                    if (lblValue.trim() == splitedNames[i].trim()) {
                        $(this).attr('checked', true);
    ...
    if (filterName == "Internal Ref") {

        $('#ctl00_ContentPlaceHolder1_drpInternalRef input[type="checkbox"]').attr('checked', false);
        if (filterValue != '') {
            var splitedNames = filterValue.split("|");
            for (i = 0; i <= splitedNames.length - 1; i++) {
                $('#ctl00_ContentPlaceHolder1_drpInternalRef input[type=checkbox]').each(function () {
                    var lblValue = $(this).next('label').html();
                    splitedNames[i] = splitedNames[i].replace("'", "").replace("'", "");
                    if (lblValue.trim() == splitedNames[i].trim()) {
                        $(this).attr('checked', true);
    ...
    if (filterName == "Subject" || filterName == "Desc" || filterName == "Ref" || filterName == "Status" || filterName == "Date") {
...
jQuery(document).ready(function () {
    ...
    $("#btnPreview").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "JournalListingReport.aspx/GeJournalReportPreviewDetails",
    ...
    $("#btnNewReport").click(function () {
        ...
        $('#spnModelTitle').html('New Report: Journal Entry Report');
    ...
    $('#ctl00_ContentPlaceHolder1_drpDate input[type="checkbox"]').click(function () {
        SetFiltersChoicesFromDropDown("#ctl00_ContentPlaceHolder1_drpDate", "Date");
    });

    $('#ctl00_ContentPlaceHolder1_drpInternalRef input[type="checkbox"]').click(function () {
        SetFiltersChoicesFromDropDown("#ctl00_ContentPlaceHolder1_drpInternalRef", "InternalRef");
    });
