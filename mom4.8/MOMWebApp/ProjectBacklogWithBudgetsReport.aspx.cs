...
public partial class ProjectBacklogWithBudgetsReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            GetSMTPUser();
            SetAddress();
            string FileName = "ProjectBacklogWithBudgetsReport.pdf";
    ...
    {
        Response.Redirect("Project.aspx" + HttpContext.Current.Request.Url.Query);
    }
    ...
    protected void lnkExcel_Click(object sender, EventArgs e)
    {
        ...
        report = LoadProjectBacklogReport();
        ...
        Response.AddHeader("Content-Disposition", $"attachment;filename=ProjectBacklogWithBudgetsReport.xls");
    ...
    protected void StiWebViewerProjectBacklog_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadProjectBacklogReport();
    }

    protected void StiWebViewerProjectBacklog_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {

    }

    private StiReport LoadProjectBacklogReport()
    {
        string reportPathStimul = Server.MapPath("StimulsoftReports/Project/ProjectBacklogWithBudgetsReport.mrt");

        if (ConfigurationManager.AppSettings["ProjectBacklogWithBudgetsReport"] != null && ConfigurationManager.AppSettings["ProjectBacklogWithBudgetsReport"].Contains(".mrt"))
        {
            reportPathStimul = Server.MapPath($"StimulsoftReports/Project/{ConfigurationManager.AppSettings["ProjectBacklogWithBudgetsReport"]}");
        }
        ...
        report.RegData("dtCompany", dsCompany.Tables[0]);
        ...
        if (HttpContext.Current.Request.QueryString["sb"].ToString() != "")
        {
            objProp_Customer.SearchBy = HttpContext.Current.Request.QueryString["sb"].ToString();
        }

        if (HttpContext.Current.Request.QueryString["sv"].ToString() != "")
        {
            objProp_Customer.SearchValue = HttpContext.Current.Request.QueryString["sv"].ToString();
        }

        if (HttpContext.Current.Request.QueryString["df"].ToString() != "")
        {
            startDate = Convert.ToDateTime(HttpContext.Current.Request.QueryString["df"].ToString());
            objProp_Customer.StartDate = startDate.ToShortDateString();
        }

        if (HttpContext.Current.Request.QueryString["dt"].ToString() != "")
        {
            endDate = Convert.ToDateTime(HttpContext.Current.Request.QueryString["dt"].ToString());
            objProp_Customer.EndDate = endDate.ToShortDateString();
        }

        objProp_Customer.Range = Convert.ToInt16(HttpContext.Current.Request.QueryString["rng"].ToString());

        short templateId = -1;
        if (HttpContext.Current.Request.QueryString["tab"].ToString() != "")
        {
            templateId = Convert.ToInt16(HttpContext.Current.Request.QueryString["tab"].ToString());
        }

        int IncludeClose = 0;

        if (HttpContext.Current.Request.QueryString["close"].ToString() != "")
        {
        ...
        objProp_Customer.JobType = templateId;
        ...
        DataSet ds = objBL_Customer.getJobProject(objProp_Customer, dtFilters, new GeneralFunctions().GetSalesAsigned(), IncludeClose);

        if (ds != null)
        {
            report.RegData("dtJob", ds.Tables[0]);
        }

        report.Dictionary.Variables["StartDate"].Value = objProp_Customer.StartDate;
        report.Dictionary.Variables["EndDate"].Value = objProp_Customer.EndDate;
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Project Backlog with Budgets Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Project Backlog with Budgets Report attached.";
                }
                ...
                System.IO.MemoryStream stream = new System.IO.MemoryStream();
                service.ExportTo(LoadProjectBacklogReport(), stream, settings);
                buffer1 = stream.ToArray();
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "ProjectBacklogWithBudgetsReport.pdf")
                        {
                ...
                mail.FileName = "ProjectBacklogWithBudgetsReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ProjectBacklogWithBudgetsReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ProjectBacklogWithBudgetsReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Project Backlog with Budgets Report.";
