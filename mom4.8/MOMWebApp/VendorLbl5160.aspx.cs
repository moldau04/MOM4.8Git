...
public partial class VendorLbl5160 : System.Web.UI.Page
{
    ...
    GetVendorLabelParam _getVendorLabel = new GetVendorLabelParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    ddlVendorSearch.SelectedValue = HttpUtility.UrlDecode(Request.QueryString["stype"]);
                }

                if (!string.IsNullOrEmpty(Request.QueryString["stext"]))
                {
                    txtVendorSearch.Text = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "VendorLabel5160.pdf";
    ...
    protected void StiWebViewerVendorLabel_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadVendorLabelReport();
    }

    protected void StiWebViewerVendorLabel_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    {
        Response.Redirect("VendorLbl5160.aspx?stype=" + ddlVendorSearch.SelectedValue + "&stext=" + txtVendorSearch.Text);
    }

    private StiReport LoadVendorLabelReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/VendorLabel5160.mrt");
            ...
            {
                string APINAME = "VendorAPI/VendorReport_GetCompanyDetails";
            ...
            _getVendorLabel.ConnConfig = connString;

            if (!string.IsNullOrEmpty(Request.QueryString["stype"]))
            {
                if (Request.QueryString["stype"] == "Name")
                {
                    objCust.Name = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _getVendorLabel.Name = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "Address")
                {
                    objCust.Address = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _getVendorLabel.Address = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "City")
                {
                    objCust.City = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _getVendorLabel.City = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "State")
                {
                    objCust.State = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _getVendorLabel.State = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
                if (Request.QueryString["stype"] == "Zip")
                {
                    objCust.Zip = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                    _getVendorLabel.Zip = HttpUtility.UrlDecode(Request.QueryString["stext"]);
                }
            }
            ...
            List<RolViewModel> _RolViewModel = new List<RolViewModel>();

            _objAPIIntegration = (APIIntegrationModel)Session["IsAPIIntegration"];
            //if (IsAPIIntegrationEnable == "YES")
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "VendorAPI/VendorReport_GetVendorLabel";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getVendorLabel, true);

                _RolViewModel = (new JavaScriptSerializer()).Deserialize<List<RolViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<RolViewModel>(_RolViewModel);
            }
            else
            {
                ds = objBL_Report.GetVendorLabel(objCust);
            }
    ...
    private void GetSMTPUser()
    {
        ...
        List<SMTPEmailViewModel> _SMTPEmailViewModel = new List<SMTPEmailViewModel>();
        ...
        if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
        {
            string APINAME = "VendorAPI/VendorReport_GetSMTPByUserID";

            APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getSMTPByUserID, true);

            _SMTPEmailViewModel = (new JavaScriptSerializer()).Deserialize<List<SMTPEmailViewModel>>(_APIResponse.ResponseData);
            ds = CommonMethods.ToDataSet<SMTPEmailViewModel>(_SMTPEmailViewModel);
        }
        else
        {
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Vendor Label 5160 Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Vendor Label 5160 Report attached.";
                }
                ...
                service.ExportTo(LoadVendorLabelReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "VendorLabel5160.pdf")
                        {
                ...
                mail.FileName = "VendorLabel5160.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "VendorLabel5160.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=VendorLabel5160.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Vendor Label 5160 Report.";
