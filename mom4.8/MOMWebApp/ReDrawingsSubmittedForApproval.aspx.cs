...
public partial class ReDrawingsSubmittedForApproval : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (pubReportId != 0)
            {
                ...
                if (dsGetRptDetails.Tables.Count > 0)
                {
                    ...
                    if (sortBy == "Re-Do Drawings Submitted for approval" || sortBy == "Project #" || sortBy == "Re-Do Drawings Approved")
                    {
                        sortBy = "[" + sortBy + "]";
                    }
    ...
    private void GetCustReportFiltersValue()
    {
        ...
        objProp_User.ConnConfig = Session["config"].ToString();
        dsGetCustReportFiltersValue = objBL_ReportsData.GetReDrawingsReportFiltersValue(objProp_User);
    ...
    private void GetCustomerDetails()
    {
        try
        {
            ...
            objProp_User.DBName = Session["dbname"].ToString();
            objProp_User.ConnConfig = Session["config"].ToString();
            dsGetCustDetails = objBL_ReportsData.GetReDrawingsColumns(objProp_User);

            if (dsGetCustDetails.Tables.Count > 0)
            {
                var table = dsGetCustDetails.Tables[0];
                ...
                {
                    chkColumnList.Items.Add(row["COLUMN_NAME"].ToString());
                    lstFilter.Items.Add(row["COLUMN_NAME"].ToString());
                }
    ...
    protected void btnSaveReport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            objCustReport.Module = "ReDrawings";
    ...
    private void BindGridReport(string[] checkedColumns, string[] selectedFiltersColumns, string[] selectedFiltersValues, string sortBy)
    {
        ...
        {
            query += " FROM vw_ReDrawingsSubmittedForApproval ORDER BY " + sortBy;
        }
        else
        {
            ...
            query += " FROM vw_ReDrawingsSubmittedForApproval WHERE " + filters + " ORDER BY " + sortBy;
        }
    ...
    public static string GetReDrawingPreviewDetails(int reportId, string FilterColumn, string FilterValues, string ColumnWidth, string SortColumn, string DataSortBy)
    {
        try
        {
            ReDrawingsSubmittedForApproval _a = new ReDrawingsSubmittedForApproval();
            BL_ReportsData objBL_ReportsData = new BL_ReportsData();
            ...
            if (DataSortBy == "Re-Do Drawings Submitted for approval" || DataSortBy == "Project #" || DataSortBy == "Re-Do Drawings Approved")
            {
            ...
            if (selectedFiltersColumns == null)
            {
                query += " FROM vw_ReDrawingsSubmittedForApproval ORDER BY " + DataSortBy;
            }
            else
            {
                string filters = string.Empty;
                ...
                filters = filters.Substring(0, filters.Length - 4);
                query += " FROM vw_ReDrawingsSubmittedForApproval WHERE " + filters + " ORDER BY " + DataSortBy;
            }
            #endregion
