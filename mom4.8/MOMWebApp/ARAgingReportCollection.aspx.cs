...
public partial class ARAgingReportCollection : System.Web.UI.Page
{
    #region Variables
    ...
    {
        var url = "ARAgingReportCollection.aspx?inclNotes=" + chkIncludeNotes.Checked + "&creditFlag=" + chkCreditFlag.Checked + "&showRetainage=" + chkRetainageAmount.Checked;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingCollectionReport.mrt");
            if (!string.IsNullOrEmpty(Request["showRetainage"]) && Request["showRetainage"] == "True")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/ARAgingCollectionReport_Retainage.mrt");
            }
            ...
            if (Session["CollectionARData"] != null)
            {
                ...
                if (!string.IsNullOrEmpty(Request["creditFlag"]) && Convert.ToBoolean(Request.QueryString["creditFlag"]))
                {
                    ...
                    dView.RowFilter = "CreditFlag = 1";
                    DataTable dtFilter = dView.ToTable();

                    report.RegData("ReportData", dtFilter);
                }
                else
                {
            ...
            report.Dictionary.Variables["EndDate"].Value = DateTime.Now.ToLongDateString();
