...
public partial class Vendor1099 : System.Web.UI.Page
{
    #region "Variables"
    ...
    GetFederalReportParam _getFederalReport = new GetFederalReportParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {            
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtEndDate.Text = lastDay.Date.ToString("MM/dd/yyyy");
                }

                if (!string.IsNullOrEmpty(Request["gross"]))
                {
                    txt_dollar.Text = Request["gross"];
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "Federal1099.pdf";
    ...
    {
        var gross = 0.00;
        if (!string.IsNullOrEmpty(txt_dollar.Text))
        {
            gross = Convert.ToDouble(txt_dollar.Text.Replace("$", ""));
        }

        var url = "Vendor1099.aspx?sd=" + txtStartDate.Text + "&ed=" + txtEndDate.Text + "&gross=" + gross;

        Response.Redirect(url);
    }
   

    protected void StiWebViewerFederal_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerFederal_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/Federal1099.mrt");
            ...
            _objCD.ConnConfig = connString;
            _getFederalReport.ConnConfig = connString;

            int year = DateTime.Now.Year;

            if(!string.IsNullOrEmpty(Request.QueryString["sd"]))
            {
                _objCD.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
                _getFederalReport.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
            }
            else
            {
                _objCD.StartDate = new DateTime(year - 1, 1, 1);
                _getFederalReport.StartDate = new DateTime(year - 1, 1, 1);
            }

            if (!string.IsNullOrEmpty(Request.QueryString["ed"]))
            {
                _objCD.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
                _getFederalReport.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
            }
            else
            {
                _objCD.EndDate = new DateTime(year - 1, 12, 31);
                _getFederalReport.EndDate = new DateTime(year - 1, 12, 31);
            }

            if (!string.IsNullOrEmpty(Request.QueryString["gross"]))
            {
                _objCD.Amount = Convert.ToDouble(Request.QueryString["gross"]);
                _getFederalReport.Amount = Convert.ToDouble(Request.QueryString["gross"]);
            }
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "VendorAPI/VendorReport_GetFederalReport";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getFederalReport, true);
            ...
            {
                ds = objBL_Report.GetFederalReport(_objCD);
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = " Federal 1099 Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Federal 1099 Report attached.";
                }
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "Federal1099.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=Federal1099.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached  Federal 1099 Report.";
