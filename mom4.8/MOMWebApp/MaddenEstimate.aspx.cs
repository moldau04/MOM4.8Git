...
public partial class MaddenEstimate : System.Web.UI.Page
{
    ...
    protected void rvEstimate_ReportRefresh(object sender, System.ComponentModel.CancelEventArgs e)
    {
        GetEstimateAgreement();
    }
    ...
    private void GetEstimateAgreement()
    {
        try
        {
                ...
                rvEstimate.LocalReport.DataSources.Clear();
                rvEstimate.LocalReport.DataSources.Add(new ReportDataSource("dsEstimate", ds.Tables[0]));
                rvEstimate.LocalReport.DataSources.Add(new ReportDataSource("dsRol", ds.Tables[1]));
                rvEstimate.LocalReport.DataSources.Add(new ReportDataSource("dsEmp", ds.Tables[2]));
                rvEstimate.LocalReport.DataSources.Add(new ReportDataSource("dsCompany", dsC.Tables[0]));
                rvEstimate.LocalReport.ReportPath = "Reports/MaddenEstimate.rdlc";
                rvEstimate.LocalReport.EnableExternalImages = true;
                rvEstimate.LocalReport.EnableHyperlinks = true;
                //rvEstimate.LocalReport.SetParameters((new ReportParameter[] { rpUser }));
                rvEstimate.LocalReport.Refresh();
      ...
      private byte[] ExportReportToPDF(string reportName)
    {
        ...
        byte[] bytes = rvEstimate.LocalReport.Render(
