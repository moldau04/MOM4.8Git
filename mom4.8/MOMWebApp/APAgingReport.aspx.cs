...
public partial class APAgingReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(Request["Zero"]))
                {
                    int chkint = Convert.ToInt32(HttpUtility.UrlDecode(Request.QueryString["Zero"]));
                    if (chkint == 1)
                    {
                        lnkChk.Checked = true;
                    }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "APAgingReport.pdf";
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        //var url = "APAgingReport.aspx?stype=" + ddlInvoice.SelectedValue + "&sdate=" + txtSearchDate.Text + "&type=" + type;
        var includezerobill = lnkChk.Checked ? "1" : "0";
        var url = "APAgingReport.aspx?stype=" + ddlInvoice.SelectedValue + "&sdate=" + txtSearchDate.Text + "&type=" + type + "&Zero=" + includezerobill;
    ...
    private StiReport LoadReport(bool isExport = false)
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/APAgingReport.mrt");

            if (!string.IsNullOrEmpty(Request["type"]) && Request["type"] == "summary")
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/APAgingSummaryReport.mrt");
            }
            else if (isExport && (string.IsNullOrEmpty(Request["type"]) || Request["type"] == "detail"))
            {
                reportPathStimul = Server.MapPath("StimulsoftReports/APAgingToExportExcel.mrt");
            }
            ...
            {
                string APINAME = "BillAPI/BillsReport_GetCompanyDetails";
            ...
            //API
            _getBillsDetailsByDue.ConnConfig = connString;
            _getBillsDetailsByDue.UserID = Convert.ToInt32(Session["UserID"].ToString());

            _getAPAgingByDate.ConnConfig = connString;
            _getAPAgingByDate.UserID = Convert.ToInt32(Session["UserID"].ToString());

            if (!string.IsNullOrEmpty(Request["stype"]))
            {
                objPJ.SearchValue = Convert.ToInt16(Request.QueryString["stype"]);
                _getBillsDetailsByDue.SearchValue = Convert.ToInt16(Request.QueryString["stype"]);
            }
            else
            {
                objPJ.SearchValue = 3;
                _getBillsDetailsByDue.SearchValue = 3;
            }

            if (!string.IsNullOrEmpty(Request["sdate"]))
            {
                objPJ.SearchDate = Convert.ToDateTime(Request.QueryString["sdate"]);
                _getBillsDetailsByDue.SearchDate = Convert.ToDateTime(Request.QueryString["sdate"]);
                _getAPAgingByDate.SearchDate = Convert.ToDateTime(Request.QueryString["sdate"]);
            }
            else
            {
                objPJ.SearchDate = DateTime.Now;
                _getBillsDetailsByDue.SearchDate = DateTime.Now;
                _getAPAgingByDate.SearchDate = DateTime.Now;
            }

            if (Session["CmpChkDefault"].ToString() == "1")
            {
                objPJ.EN = 1;
                _getBillsDetailsByDue.EN = 1;
                _getAPAgingByDate.EN = 1;
            }
            else
            {
                objPJ.EN = 0;
                _getBillsDetailsByDue.EN = 0;
                _getAPAgingByDate.EN = 0;
            }
            if (!string.IsNullOrEmpty(Request["Zero"]))
            {
                objPJ.Frequency = Convert.ToInt32(Request.QueryString["Zero"]);
                _getBillsDetailsByDue.Frequency = Convert.ToInt32(Request.QueryString["Zero"]);
                _getAPAgingByDate.Frequency = Convert.ToInt32(Request.QueryString["Zero"]);
            }
            else
            {
                objPJ.Frequency = 0;
                _getBillsDetailsByDue.Frequency = 0;
                _getAPAgingByDate.Frequency = 0;
            }
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                if (_getBillsDetailsByDue.SearchValue != 3)
                {
                    List<GetBillsDetailsByDueViewModel> _lstGetBillsDetailsByDue = new List<GetBillsDetailsByDueViewModel>();

                    string APINAME = "BillAPI/BillsReport_GetBillsDetailsByDue";

                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getBillsDetailsByDue, true);

                    JavaScriptSerializer serializer = new JavaScriptSerializer();

                    serializer.MaxJsonLength = Int32.MaxValue;

                    _lstGetBillsDetailsByDue = serializer.Deserialize<List<GetBillsDetailsByDueViewModel>>(_APIResponse.ResponseData);

                    ds = CommonMethods.ToDataSet<GetBillsDetailsByDueViewModel>(_lstGetBillsDetailsByDue);
                    ...
                    report.Dictionary.Variables["EndDate"].Value = _getBillsDetailsByDue.SearchDate.ToLongDateString();
                }
                else
                {
                    _getAPAgingByDate.fDate = _getAPAgingByDate.SearchDate;

                    List<GetAPAgingByDateViewModel> _lstGetAPAgingByDate = new List<GetAPAgingByDateViewModel>();

                    string APINAME = "BillAPI/BillsReport_GetAPAgingByDate";

                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getAPAgingByDate, true);

                    JavaScriptSerializer serializer = new JavaScriptSerializer();

                    serializer.MaxJsonLength = Int32.MaxValue;

                    _lstGetAPAgingByDate = serializer.Deserialize<List<GetAPAgingByDateViewModel>>(_APIResponse.ResponseData);
                    ds = CommonMethods.ToDataSet<GetAPAgingByDateViewModel>(_lstGetAPAgingByDate);
                    ...
                    report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
                    report.Dictionary.Variables["EndDate"].Value = _getAPAgingByDate.SearchDate.ToLongDateString();
                }
            }
            else
            {
                ...
                {
                    ds = objBL_Bills.GetBillsDetailsByDue(objPJ);
                    if (ds != null && ds.Tables.Count > 0)
                    {
                        report.RegData("ReportData", ds.Tables[0]);
                    }
                ...
                else
                {
                    ...
                    ds = objBL_Bills.GetAPAgingByDate(objPJ);
