...
public partial class TicketCategoryDueReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            GetSMTPUser();
            SetAddress();
            string FileName = "CategoryDueReport.pdf";
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        url = RemoveQueryStringByKey(url, "cat");

        url += "&sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text + "&cat=" + GetSelectedCategory();

        Response.Redirect(url, true);
    }

    protected void StiWebViewerCategoryDue_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerCategoryDue_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    {
        string reportPathStimul = Server.MapPath("StimulsoftReports/TicketCategoryDueReport.mrt");
        ...
        if (Request["cat"] != null)
        {
            objPropMapData.Category = Request["cat"];
        }
        ...
        if (Session["RadGvTicketList_Filters"] != null)
        {
            ///Get  rad grid view  search filter value from TicketList View
            filters = (List<RetainFilter>)Session["RadGvTicketList_Filters"];
        }

        var ds = bL_Report.GetCategoryDueReport(objPropMapData, filters);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Ticket Category Due Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Ticket Category Due Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "CategoryDueReport.pdf")
                        {
                ...
                mail.FileName = "CategoryDueReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "CategoryDueReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=CategoryDueReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Ticket Category Due Report.";
    ...
    private void FillCategory()
    {
        ...
        DataSet ds = objBL_User.getCategory(objPropUser);

        chkcatlist.DataSource = ds.Tables[0];
        chkcatlist.DataTextField = "type";
        chkcatlist.DataValueField = "type";
        chkcatlist.DataBind();

        if (!string.IsNullOrEmpty(Request["cat"]))
        {
            var cats = HttpUtility.UrlDecode(Request.QueryString["cat"]);

            foreach (RadComboBoxItem item in chkcatlist.Items)
            {
                if (cats.Contains(item.Text))
                {
                    item.Checked = true;
                }
            }
