...
public partial class MaintenanceControlPlan : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "MaintenanceControlPlan.pdf";
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Maintenance Control Plan Report.";
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Maintenance Control Plan Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Maintenance Control Plan Report attached.";
                }
                ...
                service.ExportTo(LoadMCPReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "MaintenanceControlPlan.pdf")
                        {
                ...
                mail.FileName = "MaintenanceControlPlan.pdf";
    ...
    protected void StiWebViewerMCP_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadMCPReport();
    }

    protected void StiWebViewerMCP_GetReportData(object sender, StiReportDataEventArgs e)
    {

    }

    private StiReport LoadMCPReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/MaintenanceControlPlan.mrt");
            if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["MCPReport"]) && ConfigurationManager.AppSettings["MCPReport"].Contains(".mrt"))
            {
                reportPathStimul = Server.MapPath(string.Format("StimulsoftReports/{0}", ConfigurationManager.AppSettings["MCPReport"]));
            }
            ...
            {
                elevID = Convert.ToInt32(HttpUtility.UrlDecode(Request.QueryString["eid"]));
            }

            if (elevID > 0)
            {
                objChart.ConnConfig = Session["config"].ToString();
                var ds = objBL_Report.GetMaintenanceControlPlan(objChart, elevID);

                report.RegData("Elev", ds.Tables[0]);
                report.RegData("EquipTempItem", ds.Tables[1]);
            }
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "MaintenanceControlPlan.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=MaintenanceControlPlan.pdf");
