...
public partial class PreviewInvoiceWithTicket : System.Web.UI.Page
{
    ...
    private void LoadMailContent()
    {
        ...
        emailTemplate.FunctionName = "Preview Invoice With Ticket";
    ...
    private StiReport PrintInvoices(bool isAttachment)
    {
        try
        {
            int invoiceNo = Convert.ToInt32(Request.QueryString["uid"].ToString());
            ...
            // Template name
            string configRepostName = ConfigurationManager.AppSettings["InvoiceLNYWithTicket"].ToString();
            ...
            if (ConfigurationManager.AppSettings["CustomerName"] == "PCE")
            {
                if (dtInvoice.Rows.Count > 0)
                {
                    ...
                    if (dtInvoice.Rows[0]["TypeName"] != null && dtInvoice.Rows[0]["TypeName"].ToString() == "Installation" && !showItems)
                    {
                        pceInstallation = true;
                        configRepostName = "InstallationInvoiceWithTicket-PortCity.mrt";
    ...
    private byte[] GetReportAsAttachment()
    {
        ...
        service.ExportTo(PrintInvoices(true), stream, settings);
    ...
    {
        var url = "PreviewInvoiceWithTicket.aspx?uid=" + Request.QueryString["uid"] + "&showItems=" + chkShowItems.Checked;
    ...
    protected void LnkSend_Click(object sender, EventArgs e)
    {
        if (txtEmail.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                emailLog.Function = "Preview Invoice With Ticket";
                ...
                {
                    //ClientScript.RegisterStartupScript(Page.GetType(), "keyRefresh", "refreshPage(PreviewInvoice.aspx?uid="+ Request.QueryString["uid"].ToString() + ")", true);
                    ClientScript.RegisterStartupScript(Page.GetType(), "keySucc", "noty({text: 'Email sent successfully!',  type : 'success', layout:'topCenter',closeOnSelfClick:false, timeout : false,theme : 'noty_theme_default', closable: false, dismissQueue: true});", true);
                }
    ...
    {
        Response.Redirect("PreviewInvoiceWithTicket.aspx?uid=" + Request.QueryString["uid"].ToString());
    }
