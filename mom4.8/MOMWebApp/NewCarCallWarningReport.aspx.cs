...
public partial class NewCarCallWarningReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request["dfilter"]))
            {
                txtInstallationDate.Text = HttpUtility.UrlDecode(Request.QueryString["dfilter"]);
            }

            GetSMTPUser();
            SetAddress();
            string FileName = "CallWarningReport.pdf";
    ...
    {
        var regex = @"^((\=)|(\>)|(\<)|(\>=)|(\<=))((0?[13578]|10|12)(-|\/)((0[0-9])|([12])([0-9]?)|(3[01]?))(-|\/)((\d{4})|(\d{2}))|(0?[2469]|11)(-|\/)((0[0-9])|([12])([0-9]?)|(3[0]?))(-|\/)((\d{4})))$";
        Match match = Regex.Match(txtInstallationDate.Text, regex, RegexOptions.IgnoreCase);

        if (string.IsNullOrEmpty(txtInstallationDate.Text) || (!string.IsNullOrEmpty(txtInstallationDate.Text) && match.Success))
        {
            var url = Request.Url.ToString();
            url = RemoveQueryStringByKey(url, "sd");
            url = RemoveQueryStringByKey(url, "ed");
            url = RemoveQueryStringByKey(url, "dfilter");

            url += "&sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text + "&dfilter=" + HttpUtility.UrlEncode(txtInstallationDate.Text);

            Response.Redirect(url);
        }
        else
        {
            ScriptManager.RegisterStartupScript(this, Page.GetType(), "warningDateRange", "noty({text: 'Installation date incorrect format to filter.',  type : 'warning', layout:'topCenter',closeOnSelfClick:false, timeout : 5000,theme : 'noty_theme_default',  closable : true});", true);
        }
    }

    protected void StiWebViewerCallWarning_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadCallWarningReport();
    }

    protected void StiWebViewerCallWarning_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {

    }

    private StiReport LoadCallWarningReport()
    {
        var filterText = string.Empty;
        if (!string.IsNullOrEmpty(Request["dfilter"]))
        {
            filterText = HttpUtility.UrlDecode(Request.QueryString["dfilter"]);
        }

        var regex = @"^((\=)|(\>)|(\<)|(\>=)|(\<=))((0?[13578]|10|12)(-|\/)((0[0-9])|([12])([0-9]?)|(3[01]?))(-|\/)((\d{4})|(\d{2}))|(0?[2469]|11)(-|\/)((0[0-9])|([12])([0-9]?)|(3[0]?))(-|\/)((\d{4})))$";
        Match match = Regex.Match(filterText, regex, RegexOptions.IgnoreCase);

        if (string.IsNullOrEmpty(filterText) || (!string.IsNullOrEmpty(filterText) && match.Success))
        {
            DateTime filterDate = DateTime.MinValue;
            string filterOperator = string.Empty;
            string filterValue = string.Empty;

            if (!string.IsNullOrEmpty(filterText) && match.Success)
            {
                filterOperator = match.Groups[1].Value;
                filterDate = Convert.ToDateTime(filterText.Replace(filterOperator, ""));

                filterValue = string.Format("{0} '{1}'", filterOperator, filterDate);
            }

            string reportPathStimul = Server.MapPath("StimulsoftReports/NewCallWarningReport.mrt");
            ...
            var ds = bL_Report.GetCompletedTicketWarning(objPropMapData, filters, true, filterValue);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "New Car Call Warning Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the New Car Call Warning Report attached.";
                }
                ...
                service.ExportTo(LoadCallWarningReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "CallWarningReport.pdf")
                        {
                ...
                mail.FileName = "CallWarningReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "CallWarningReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=CallWarningReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached New Car Call Warning Report.";
