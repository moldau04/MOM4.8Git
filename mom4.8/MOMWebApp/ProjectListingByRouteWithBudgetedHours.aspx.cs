...
public partial class ProjectListingByRouteWithBudgetedHours : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["sd"]) && !string.IsNullOrEmpty(Request.QueryString["ed"]))
            {
                StiWebViewerProjectListingByRoute.Visible = true;
            }
            ...
            {
                var routes = HttpUtility.UrlDecode(Request["routes"]).Split(',');
                foreach (var route in routes)
                {
                    var item = rcRoute.Items.FindItem(x => x.Value == route);
                    if (item != null)
                    {
                        item.Checked = true;
                    }
            ...
            GetSMTPUser();
            SetAddress();
            string FileName = "ProjectListingByRouteWithBudgetedHours.pdf";
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        else if (rcRoute.CheckedItems.Count == 0)
        {
            ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyRouteDateWarning", "noty({text: 'Route is required. Please select route!',  type : 'warning', layout:'topCenter',closeOnSelfClick:true, timeout : 5000,theme : 'noty_theme_default',  closable : false});", true);
        }
        else
        {
            ...
            var url = "ProjectListingByRouteWithBudgetedHours.aspx?sd=" + txtFromDate.Text + "&ed=" + txtToDate.Text + "&routes=" + HttpUtility.UrlEncode(routes) + "&department=" + HttpUtility.UrlEncode(department);

            Response.Redirect(url);
        }
    }

    protected void StiWebViewerProjectListingByRoute_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadProjectListingReport();
    }

    protected void StiWebViewerProjectListingByRoute_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {

    }

    private StiReport LoadProjectListingReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/ProjectListingByRouteReport.mrt");
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["sd"]))
            {
                objChart.StartDate = Convert.ToDateTime(HttpUtility.UrlDecode(Request.QueryString["sd"]));
            }

            if (!string.IsNullOrEmpty(Request.QueryString["ed"]))
            {
                objChart.EndDate = Convert.ToDateTime(HttpUtility.UrlDecode(Request.QueryString["ed"])).AddDays(1).AddSeconds(-1);
            }

            var routes = string.Empty;
            if (!string.IsNullOrEmpty(Request.QueryString["routes"]))
            {
            ...
            var ds = bL_Report.GetProjectListingByRouteWithBudgetedHours(objChart, routes, department, false);
            if (ds != null && ds.Tables.Count > 0)
            {
                DataTable dtDetail = ds.Tables[0];

                DataSet dsReportData = new DataSet();
                DataTable dtARAging = dtDetail.Copy();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Project Listing by Route with Budgeted Hours Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Project Listing by Route with Budgeted Hours Report attached.";
                }
                ...
                service.ExportTo(LoadProjectListingReport(), stream, settings);
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "ProjectListingByRouteWithBudgetedHours.pdf")
                        {
                ...
                mail.FileName = "ProjectListingByRouteWithBudgetedHours.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ProjectListingByRouteWithBudgetedHours.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ProjectListingByRouteWithBudgetedHours.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Project Listing by Route with Budgeted Hours Report.";
    ...
    private void FillRoute()
    {
        User objPropUser = new User();
        objPropUser.ConnConfig = Session["config"].ToString();
        DataSet ds = objBL_User.getRouteActive(objPropUser);

        rcRoute.Items.Insert(0, new RadComboBoxItem("Unassigned", "0"));
        if (ds.Tables.Count > 0)
        {
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                var row = ds.Tables[0].Rows[i];
                rcRoute.Items.Insert(i + 1, new RadComboBoxItem(string.Format("{0} - {1}", row["Name"], row["MechName"]), row["ID"].ToString()));
            }
        }
    }
    private void FillDepartment()
    {
        ...
        DataSet ds = objBL_Job.GetAllJobType(objJob);
