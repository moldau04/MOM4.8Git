...
public partial class WriteChecksRecur : System.Web.UI.Page
{
    #region Variables
    ...
    GetRecurCDByIDParam _getRecurCDByID = new GetRecurCDByIDParam();
    GetRecurrBillDetailByIDParam _getRecurrBillDetailByID = new GetRecurrBillDetailByIDParam();
    UpdateCheckRecurrParam _updateCheckRecurr = new UpdateCheckRecurrParam();
    ...
    private void SetEditData()            //Initialization of Datatable.
    {
        try
        {
            ...
            _getRecurCDByID.ConnConfig = Session["config"].ToString();
            _getRecurCDByID.ID = Convert.ToInt32(Request.QueryString["id"]);
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "ManageChecksAPI/AddCheckRecurr_GetRecurCDByID";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getRecurCDByID);

                _lstGetRecurCDByIDViewModel = (new JavaScriptSerializer()).Deserialize<List<GetRecurCDByIDViewModel>>(_APIResponse.ResponseData);
                _dsCD = CommonMethods.ToDataSet<GetRecurCDByIDViewModel>(_lstGetRecurCDByIDViewModel);
                _dsCD.Tables[0].Columns["AcctNumber"].ColumnName = "Acct#";
            }
            else
            {
                _dsCD = _objBLBill.GetRecurCDByID(_objCD);
            }

            if (_dsCD.Tables[0].Rows.Count > 0)
            {
                DataRow _dr = _dsCD.Tables[0].Rows[0];
                txtMemo1.Text = _dr["BillfDesc"].ToString();
                txtbillref.Text = _dr["BillRef"].ToString();
                lblTotalAmount11.Text = _dr["BillAmount"].ToString();
                lblTotalUseTax.Text = _dr["BillUseTax"].ToString();
                txtVendor.Text = _dr["fDesc"].ToString();
                hdnVendorID.Value = _dr["Vendor"].ToString();
                chkIsRecurr.Checked = true;
                ddlFrequency.SelectedValue = _dr["Frequency"].ToString();
                hdnPJIdRecur.Value = _dr["PJID"].ToString();

                if (Convert.ToBoolean(_dr["IsRecon"]) == true)
                {
                    chkIsRecurr.Enabled = false;
                }
                BindEditRecCheckGrid();
    ...
    private void BindEditRecCheckGrid()
    {
        try
        {
            ...
            _getRecurrBillDetailByID.ConnConfig = Session["config"].ToString();
            _getRecurrBillDetailByID.PITR = Convert.ToInt32(Request.QueryString["id"]);
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "ManageChecksAPI/AddCheckRecurr_GetRecurrBillDetailByID";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getRecurrBillDetailByID);

                _lstGetRecurrBillDetailByIDViewModel = (new JavaScriptSerializer()).Deserialize<List<GetRecurrBillDetailByIDViewModel>>(_APIResponse.ResponseData);
                _ds = CommonMethods.ToDataSet<GetRecurrBillDetailByIDViewModel>(_lstGetRecurrBillDetailByIDViewModel);
            }
            else
            {
                _ds = _objBLBill.GetRecurrBillDetailByID(_objPaid);
            }
    ...
    private void BINDGRID(DataTable dt)
    {
        ...
        if (dsCustom.Tables[0].Rows.Count > 0)
        {
            if (!string.IsNullOrEmpty(dsCustom.Tables[0].Rows[0]["Label"].ToString()) && dsCustom.Tables[0].Rows[0]["Label"].ToString().Equals("1"))
            {
                ...
                RadGrid_gvJobCostItems.Columns[13].HeaderText = "Provincial Tax";
                ...
                if (Flag)
                {
                    ...
                    else
                    {
                        ...
                        if (Convert.ToDouble(GetBillItems().Compute("sum(Paid)", "").ToString()) == 0)
                        {
                            ...
                            _applyCredit.fDate = Convert.ToDateTime(txtaplyDate.Text);
                            _applyCredit.NextC = Convert.ToInt32("0");
    ...
    private void FillReportMaddenDataSet_New(string reportName)
    {
        try
        {
            ...
            foreach (DataRow dr in dtN.Rows)
            {
                ...
                int checkNo = Convert.ToInt32(dr["CheckNo"].ToString());
        ...
        try
        {
            ...
            if (int.Parse(ddlVendor.SelectedValue) <= 0)
            {
                ...
                foreach (DataRow dr in dtN.Rows)
                {
                    ...
                    double AmountPay = 0.00;
                    SumAmountpay = 0.00;
                    int checkNo = Convert.ToInt32(ViewState["Checkno"].ToString());
                    ...
                    foreach (GridDataItem gr in gvBills.Items)
                    {
                        ...
                        if (lblVendor.Text == dr["Name"].ToString())
                        {
                            ...
                            if (chkSelect.Checked == true)
                            {
                                ...
                                if (!string.IsNullOrEmpty(txtNextCheck.Text))
                                {
                                    checkNo = Convert.ToInt32(txtNextCheck.Text);
                                }
                                else
                                {
            ...
            else
            {
                ...
                //RAHIL'S IMPLEMENTATION
                //_objCD.ConnConfig = Session["config"].ToString();
                int checkno = Convert.ToInt32(ViewState["Checkno"]);
                _objCD.Ref = checkno;
                        ...
                        if (lblVendor.Text == dr["Name"].ToString())
                        {
                            ...
                            if (chkSelect.Checked == true)
                            {
                                ...
                                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                                {
                                    ...
                                    _lstGetVendorAcct = (new JavaScriptSerializer()).Deserialize<List<GetVendorAcctList>>(_APIResponse.ResponseData);
                                    da = CommonMethods.ToDataSet<GetVendorAcctList>(_lstGetVendorAcct);
                ...
                if (dsBank.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (ddlPayment.SelectedValue != "-1")
                    {
                        if (ddlPayment.SelectedValue == "0")
                        {
                            ...
                            checkno = !string.IsNullOrEmpty(dr["NextC"].ToString()) ? Convert.ToInt32(dr["NextC"]) : 0;
                            var nextCheck = Convert.ToInt32(checkno);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        if (ValidateGrid(dt))
                        {
                            if (chkIsRecurr.Checked)
                            {
                                ...
                                _updateRecurrBills.PostDate = Convert.ToDateTime(txtDate.Text);
                                _updateRecurrBills.Due = Convert.ToDateTime(txtDate.Text);
                                _updateRecurrBills.Ref = txtbillref.Text;
                                _updateRecurrBills.fDesc = txtMemo1.Text;
                                _updateRecurrBills.Terms = Convert.ToInt16("0");
                                _updateRecurrBills.Spec = Convert.ToInt16("0");
                                ...
                                //string strpjid = _objBLBills.AddBills(_objPJ);
                                _objPJ.ID = Convert.ToInt32(hdnPJIdRecur.Value);
                                _updateRecurrBills.ID = Convert.ToInt32(hdnPJIdRecur.Value);
                                ...
                                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                                {
                                    string APINAME = "ManageChecksAPI/AddCheckRecurr_UpdateRecurrBills";
                                ...
                                _objCD.TransID = Convert.ToInt32(hdnPJIdRecur.Value);
                                // _objCD.fDate = DateTime.Now;
                                _objCD.fDate = Convert.ToDateTime(txtDate.Text);
                                _objCD.NextC = long.Parse(txtNextCheck.Text);
                                ...
                                //API
                                _updateCheckRecurr.ConnConfig = Session["config"].ToString();
                                _updateCheckRecurr.Amount = sumAmount;
                                _updateCheckRecurr.fDateYear = Convert.ToInt32(ddlFrequency.SelectedValue);
                                _updateCheckRecurr.TransID = Convert.ToInt32(hdnPJIdRecur.Value);
                                _updateCheckRecurr.fDate = Convert.ToDateTime(txtDate.Text);
                                _updateCheckRecurr.NextC = long.Parse(txtNextCheck.Text);
                                _updateCheckRecurr.fDesc = txtVendor.Text;
                                _updateCheckRecurr.Bank = Convert.ToInt32(ddlBank.SelectedValue);
                                _updateCheckRecurr.Vendor = Convert.ToInt32(hdnVendorID.Value);
                                _updateCheckRecurr.Memo = txtMemo.Text;
                                _updateCheckRecurr.ID = Convert.ToInt32(Request.QueryString["Id"]);

                                if (!string.IsNullOrEmpty(hdnDiscGL.Value))
                                {
                                    _objCD.DiscGL = Convert.ToInt32(hdnDiscGL.Value);
                                    _updateCheckRecurr.DiscGL = Convert.ToInt32(hdnDiscGL.Value);
                                }

                                _objCD.Type = Convert.ToInt16(ddlPayment.SelectedValue);
                                _objCD.MOMUSer = Session["Username"].ToString();

                                _updateCheckRecurr.Type = Convert.ToInt16(ddlPayment.SelectedValue);
                                _updateCheckRecurr.MOMUSer = Session["Username"].ToString();
                                ...
                                if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
                                {
                                    string APINAME = "ManageChecksAPI/AddCheckRecurr_UpdateCheckRecurr";

                                    APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _updateCheckRecurr);
                                    _objCD.ID = Convert.ToInt32(_APIResponse.ResponseData);
                                }
                                else
                                {
                                    _objCD.ID = _objBLBill.UpdateCheckRecurr(_objCD);
                                }
