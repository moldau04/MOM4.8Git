...
public partial class InvoiceByRecurringFrequency : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtfromDate.Text = firstDay.ToString("MM/dd/yyyy");
                }
                ...
                {
                    drpCompany.SelectedValue = Request["companyID"];
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "InvoiceByRecurringFrequency.pdf";
    ...
    protected void StiWebViewerInvoiceFrequencyReport_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerInvoiceFrequencyReport_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
    ...
    {
        var url = "InvoiceByRecurringFrequency.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
        if(drpCompany.SelectedValue != "0")
        {
            url += "&companyID=" + drpCompany.SelectedValue;
        }
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/Invoices/InvoiceReportByRecurringFrequency.mrt");
            ...
            report.RegData("dsCompany", companyInfo.Tables[0]);
            ...
            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objContract.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
            }
            else
            {
                objContract.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }

            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objContract.EndDate = Convert.ToDateTime(Request.QueryString["ed"]).AddDays(1).AddSeconds(-1);
            }
            else
            {
                objContract.EndDate = DateTime.Now.Date.AddDays(1).AddSeconds(-1);
            }

            DataSet dsInvoice = objBLContracts.GetInvoiceByRecurringFrequency(objContract);
            DataTable dTable = new DataTable();
            if (!string.IsNullOrEmpty(Request["companyID"]) && !Request["companyID"].Equals("0"))
            {
                var filteredTable = new DataTable();
                DataView dataView = dsInvoice.Tables[0].DefaultView;
                dataView.RowFilter = "EN = " + Session["CompanyID"];
                filteredTable = dataView.ToTable();

                dTable = filteredTable;
            }
            else
            {
                dTable = dsInvoice.Tables[0];
            }

            var dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = 'Monthly'";
            dView.Sort = "EN ASC";
            DataTable Monthly = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = 'Bi-Monthly'";
            dView.Sort = "EN ASC";
            DataTable BiMonthly = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = 'Quarterly'";
            dView.Sort = "EN ASC";
            DataTable Quarterly = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = '3 Times/Year'";
            dView.Sort = "EN ASC";
            DataTable ThreeTimesPerYear = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = 'Semi-Annually'";
            dView.Sort = "EN ASC";
            DataTable SemiAnnually = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = 'Annually'";
            dView.Sort = "EN ASC";
            DataTable Annually = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = 'Never'";
            dView.Sort = "EN ASC";
            DataTable Never = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = '3 Years'";
            dView.Sort = "EN ASC";
            DataTable ThreeYears = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = '5 Years'";
            dView.Sort = "EN ASC";
            DataTable FiveYears = dView.ToTable();

            dView = dTable.DefaultView;
            dView.RowFilter = "BCycle = '2 Years'";
            dView.Sort = "EN ASC";
            DataTable TwoYears = dView.ToTable();

            report.RegData("Monthly", Monthly);
            report.RegData("BiMonthly", BiMonthly);
            report.RegData("Quarterly", Quarterly);
            report.RegData("ThreeTimesPerYear", ThreeTimesPerYear);
            report.RegData("SemiAnnually", SemiAnnually);
            report.RegData("Annually", Annually);
            report.RegData("Never", Never);
            report.RegData("ThreeYears", ThreeYears);
            report.RegData("FiveYears", FiveYears);
            report.RegData("TwoYears", TwoYears);

            report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
            report.Dictionary.Variables["StartDate"].Value = objContract.StartDate.ToLongDateString();
            report.Dictionary.Variables["EndDate"].Value = objContract.EndDate.ToLongDateString();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = " Invoice By Recurring Frequency Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the  Invoice By Recurring Frequency Report attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "InvoiceByRecurringFrequency.pdf")
                        {
                ...
                mail.FileName = "InvoiceByRecurringFrequency.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "InvoiceByRecurringFrequency.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=InvoiceByRecurringFrequency.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached  Invoice By Recurring Frequency Report.";
    ...
    private void FillCompany()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            drpCompany.DataTextField = ds.Tables[0].Columns["Name"].ToString();
            drpCompany.DataValueField = ds.Tables[0].Columns["CompanyID"].ToString();
            drpCompany.DataBind();

            drpCompany.Items.Add(new ListItem("Select Company", "0", true));
            drpCompany.Items.FindByText("Select Company").Selected = true;
        }
        else
        {
