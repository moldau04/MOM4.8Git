...
public partial class AddTicketNew : Page
{
    ...
    {
        HiddenFieldCanada.Value = "0";
        if (isCanadaCompany()) {
            HiddenFieldCanada.Value = "1";
        }
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                catch (Exception ex) { string ssss = ex.Message; }
                #endregion
    ...
    private void Permission()
    {
        ...
        if (Session["type"].ToString() != "am" && Session["type"].ToString() != "c")
        {
            ...
            //else if (Edit=="Y")
            //{
            //    if (EditResolved == "N" && ddlStatus.SelectedItem.ToString() != "Completed")
            //    {
            //        ddlStatus.Items.Remove(ddlStatus.Items.FindByText("Completed"));                  
            //    }
            //    if (DeleteResolved == "N" && ddlStatus.SelectedItem.ToString() == "Completed")
            //    {
            //        ddlStatus.Enabled = false;
            //    }
    ...
    protected void lnkSave_Click(object sender, EventArgs e)
    {
        ...
        {
            int invoiceId = 0;// txtInvoiceNo.Text;
            ...
            {
                ClientScript.RegisterStartupScript(Page.GetType(), "invalidInvoiceNo", "noty({text: 'Please input a valid invoice number or leave it blank.',  type : 'warning', layout:'topCenter',closeOnSelfClick:false, timeout : false,theme : 'noty_theme_default',  closable : true});", true);
                return;
            }
        ...
        try
        {
            ...
            if (ValidateINV_GRID(dtTicketINV))
            {
                ...
                if (Convert.ToInt32(ViewState["mode"]) == 1)
                {
                    ...
                    else
                    {
                        ...
                        if (ddlStatus.SelectedValue != "4")
                        {
                            ...
                            ScriptManager.RegisterStartupScript(this, Page.GetType(), "keyNotyConfirm", "notyConfirm('" + ViewState["ticid"].ToString() + "');", true);
                        }
                ...
                if (!string.IsNullOrEmpty(txtInvoiceNo.Text))
                {
                    ...
                    ChangeImage();
                    //todo
                    lnkInvoice.Attributes["onclick"] = "window.open('addinvoiceNew.aspx?o=1&uid=" + txtInvoiceNo.Text + "', 'Invoice', 'height=768,width=1280,scrollbars=yes');";
    ...
    protected void ddlRoute_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (wagesRage == "1")
        {
            ...
            if (ViewState["ProjectwageID"] != null)
            {
                string str = ViewState["ProjectwageID"].ToString();
        ...
        if (hdnProjectId.Value != string.Empty)
        {
            ...
            if (Convert.ToInt32(hdnProjectId.Value) != 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //foreach (ListItem li in ddlWage.Items)
                    //{
                    //    string str = ds.Tables[0].Rows[0]["wage"].ToString();
                    //    ViewState["ProjectwageID"] = str;
                    //    if (li.Value.Equals(str, StringComparison.CurrentCultureIgnoreCase))
                    //    {
                    string str = ds.Tables[0].Rows[0]["wage"].ToString();
                    ListItem item = ddlWage.Items.FindByValue(str);
                    if (item != null)
        ...
        if (ds.Tables[0].Rows[0]["invoice"] != DBNull.Value)
        {
            if (ds.Tables[0].Rows[0]["invoice"].ToString().Trim() != string.Empty && ds.Tables[0].Rows[0]["invoice"].ToString().Trim() != "0")
            {
                ...
                if (Session["MSM"].ToString() != "TS")
                {
                    lnkInvoice.Visible = true;
                    //todo
                    lnkInvoice.Attributes["onclick"] = "window.open('addinvoiceNew.aspx?o=1&uid=" + ds.Tables[0].Rows[0]["invoice"].ToString() + "', 'Invoice', 'height=768,width=1280,scrollbars=yes');";
