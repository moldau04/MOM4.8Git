...
public partial class ListOfLeadsReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                HighlightSideMenu("SalesMgr", "lnkProspect", "SalesMgrSub");

                GetSMTPUser();
                SetAddress();
                string FileName = "ListOfLeads.pdf";
    ...
    protected void StiWebViewerListOfLead_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerListOfLead_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/ListLeadsReport.mrt");
            ...
            var searchValue = Request.QueryString["SearchValue"];

            objPropCustomer.ConnConfig = Session["config"].ToString();
            objPropCustomer.SearchBy = searchBy;
            objPropCustomer.SearchValue = searchValue;

            DataSet dsLeads = objBL_Customer.getProspect(objPropCustomer);
            if (dsLeads != null && dsLeads.Tables.Count > 0)
            {
                report.RegData("ListLeads", dsLeads.Tables[0]);
            }

            DataSet dsContacts = objBL_Customer.getContactList(objPropCustomer);
            if (dsContacts != null && dsContacts.Tables.Count > 0)
            {
                report.RegData("ListContacts", dsContacts.Tables[0]);
            }
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "List of Leads Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the List of Leads Report attached.";
                }
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ListOfLeads.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ListOfLeads.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached List of Leads Report.";
