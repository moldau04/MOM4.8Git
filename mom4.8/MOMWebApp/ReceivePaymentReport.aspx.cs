...
public partial class ReceivePaymentReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "ReceivePayment.pdf";
    ...
    protected void StiWebViewerReceivePayment_GetReport(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {
        e.Report = LoadLReport();
    }

    protected void StiWebViewerReceivePayment_GetReportData(object sender, Stimulsoft.Report.Web.StiReportDataEventArgs e)
    {

    }

    private StiReport LoadLReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/ReceivePaymentReport.mrt");
            ...
            objReceivePay.ConnConfig = connString;
            objReceivePay.ID = Convert.ToInt32(Request.QueryString["id"]);
            var dsReceive = objBL_Deposit.GetReceivePaymentByID(objReceivePay);
            double balance = 0;

            if (dsReceive != null && dsReceive.Tables[0].Rows.Count > 0)
            {
                objPayment.ConnConfig = connString;
                objPayment.ReceivedPaymentID = Convert.ToInt32(dsReceive.Tables[0].Rows[0]["ID"]);
                objPayment.Rol = Convert.ToInt32(dsReceive.Tables[0].Rows[0]["Owner"]);
                objPayment.Loc = Convert.ToInt32(dsReceive.Tables[0].Rows[0]["Loc"]);
                DataSet dsDetail = objBL_Deposit.GetInvoicesByReceivedPay(objPayment);

                report.RegData("ReceivePaymentHead", dsReceive.Tables[0]);
                report.RegData("ReceivePayment", dsDetail.Tables[0]);

                if (dsDetail.Tables[1].Rows.Count > 0)
                {
                    balance = Convert.ToDouble(dsDetail.Tables[1].Rows[0]["Balance"]);
                }
            }

            report.RegData("CompanyDetails", companyInfo.Tables[0]);
            report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
            report.Dictionary.Variables["DepositTo"].Value = Session["DepositTo"] != null ? Session["DepositTo"].ToString() : "";
            report.Dictionary.Variables["Balance"].Value = balance.ToString();
            report.Render();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Receive Payment Report";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Receive Payment Report attached.";
                }
                ...
                service.ExportTo(LoadLReport(), stream, settings);
                ...
                mail.FileName = "ReceivePayment.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "ReceivePayment.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=ReceivePayment.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Receive Payment Report.";
