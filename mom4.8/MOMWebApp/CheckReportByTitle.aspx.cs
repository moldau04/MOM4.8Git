...
public partial class CheckReportByTitle : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    StiWebViewerCheckReportByTitle.Visible = true;
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "CheckReportByTitle.pdf";
                ...
                //FillSupervisor();
                FillTitle();
                if (!string.IsNullOrEmpty(Request["Emp.CType"]))
                {
                    var routes = HttpUtility.UrlDecode(Request["Emp.CType"]).Split(',');
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "CheckReportByTitle.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=CheckReportByTitle.pdf");
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/CheckReportByTitle.mrt");
            ...
            if (!string.IsNullOrEmpty(Request["Emp.CType"]))
            {
                Category = HttpUtility.UrlDecode(Request["Emp.CType"]).Split(',');
                objPropContracts.Ctype = HttpUtility.UrlDecode(Request.QueryString["Emp.CType"]);
            }

            DataSet ds = objBL_Contracts.GetCheckReportByTitle(objPropContracts, null, null, null);
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                mail.Title = "Check Report By Title";
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Check Report By Title attached.";
                }
                ...
                if (ViewState["pathmailatt"] != null)
                {
                    ...
                    {
                        if (strpath != "CheckReportByTitle.pdf")
                        {
                ...
                mail.FileName = "CheckReportByTitle.pdf";
    ...
    {
        var url = "CheckReportByTitle.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
        ...
        //}
        if (rbDepartment.CheckedItems.Count != 0)
        {
            var routes = GetRadComboBoxSelectedItems(rbDepartment);
            url = url + "&Emp.CType=" + routes;
        }
    ...
    protected void StiWebViewerCheckReportByTitle_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    //Report Data Set Event
    protected void StiWebViewerCheckReportByTitle_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    private void FillTitle()
    {
        DataSet ds = new DataSet();
        objPropUser.ConnConfig = Session["config"].ToString();
        ds = objBL_User.getTitle(objPropUser);
        rbDepartment.DataSource = ds.Tables[0];
        rbDepartment.DataTextField = "Title";
        rbDepartment.DataValueField = "Title";
