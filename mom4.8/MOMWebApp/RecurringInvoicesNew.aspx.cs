...
public partial class RecurringInvoicesNew : System.Web.UI.Page
{
    #region Variable
        ...
        try
        {
            ...
            if ((bool)ViewState["FlagPeriodClose"])
            {
                ...
                foreach (DataRow drow in dtr.Rows)
                {
                    ...
                    objProp_Contracts.JobId = Convert.ToInt16(drow["OrgLoc"].ToString());
                ...
                {                   
                    ...
                    if (isCanada)
                    {
                        ...
                        //Save UI history
                        objProp_Contracts.TaxApply = ddlTaxType.SelectedValue;
                        objProp_Contracts.isCanadaCompany = isCanada;
    ...
    protected void lnkSave_Click(object sender, EventArgs e)
    {
        ...
        if (reportFormat.Equals("RDLC"))
            Response.Redirect("PrintInvoices.aspx?uid=" + objProp_Contracts.InvoiceID + "&eid=" + objProp_Contracts.InvoiceEndID + "&m=" + monthName + "&y=" + year + "&check=1", true);
        else
            Response.Redirect("PrintInvoicesMrt.aspx?uid=" + objProp_Contracts.InvoiceID + "&eid=" + objProp_Contracts.InvoiceEndID + "&m=" + monthName + "&y=" + year + "&check=1", true);
    }
    #endregion
    ...
    private void BindRecurringInvoice(int? taxable = 0, bool isreset = false)
    {
        try
        {
            //todo
            ...
            if (isCanada)
            {
                ds = objBL_Contracts.GetListRecurringInvoices(objProp_Contracts);
            }
            else
            {
                ds = objBL_Contracts.GetRecurringInvoices(objProp_Contracts);
            }
    ...
    private void BindGridDatatable(DataTable dt)
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (dr["amount"] != DBNull.Value && Convert.ToString( dr["amount"]) != "")
                    { 
                    ...
                    if (dr["total"] != DBNull.Value && Convert.ToString(dr["total"]) != "")
                    {
    ...
    protected void chkTaxable_CheckedChanged(object sender, EventArgs e)
    {
        if (chkTaxable.Checked == true)
        {
            BindRecurringInvoice(1);
        }
        else
        {
            BindRecurringInvoice(0);
        }
    }
