...
public partial class OAddCustomerNew : System.Web.UI.Page
{
    Loc _objLoc = new Loc();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            foreach (Tuple<RadioButton, HtmlGenericControl> objPair in listRadio)
            {
                ...
                objPair.Item2.Attributes["Class"] = "btn btn-primary btn-sm " + (objPair.Item1.Checked ? " active" : "");
        ...
        switch (eventTarget)
        {
            case "ctl00_ContentPlaceHolder1_TabContainer1_tpViewInvoicelinks_rdDay":
                AddClass();
                rdDay_CheckedChanged();
                GetData();
                break;
            case "ctl00_ContentPlaceHolder1_TabContainer1_tpViewInvoicelinks_rdWeek":
                AddClass();
                rdWeek_CheckedChanged();
                GetData();
                break;
            case "ctl00_ContentPlaceHolder1_TabContainer1_tpViewInvoicelinks_rdMonth":
                AddClass();
                rdMonth_CheckedChanged();
                GetData();
                break;
            case "ctl00_ContentPlaceHolder1_TabContainer1_tpViewInvoicelinks_rdQuarter":
                AddClass();
                rdQuarter_CheckedChanged();
                GetData();
                break;
            case "ctl00_ContentPlaceHolder1_TabContainer1_tpViewInvoicelinks_rdYear":
                AddClass();
    ...
    public bool CheckAddEditPermission()
    {
        ...
        if (Session["type"].ToString() != "am" && Session["type"].ToString() != "c")
        {
            ...
            { 
                ...
                else if (ViewOwner == "Y" && stEditeOwner == "N")
                {
    ...
    private DataTable SaveDocInfo()
    {
        ...
        foreach (GridViewRow gr in gvDocuments.Rows)
        {
            ...
            CheckBox chkPortal = (CheckBox)gr.FindControl("chkPortal");
            TextBox txtRemarks = (TextBox)gr.FindControl("txtRemarks");
            CheckBox chkMSVisible = (CheckBox)gr.FindControl("MSVisible");
    ...
    private void CalculateBalance()
    {
        ...
        lblEquipTotal.Text = string.Format("{0:0.00}", equip);
    ...
    private void BindInvoiceGridDatatable(DataTable dt)
    {
        ...
        {
            if ((row["Type"].ToString() == "Received Payment") && Convert.ToDouble(row["Amount"].ToString()) > 0)
            {
                //row["Amount"] = Convert.ToDouble(row["Amount"].ToString()) * (-1);
            }
            
            row["RunTotal"]= Convert.ToDouble(row["amount"].ToString()) + Runtotal;
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            Label lblRunTotal = (Label)gvInvoice.FooterRow.FindControl("lblRunTotal");
            lblRunTotal.Text = string.Format("{0:c}", dt.Rows[rowCount - 1]["RunTotal"], string.Empty);
        }
    }
