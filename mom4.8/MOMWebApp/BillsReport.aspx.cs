...
public partial class BillsReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    txtToDate.Text = firstDay.AddMonths(1).AddDays(-1).ToString("MM/dd/yyyy");
                }

                if (!string.IsNullOrEmpty(Request["itype"]))
                {
                    ddlInvoice.SelectedValue = HttpUtility.UrlDecode(Request.QueryString["itype"]);
                }
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "BillsReport.pdf";
    ...
    protected void StiWebViewerBills_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerBills_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    {
        Response.Redirect("managebills.aspx?f=1");
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        url = RemoveQueryStringByKey(url, "itype");
        url = RemoveQueryStringByKey(url, "sdate");

        url += "&sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text + "&iype=" + ddlInvoice.SelectedValue;

        if (ddlInvoice.SelectedValue.Equals("2"))
        {
            url += "&sdate=" + txtSearchDate.Text;
        }
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/BillsSummaryReport.mrt");
            ...
            if (!string.IsNullOrEmpty(Request["itype"]))
            {
                if (Request["itype"].Equals(2))
                {
                    ...
                    else
                    {
                        ...
                        {
                            objPJ.SearchDate = Convert.ToDateTime(Request["sdate"]);
                        }
                    }
                }
            }
            ...
            objPJ.SearchwithmultipleStatus = "99";

            DataSet ds = objBL_Bills.GetAllPJDetails(objPJ);
            ...
            report.Dictionary.Variables["StartDate"].Value = objPJ.StartDate.ToLongDateString();
            report.Dictionary.Variables["EndDate"].Value = objPJ.EndDate.ToLongDateString();
            report.Render();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Bills Report attached.";
                }
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "BillsReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=BillsReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Bills Report.";
