...
public partial class UseTaxReport : System.Web.UI.Page
{
    ...
    GetUseTaxForReportsParam _getUseTaxForReports = new GetUseTaxForReportsParam();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "UseTaxReport.pdf";
    ...
    {
        var url = "UseTaxReport.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/UseTaxDetailsReport.mrt");
            ...
            _getUseTaxForReports.ConnConfig = connString;

            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objPJ.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
                _getUseTaxForReports.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
            }
            else
            {
                objPJ.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                _getUseTaxForReports.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }

            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objPJ.EndDate = Convert.ToDateTime(Request.QueryString["ed"]).AddDays(1).AddSeconds(-1);
                _getUseTaxForReports.EndDate = Convert.ToDateTime(Request.QueryString["ed"]).AddDays(1).AddSeconds(-1);
            }
            else
            {
                objPJ.EndDate = DateTime.Now.AddMonths(1).AddDays(1).AddSeconds(-1);
                _getUseTaxForReports.EndDate = DateTime.Now.AddMonths(1).AddDays(1).AddSeconds(-1);
            }
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                string APINAME = "BillAPI/BillsReport_GetUseTax";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _getUseTaxForReports, true);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetUseTax = serializer.Deserialize<List<GetUseTaxViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<GetUseTaxViewModel>(_lstGetUseTax);
            }
            else
            {
                ds = objBL_ReportsData.GetUseTax(objPJ);
            }
            ...
            if (_objAPIIntegration.IsAPIIntegrationForAPModule == true)
            {
                ...
                report.Dictionary.Variables["StartDate"].Value = _getUseTaxForReports.StartDate.ToLongDateString();
                report.Dictionary.Variables["EndDate"].Value = _getUseTaxForReports.EndDate.ToLongDateString();
            }
            else
            {
                report.Dictionary.Variables["Username"].Value = Session["Username"].ToString();
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the Use Tax Report attached.";
                }
                ...
                mail.FileName = "UseTaxReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "UseTaxReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=UseTaxReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached Use Tax Report.";
