...
public partial class POWeeklyReport : System.Web.UI.Page
{
    #region Variables
    ...
    GetPOWeeklyParam _GetPOWeekly = new GetPOWeeklyParam();
    #endregion
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                GetSMTPUser();
                SetAddress();
                string FileName = "POWeeklyReport.pdf";
    ...
    protected void StiWebViewerPOWeekly_GetReport(object sender, StiReportDataEventArgs e)
    {
        e.Report = LoadReport();
    }

    protected void StiWebViewerPOWeekly_GetReportData(object sender, StiReportDataEventArgs e)
    {
    ...
    {
        var url = "POWeeklyReport.aspx?sd=" + txtfromDate.Text + "&ed=" + txtToDate.Text;
    ...
    private StiReport LoadReport()
    {
        try
        {
            string reportPathStimul = Server.MapPath("StimulsoftReports/POWeeklyReport.mrt");
            ...
            {
                string APINAME = "ItemAdjustmentAPI/POWeeklyReport_GetCompanyDetails";
            ...
            objPO.ConnConfig = connString;
            _GetPOWeekly.ConnConfig = connString;
            if (!string.IsNullOrEmpty(Request["sd"]))
            {
                objPO.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
                _GetPOWeekly.StartDate = Convert.ToDateTime(Request.QueryString["sd"]);
            }
            else
            {
                objPO.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                _GetPOWeekly.StartDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            }

            if (!string.IsNullOrEmpty(Request["ed"]))
            {
                objPO.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
                _GetPOWeekly.EndDate = Convert.ToDateTime(Request.QueryString["ed"]);
            }
            else
            {
                objPO.EndDate = objPO.StartDate.AddMonths(1).AddDays(-1);
                _GetPOWeekly.EndDate = objPO.StartDate.AddMonths(1).AddDays(-1);
            }
            ...
            if (IsAPIIntegrationEnable == "YES")
            {
                string APINAME = "ItemAdjustmentAPI/POWeeklyReport_GetPOWeekly";

                APIResponse _APIResponse = new MOMWebUtility().CallMOMWebAPI(APINAME, _GetPOWeekly, true);

                JavaScriptSerializer serializer = new JavaScriptSerializer();

                serializer.MaxJsonLength = Int32.MaxValue;

                _lstGetPOWeekly = serializer.Deserialize<List<GetPOWeeklyViewModel>>(_APIResponse.ResponseData);
                ds = CommonMethods.ToDataSet<GetPOWeeklyViewModel>(_lstGetPOWeekly);
            }
            else
            {
                ds = objBL_Report.GetPOWeekly(objPO);
            }
            ...
            if (IsAPIIntegrationEnable == "YES")
            {
                report.Dictionary.Variables["StartDate"].Value = _GetPOWeekly.StartDate.ToLongDateString();
                report.Dictionary.Variables["EndDate"].Value = _GetPOWeekly.EndDate.ToLongDateString();
            }
            else
            {
                report.Dictionary.Variables["StartDate"].Value = objPO.StartDate.ToLongDateString();
                report.Dictionary.Variables["EndDate"].Value = objPO.EndDate.ToLongDateString();
            }
    ...
    private void GetSMTPUser()
    {
        ...
        if (IsAPIIntegrationEnable == "YES")
        {
            string APINAME = "ItemAdjustmentAPI/POWeeklyReport_GetSMTPByUserID";
    ...
    protected void hideModalPopupViaServerConfirm_Click(object sender, EventArgs e)
    {
        if (txtTo.Text.Trim() != string.Empty)
        {
            try
            {
                ...
                {
                    mail.Text = "This is report email sent from Mobile Office Manager. Please find the PO Weekly Report attached.";
                }
                ...
                mail.FileName = "POWeeklyReport.pdf";
    ...
    private void DownloadDocument(string filePath, string DownloadFileName)
    {
        ...
        {
            if (DownloadFileName == "POWeeklyReport.pdf")
            {
                byte[] buffer1 = null;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=POWeeklyReport.pdf");
    ...
    private void SetAddress()
    {
        ...
        string mailBody = "Please review the attached PO Weekly Report.";
