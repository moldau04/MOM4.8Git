...
namespace Bryntum.Gantt
{
    public class Gantt
    {
        ...
        public Gantt()
        {
            var connectionString = "metadata=res://*/;provider=System.Data.SqlClient;provider connection string=\"" + Convert.ToString(HttpContext.Current.Session["config"]) + ";MultipleActiveResultSets=True;App=EntityFramework;\"";
            //context.Database.Connection.ConnectionString = connectionString;
            context = new GanttEntities(connectionString);
        }
        ...
        public void saveTaskSegment(TaskSegment segment)
        {
            ...
            {
                var queryStr = @"insert [TaskSegments]([TaskIdRaw],[StartDate],[EndDate],[Duration],[DurationUnit],[Cls]) values(@p0,@p1,@p2,@p3,@p4,@p5);
        ...
        public void saveTask(Task task)
        {
            ...
            {
                var queryStr = @"INSERT[Tasks]([parentIdRaw],[Name],[StartDate],[EndDate],[Duration],[DurationUnit],[PercentDone],[SchedulingMode]
      ,[BaselineStartDate],[BaselineEndDate],[BaselinePercentDone],[Cls],[index],[CalendarIdRaw],[expanded],[Effort],[EffortUnit],[Note]
      ,[ConstraintType],[ConstraintDate],[ManuallyScheduled],[Draggable],[Resizable],[Rollup],[ShowInTimeline],[Color],[ProjectID])
      ...
      select SCOPE_IDENTITY();";
                ...
                var id = context.Database.SqlQuery<Decimal>(queryStr,
                    ...
                    task.Resizable, task.Rollup, task.ShowInTimeline, task.Color, task.ProjectID).Single();
        ...
        public void removeTask(int id)
        {
            ...
            {
                foreach (int childId in context.Database.SqlQuery<int>("select Id from Tasks where parentIdRaw = @p0;", id).ToList())
                {
                    removeTask(childId);
                }
        ...
        public void saveAssignment(Assignment assignment)
        {
            ...
            {
                var queryStr = @"insert [Assignments]([TaskIdRaw],[ResourceIdRaw],[Units]) values(@p0,@p1,@p2); select SCOPE_IDENTITY();";
        ...
        public void saveResource(Resource resource)
        {
            ...
            {
                var queryStr = @"insert [Resources]([Name],[CalendarIdRaw]) values (@p0,@p1); select SCOPE_IDENTITY();";
        ...
        /// <param name="id">Dependency identifier.</param>
        /// <returns></returns>
        public Dependency getDependency(int from, int to)
        {
            return context.Dependencies.Where(x=>x.FromIdRaw == from && x.ToIdRaw == to).FirstOrDefault();
        }
        ...
        /// <param name="id">Dependency identifier.</param>
        /// <returns></returns>
        public IList<Dependency> getDependenciesFromTask(int fromTask)
        {
            return context.Dependencies.Where(x => x.FromIdRaw == fromTask).ToList();
        }
        ...
        public void saveDependency(Dependency dependency)
        {
            ...
            {
                var queryStr = @"insert [Dependencies]([FromIdRaw],[ToIdRaw],[Type],[Cls],[Lag],[LagUnit]) values(@p0,@p1,@p2,@p3,@p4,@p5); select SCOPE_IDENTITY();";
        ...
        public void saveCalendarDay(CalendarDay day)
        {
            ...
            {
                var queryStr = @"insert [CalendarDays]([CalendarIdRaw],[Name],[Type],[Date],[AvailabilityRaw],[Weekday],[OverrideStartDate],[OverrideEndDate]
                                ,[IsWorkingDayRaw],[Cls]) values(@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7,@p8,@p9); select SCOPE_IDENTITY();";
                var id = context.Database.SqlQuery<decimal>(queryStr, day.CalendarIdRaw, day.Name, day.Type, day.Date, day.AvailabilityRaw,
                    day.Weekday, day.OverrideStartDate, day.OverrideEndDate, day.IsWorkingDayRaw, day.Cls).First();
                day.Id = decimal.ToInt32(id);
            }
        }
        ...
        public void saveCalendar(Calendar calendar)
        {
            ...
            {
                var queryStr = @"insert [Calendars]([parentIdRaw],[Name],[DaysPerMonth],[DaysPerWeek],[HoursPerDay],[WeekendsAreWorkdaysRaw],[WeekendFirstDay]
                                  ,[WeekendSecondDay],[DefaultAvailabilityRaw]) values(@p0,@p1,@p2,@p3,@p4,@p5,@p6,@p7,@p8); select SCOPE_IDENTITY();";
                var id = context.Database.SqlQuery<decimal>(queryStr,
                    calendar.parentIdRaw, calendar.Name, calendar.DaysPerMonth, calendar.DaysPerWeek, calendar.HoursPerDay,
                    calendar.WeekendsAreWorkdaysRaw, calendar.WeekendFirstDay, calendar.WeekendSecondDay, calendar.DefaultAvailabilityRaw).First();
        ...
        public void UpdateTasks(List<Task> modifiedTasks)
        {
            Task task = null;
        ...
        private Task UpdateTask(Task task, Task modifiedTask)
        {
            if (modifiedTask.BaselineEndDate.HasValue) task.BaselineEndDate = modifiedTask.BaselineEndDate;
            if (modifiedTask.BaselinePercentDone.HasValue) task.BaselinePercentDone = modifiedTask.BaselinePercentDone;
            if (modifiedTask.BaselineStartDate.HasValue) task.BaselineStartDate= modifiedTask.BaselineStartDate;
            if (modifiedTask.CalendarIdRaw.HasValue) task.CalendarIdRaw = modifiedTask.CalendarIdRaw;
            if (modifiedTask.ConstraintDate.HasValue) task.ConstraintDate= modifiedTask.ConstraintDate;
            if (modifiedTask.Duration.HasValue) task.Duration= modifiedTask.Duration;
            if (modifiedTask.Effort.HasValue) task.Effort= modifiedTask.Effort;
            if (modifiedTask.EndDate.HasValue) task.EndDate= modifiedTask.EndDate;
            if (modifiedTask.parentIdRaw.HasValue) task.parentIdRaw = modifiedTask.parentIdRaw;
            if (modifiedTask.PercentDone.HasValue) task.PercentDone = modifiedTask.PercentDone;
            if (modifiedTask.StartDate.HasValue) task.StartDate = modifiedTask.StartDate;
            if (!string.IsNullOrEmpty(modifiedTask.Cls)) task.Cls = modifiedTask.Cls;
            if (!string.IsNullOrEmpty(modifiedTask.Color)) task.Color = modifiedTask.Color;
            if (!string.IsNullOrEmpty(modifiedTask.ConstraintType)) task.ConstraintType= modifiedTask.ConstraintType;
            if (!string.IsNullOrEmpty(modifiedTask.DurationUnit)) task.DurationUnit= modifiedTask.DurationUnit;
            if (!string.IsNullOrEmpty(modifiedTask.EffortUnit)) task.EffortUnit= modifiedTask.EffortUnit;
            if (!string.IsNullOrEmpty(modifiedTask.Name)) task.Name= modifiedTask.Name;
            if (!string.IsNullOrEmpty(modifiedTask.Note)) task.Note= modifiedTask.Note;
            if (!string.IsNullOrEmpty(modifiedTask.PhantomCalendarId)) task.PhantomCalendarId= modifiedTask.PhantomCalendarId;
            if (!string.IsNullOrEmpty(modifiedTask.SchedulingMode)) task.SchedulingMode = modifiedTask.SchedulingMode;
            return task;
        }

        public void UpdateDependencies(List<Dependency> deps, int projectId)
        {
            // Delete unused dependencies
            //var ddd = deps.Select(x => x.FromIdRaw).Distinct();
            var ddd = context.Tasks.Where(x=>x.ProjectID == projectId).Select(x => x.Id);

            foreach (var item in ddd)
            {
                var depsTask = getDependenciesFromTask(item);
                if (depsTask != null && depsTask.Count() > 0)
                {
                    foreach (var de in depsTask)
                    {
                        var isUnUsed = deps.Where(x => x.FromIdRaw == de.FromIdRaw && x.ToIdRaw == de.ToIdRaw).Count() == 0;
                        if (isUnUsed)
                        {
                            // Delete
                            context.Dependencies.Remove(de);
                        }
                    }
                }
            }
            ...
            {
                dependency = getDependency(item.FromIdRaw, item.ToIdRaw);
                if (dependency != null)
                {
                    // Update current dependencies;
                    if (item.Lag.HasValue) dependency.Lag = item.Lag;
                    if (!string.IsNullOrEmpty(item.LagUnit)) dependency.LagUnit = item.LagUnit;
                    if (item.Type.HasValue) dependency.Type = item.Type;
                }
                else
                {
                    // Add new dependency
                    var queryStr = @"insert [Dependencies]([FromIdRaw],[ToIdRaw],[Type],[Cls],[Lag],[LagUnit]) values(@p0,@p1,@p2,@p3,@p4,@p5); select SCOPE_IDENTITY();";
                    var id = context.Database.SqlQuery<decimal>(queryStr, item.FromIdRaw, item.ToIdRaw, item.Type, item.Cls, item.Lag, item.LagUnit).First();
                    item.Id = decimal.ToInt32(id);
                    //AddedIds["dependencies"].Add(item.PhantomId, item.Id);
                }
            }
        }
    }
