    ...
    public class DL_GLARecur
    {
        ...
        public DataSet GetTransDataByRef(Transaction objTrans)
        {
            try
            {
                ...
                varname.Append("        SELECT r.ID, r.Ref, r.Line, r.fDesc, r.fDesc as Description, r.Amount, r.Acct, r.Acct as AcctID, r.Job as JobID, r.Phase as PhaseID, c.fDesc AS Account, c.Acct AS AcctNo,         \n");
                ...
                varname.Append("                j.fDesc as JobName, jobt.fDesc as Phase, 0 as IsRecon, l.Tag as Loc, 0 as Sel, br.Name As Company,jobt.Type AS TypeID           \n");
                ...
                varname.Append("           left join JobTItem as jobt on jobt.Line = r.Phase and jobt.Job = r.Job and jobt.Type = r.TypeID   \n");
                varname.Append("                    left join Rol  ro on  l.Rol=ro.ID     \n");
                varname.Append("                    left outer join Branch br on br.ID = ro.EN     \n");
        ...
        public void DeleteGLARecur(Journal objJournal)
        {
            try
            {
                SqlParameter paraRecur = new SqlParameter();
                paraRecur.ParameterName = "Ref";
                paraRecur.SqlDbType = SqlDbType.Int;
                paraRecur.Value = objJournal.Ref;

                SqlHelper.ExecuteNonQuery(objJournal.ConnConfig, CommandType.StoredProcedure, "spDeleteRecurJE", paraRecur);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public DataSet GetProcessTransByDate(Journal objJournal)
        {
            try
            {
                return objJournal.DsTrans = SqlHelper.ExecuteDataset(objJournal.ConnConfig, CommandType.Text, "SELECT Ref ,fDate, Internal, fDesc, Frequency, 0 as Batch, 0 As IsCleared, ISNULL((SELECT SUM(Amount) FROM GLARecurI WHERE Ref = GLARecur.Ref And Amount > 0),0.00) AS Debit, ISNULL((SELECT SUM(Amount * -1) FROM GLARecurI WHERE Ref = GLARecur.Ref  And Amount < 0),0.00) AS Credit FROM GLARecur Where fDate >= '" + objJournal.StartDate + "' AND fDate <='" + objJournal.EndDate + "' Order by fDate, Ref desc");
            }
            catch (Exception ex)
            {
                throw ex;
